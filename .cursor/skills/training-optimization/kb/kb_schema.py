"""Knowledge base entry schema, validation, and conflict detection for training optimization."""
...
def resolve_conflict(new_entry: dict, conflict: dict) -> dict:
    ...
    new_is_controlled = any(
        ...
        for kw in ["controlled", "fair a/b", "same session", "matched params", "same gbs"]
    )
    ...
    different_config = "different_parallelism" in new_tags or "different_parallelism" in old_tags

    if different_config:
        resolution["action"] = "KEEP_BOTH"
        resolution["reason"] = "Different operating points (parallelism/MBS)"
    elif new_is_controlled and not old_is_controlled:
        resolution["action"] = "SUPERSEDE"
        resolution["reason"] = "New entry has stricter methodology (controlled A/B, same GBS)"
