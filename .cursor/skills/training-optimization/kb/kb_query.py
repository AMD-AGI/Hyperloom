#!/usr/bin/env python3
"""Query the training optimization knowledge base.

Usage:
    python3 kb_query.py "MoE permute fusion GPT-OSS"
...
def main():
    parser = argparse.ArgumentParser(description="Query the training optimization KB")
    ...
    results = query(
        query_text=args.query, model=args.model, gpu=args.gpu,
        framework=args.framework, category=args.category, entry_id=args.id,
        tags=tag_list, top_k=args.top_k, min_confidence=args.min_confidence,
    )
