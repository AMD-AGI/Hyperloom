...
> **状态**: Final v0.5（v0.4 基础上把 leases / events / cursors / tasks 4 类持久化合并到单库 SQLite WAL，新增 ADR-33）  
...
> **v0.4 → v0.5 摘要**: §3.5 / §10.1 / §13 全部改用 SQLite WAL 单库（4 张表：`leases` / `events` / `cursors` / `tasks`）实现"原子化 + 跨类型事务一致性 + NFS-safe"；DB 文件落 sandbox 本地盘，定期 backup 到 `$SESSION_DIR/checkpoints/`（生产 NFS 路径 `/hyperloom/inference-optimizer-sessions/<session_id>/`）；新增 ADR-33；删除"durable file lease"细节并保留语义契约
...
5. **结构化 intent + 资源锁**：Claude 角色通过 `emit_intent` tool 发意图；Codex 角色不使用 tools，只输出 validated JSON intent；sandbox 内 4 个资源 lane 通过 **SQLite WAL 单库 + `BEGIN IMMEDIATE` 单事务**实现原子互斥（v0.5：见 §3.5 / ADR-33）；PolicyGate 按 mode 校验所有 intent。
...
╚══════════════════════════════╤════════════════════════════════════════════════╝
                               ↓
        ╔════════════════════════════════════════════════════════════╗
        ║   Shared NFS ($SESSION_DIR/)  v0.5 — SQLite + 文件 hybrid   ║
        ║   生产路径: /hyperloom/inference-optimizer-sessions/<id>/   ║
        ║                                                             ║
        ║  ▼ SQLite 单库（含 ACID 事务）— 通过定期 backup 落 NFS      ║
        ║  - storage/conductor.db         4 张表：leases / events    ║
        ║                                  / cursors / tasks（§13.2） ║
        ║  - storage/conductor.db-wal     WAL                          ║
        ║  - storage/conductor.db-shm     shared memory                ║
        ║                                                             ║
        ║  ▼ 文件类（大块、append/snapshot 友好）                     ║
        ║  - state.json                  (L2 snapshot, 30min/KEEP 后) ║
        ║  - personas/<agent>.md         (L3, marathon 才蒸馏)        ║
        ║  - checkpoints/<ts>/           (含 conductor.db.bak)        ║
        ║  - kb/entries.jsonl + insights.jsonl  (L4, 全 mode 读写)    ║
        ║  - results/<task_id>/          (sub-agent 大块输出)         ║
        ║  - findings/<id>/              (RCA 结果, marathon+guided emergency) ║
        ║                                                             ║
        ║  ▶ DB 部署位置 (§3.5.8)                                     ║
        ║    路径 A (推荐): conductor.db 落 sandbox 本地盘 +          ║
        ║       VACUUM INTO 到 NFS checkpoints/                       ║
        ║    路径 B: conductor.db 直落 NFS + apsw unix-dotfile VFS   ║
        ╚════════════════════════════════════════════════════════════╝
```
...
| F02 | ResourceLockManager（4 lane + SQLite leases 表，v0.5） | ✓ | ✓ | ✓ |
...
v0.5 把 §3.5 的 leases、§10.1 的 events、§13.3 的 cursors、§13.4 的 tasks **合并到同一个 SQLite WAL 库**（`storage/conductor.db`），通过 `BEGIN IMMEDIATE` 单事务获得跨表 ACID。锁实现分两层：

1. `ResourceLockManager`：Conductor 内的统一仲裁入口，负责 action → lanes、权限检查、超时、死锁检测。
2. `ResourceLockBackend`：底层锁后端，默认用 SQLite `leases` 表 + `BEGIN IMMEDIATE` 事务；语义契约见 §3.5.7。

```python
class ResourceLockBackend(Protocol):
    async def acquire_many(self, lanes: list[str], holder_id: str,
                           task_id: str, action: str, ttl_sec: int) -> Lease:
        """Atomically acquire all lanes or acquire none (single SQLite txn)."""
    async def heartbeat(self, lease: Lease, ttl_sec: int) -> None:
        """Extend lease expiry while task is still alive."""
    async def release(self, lease: Lease) -> None:
        """Release only if holder_id still matches; idempotent."""
    async def reap_expired(self) -> list[ExpiredLease]:
        """Detect & remove stale lease whose expires_at has passed,
        emitting lease_expired event in the same txn."""


class SqliteLeaseBackend:
    """P0 default: 单 SQLite 库 leases 表 + BEGIN IMMEDIATE 原子事务。"""
    def __init__(self, db: SqliteConnection):
        self.db = db

    async def acquire_many(self, lanes, holder_id, task_id, action, ttl_sec):
        # 1. BEGIN IMMEDIATE 事务（writer 排他）
        # 2. SELECT * FROM leases WHERE lane IN (?,?,?,?)
        # 3. 对每条已有 row：若 expires_at 未过期 → 抛 LaneBusy 整体 ROLLBACK
        # 4. 对过期 row：先 DELETE 并 emit lease_expired event 入 events 表
        # 5. INSERT 全部 lane 行；任何冲突自动 ROLLBACK
        # 6. COMMIT 拿 Lease 对象
        ...
```

### 3.5.5 原子 multi-lane lease 规则（v0.5：单 SQLite 事务保证）

- 所有 action 只能调用 `acquire_many(required_lanes)`，禁止嵌套获取单 lane。
- v0.5 起 lane 获取顺序由 SQL 事务隔离保证一致性；不再需要应用层"按字典序"排序。
- 获取失败时事务自动 ROLLBACK，不留半状态；同时在事务内 emit `lease_acquire_failed` event。
- lease 必须有 TTL 和 heartbeat；超时后由 `reap_expired` 在新事务内写 `lease_expired` event 后才 DELETE。
- `workspace_mutation` P0 按全局独占锁处理（PRIMARY KEY 即 lane 名）；真正 reader/writer lock 延后。
- **acquire_many 失败策略**：非阻塞 + 指数退避（100ms → 1s → 5s），总等待上限 `action.lease_ttl × 2`；超过则任务转 `failed` 并把 action 回灌给调度器，不阻塞 reactor。
- 接入第三方锁实现的契约不变：atomic acquire-many、holder 校验、TTL/heartbeat、release 幂等。

### 3.5.6 Conductor 调度时检查

调度器决定下一个 action 时，如果它需要的 lane 已被占，要么 wait，要么选另一个 action。这通过 action.metadata 里的 `requires_lanes` 字段声明。真正执行前仍必须调用 `acquire_many()`；调度检查只是优化，不能替代 lease。可用一句 `SELECT lane FROM leases WHERE expires_at > ?` 拿到当前活跃 lane 集合，O(常数)。

### 3.5.7 SQLite Backend 部署 & 语义契约

P0 默认 SQLite WAL 单库。Design Gate 已通过的关键决策：

- **DB 部署位置**（§3.5.8 详）：路径 A（推荐）= 本地盘 + 定期 backup 到 NFS；路径 B = 直落 NFS + `apsw unix-dotfile` VFS。
- **生产 NFS 根目录**：`/hyperloom/inference-optimizer-sessions/<session_id>/`，`<session_id>` 由 conductor 启动时生成（见 §13.2）。

无论 SQLite 还是接入第三方锁实现，都必须满足同一语义契约：

- `acquire_many(lanes)` 要么拿到全部 lane，要么一个都不持有（SQLite 用单事务 ROLLBACK 实现）。
- lock holder 必须包含 `holder_id`、`task_id`、`action`、`lanes`、`expires_at`、`heartbeat_at`、`pid`。
- stale lock 不能静默覆盖：`reap_expired` 必须先 INSERT `lease_expired` event 再 DELETE lease 行，两者同事务。
- release 必须校验 holder_id，且重复 release 安全（用 `DELETE ... WHERE lane=? AND holder_id=?`）。
- Resume 时必须 `SELECT * FROM leases WHERE expires_at > now()` 把仍未过期的 lease 纳入 lock summary，防止恢复后立即污染 benchmark/profile。

### 3.5.8 DB 部署位置选型

| 路径 | DB 文件位置 | NFS 同步 | 优点 | 缺点 |
|---|---|---|---|---|
| **A（推荐）** | sandbox 本地盘 `/var/inference-optimizer/<session_id>/conductor.db` | 30min `VACUUM INTO` 到 `$SESSION_DIR/checkpoints/<ts>/conductor.db.bak` | 标准 stdlib `sqlite3`；写延迟 < 1ms；NFS 锁不可靠风险被消除 | 跨 sandbox 重启需从 backup 恢复；最坏丢 ≤ 30min 状态（对 §13.2 来说可接受，KEEP 后立即 backup） |
| **B** | NFS `$SESSION_DIR/storage/conductor.db` | 直接写 NFS | 无显式 backup 步骤；resume 直接打开 | 必须用 `apsw + unix-dotfile` VFS 避开 NFS fcntl bug；写吞吐降 ~5×；NFS 不稳时整库受影响 |

**P0 选 A**。理由：
1. 跟 §3.1 "单 sandbox 默认" 一致 —— 当前没有跨 sandbox 共享 DB 需求。
2. 标准 `sqlite3` 在本地盘上的并发/原子性是业界最稳定的存储栈之一。
3. 30min `VACUUM INTO` + 每次 KEEP 后 force backup 已经覆盖"最坏丢多少状态"约束。
4. 如未来需要跨 sandbox（§23 T1/T2），可平滑迁到路径 B 而不改上层代码（接口在 `ResourceLockBackend` / `MessageBus` / `TaskRegistry` / `CursorStore` 都已抽象）。
...
- **G6**: SQLite `storage/conductor.db`（events/cursors/tasks/leases）+ state.json + personas/ + kb/ 完整 4 层记忆（L1-L4，KB 全 mode 启用）
...
| **L2 本 session 工作记忆** | action_history / KEEP-REVERT / state / messages | `state.json` + SQLite `storage/conductor.db`（events/cursors/tasks/leases，v0.5） | session（含 resume） | Conductor | ✓ | ✓ | ✓ |
...
### 10.1 Message Envelope（v0.5：SQLite events 表为 SoT）

SQLite `events` 表是 A2A Bus 的 source of truth；`asyncio.Queue` 只是运行时投递缓存。所有 message 必须先 `INSERT INTO events` 拿到 `AUTOINCREMENT` 主键作为全局递增 `seq`，再投递到内存 queue。Resume 时按 cursor 用 `SELECT ... WHERE seq > ?` 重放，不能依赖 queue 状态。

**seq 分配语义（v0.5 简化）**：SQLite 自己用 `AUTOINCREMENT` 给单调递增的 `seq`，应用层不再需要 `asyncio.Lock`。

```python
class MessageBus:
    def __init__(self, db: SqliteConnection):
        self.db = db   # storage/conductor.db, 表 events 已建好

    async def append_and_seq(self, msg: Message) -> int:
        """原子操作：INSERT 一行，主键 = seq。可与其他事务（如 cursor 推进、
        task transition）合并到同一个 BEGIN IMMEDIATE 事务里取得跨表原子性。"""
        async with self.db.transaction() as cur:
            cur.execute(
                "INSERT INTO events (msg_id, from_agent, to_agent, topic, "
                "in_reply_to, payload, priority, ts) VALUES (?,?,?,?,?,?,?,?)",
                (msg.id, msg.from_agent, msg.to, msg.topic, msg.in_reply_to,
                 json.dumps(msg.payload), msg.priority, msg.ts.isoformat()),
            )
            return cur.lastrowid   # = msg.seq

    async def tail(self, n: int = 200, *, after_seq: int = 0) -> list[Message]:
        """供 Watchdog/Critic RCA 拉最近 N 条 events。"""
...
### 13.2 v0.5 新设计：SQLite WAL 单库 + 文件大块 hybrid

**核心数据结构**（v0.5 收口：4 类原本分散文件合并到单 SQLite 库）：

```
$SESSION_DIR/   ← 生产: /hyperloom/inference-optimizer-sessions/<session_id>/
│
├── storage/                         ← v0.5 新增：SQLite ACID 单库
│   ├── conductor.db                 ← 4 张表：leases / events / cursors / tasks
│   ├── conductor.db-wal             ← WAL（PRAGMA journal_mode=WAL）
│   └── conductor.db-shm             ← shared memory
│
├── state.json                       ← L2 snapshot（30min / KEEP 后 dump 一份摘要 / 给人读）
├── personas/<agent>.md              ← L3 人格（marathon 才蒸馏）
├── checkpoints/<ts>/                ← 完整快照（含 conductor.db.bak via VACUUM INTO）
├── findings/<ts>.json               ← RCA 结果（marathon 常驻 watchdog / guided ephemeral）
├── kb/                              ← L4 跨 run KB
│   ├── entries.jsonl
│   └── insights.jsonl
└── results/<task_id>/               ← sub-agent 大块输出（trace / metrics / kernel src）
```
...
| 数据类 | SoT | 备注 |
|---|---|---|
| A2A 消息 / 通知 | `events` 表 | `seq` = AUTOINCREMENT PK，唯一全局递增 |
| Resource lease | `leases` 表 | PK = `lane`；TTL/heartbeat 列 |
| Task lifecycle | `tasks` 表 | `idempotency_key` 唯一，`history` JSON 列 |
| Per-agent cursor | `cursors` 表 | PK = `agent`；`last_processed_seq` 单调 |
| Snapshot 摘要 | `state.json` | 给人读，DB 是真理；Resume 不依赖它 |
| Sub-agent 大块产物 | `results/<task_id>/` | 不放 DB（trace / kernel src 太大） |
| KB | `kb/*.jsonl` | 沿用 sprint，仍是 jsonl |

**跨表原子性**：cursor 推进 + task transition + emit `delegated_result` event 全部在**同一个 SQLite 事务**里完成，崩在中间整体 ROLLBACK。这一点是 v0.4 分散文件方案做不到的。

**DB 部署**：见 §3.5.8。生产路径 A 把 `storage/conductor.db` 落 sandbox 本地盘，每 30min 或每次 KEEP 后用 SQLite `VACUUM INTO` 备份到 `$SESSION_DIR/checkpoints/<ts>/conductor.db.bak`。

### 13.3 Message 处理：cursor SQL UPSERT（v0.5）
...
async def process_message(self, agent_name, msg):
    ...
    # 处理（reactor 内部产生的 emit_event / task transition / delegated_result
    # 都注入同一个事务，整体 ACID）
    async with self.db.transaction() as cur:
        await self._reactor_handle_within_tx(agent_name, msg, cur)

        # cursor UPSERT：last_seq + last_id 一起更新
        cur.execute(
            "INSERT INTO cursors(agent, last_processed_seq, last_processed_msg_id, processed_at) "
            "VALUES (?,?,?,?) "
            "ON CONFLICT(agent) DO UPDATE SET "
            "  last_processed_seq=excluded.last_processed_seq, "
            "  last_processed_msg_id=excluded.last_processed_msg_id, "
            "  processed_at=excluded.processed_at",
            (agent_name, msg.seq, msg.id, now_iso()),
        )
```

**好处**：
- 一句 UPSERT 原子写，比 v0.4 tmp+rename 更便宜，且崩在中间不会留半文件
- Resume 时 `SELECT last_processed_seq FROM cursors WHERE agent=?`，再 `SELECT * FROM events WHERE seq > ?`，天然不重不漏
- 重复检测只看 `seq <= last_processed_seq`，不需要单独的 marker 目录
- **关键**：cursor 推进、reactor 副作用产生的 events/tasks 写入、`delegated_result` 通知，全部可以装进同一个事务，崩在中间 ROLLBACK 整体回滚（v0.4 分散文件做不到）

### 13.4 委托任务状态机（v0.5：tasks 表 + evidence-check 优先 retry）
...
class TaskRegistry:
    """所有 task 持久化到 SQLite tasks 表。transition 必须走这个入口。"""
    def __init__(self, db: SqliteConnection):
        self.db = db

    async def create(self, kind, params, idempotency_key, requires_lanes,
                     allowed_tools, side_effects, lease_ttl_sec) -> Task:
        ...
        async with self.db.transaction() as cur:
            cur.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?,?,0,'[]',?,?)",
                (task_id, kind, "queued", json.dumps(params), idempotency_key,
                 json.dumps(requires_lanes), json.dumps(allowed_tools),
                 json.dumps(side_effects), lease_ttl_sec,
                 now_iso(), now_iso()),
            )
        return Task(task_id=task_id, kind=kind, state="queued", ...)

    async def transition(self, task_id, new_state, evidence):
        # 单 SQL UPDATE：把 state、updated_at、attempts、history 一并改
        async with self.db.transaction() as cur:
            cur.execute(
                ...
                "WHERE task_id=?",
                (new_state, now_iso(),
                 json.dumps({"state": new_state, "ts": now_iso(), "evidence": evidence}),
                 task_id),
            )


async def dispatch_task(registry: TaskRegistry, task: Task, action):
    """v0.5: retry 前必须按 evidence-check 决定。所有 transition 都走 SQLite。"""
    await registry.transition(task.task_id, "running", {"sub_agent_pid": ...})
    try:
        result = await sub_agent_runner.run(task)
        await registry.transition(task.task_id, "succeeded", result.evidence)
        return result
    except Exception as e:
        await registry.transition(task.task_id, "failed", {"exception": str(e)})

        # evidence-check 优先 retry
        if not action.side_effects:
            # 纯只读 action：可以自动重试
            if task.attempts < MAX_RETRY:
                await registry.bump_attempts(task.task_id)
                return await dispatch_task(registry, task, action)
            raise
        else:
            # 副作用 action：检查 evidence
            ev = evidence_check(task, action)
            if ev.verdict == "succeeded_recovered":
                await registry.transition(task.task_id, "succeeded",
                                          {"recovered": True, **ev.details})
                return load_result_from_evidence(task)
            else:
                # evidence 不足 → needs_manual_review，不自动重跑
                await registry.transition(task.task_id, "needs_manual_review", {
...
### 13.5 Resume 流程（v0.5：单 DB 一发起所有恢复查询）

```python
async def resume_from_session_dir(session_dir):
    db = open_sqlite(session_dir / "storage" / "conductor.db")  # WAL mode

    # 1. 加载 cursors（一句 SQL）
    cursors = {row["agent"]: CursorState(**row)
               for row in await db.fetchall("SELECT * FROM cursors")}

    # 2. 加载 personas（仍是文件）
    personas = load_personas(session_dir / "personas/")

    # 3. 处理所有 in-flight 任务（一句 SQL）
    inflight = await db.fetchall("SELECT * FROM tasks WHERE state='running'")
    for row in inflight:
        ...
        if ev.verdict == "succeeded":
            ...
            await registry.transition(task.task_id, "failed",
                                      {"reason": "crashed_mid_run, no side effects"})
    ...
    # 4. 释放过期 leases（一句 SQL + emit lease_expired event）
    expired = await lock_mgr.reap_expired()

    # 5. 重放 cursor 之后的 events 给每个 agent reactor
    for agent_name, cursor in cursors.items():
        events = await db.fetchall(
            "SELECT * FROM events WHERE to_agent=? AND seq>? ORDER BY seq",
            (agent_name, cursor.last_processed_seq),
        )
        for ev in events:
            await self.bus.deliver_inmemory(agent_name, Message.from_row(ev))

    return ResumeState(cursors=cursors, personas=personas)
```

### 13.6 副作用 Action 崩溃点恢复矩阵

所有有副作用 action 必须写入 SQLite `tasks` 表（v0.5），并在 `history` JSON 列中追加 evidence。Resume 时不根据"上次跑到哪一行"猜测，而是按可验证 evidence 判定状态。
...
├── __init__.py
├── paths.py                          # 生产 NFS 路径常量（/hyperloom/.../<session_id>/）
│                                       + 本地 dev/test 通过 INFERENCE_OPTIMIZER_SESSION_ROOT 覆盖
├── storage/                          # v0.5 新增：SQLite WAL 单库后端
│   ├── __init__.py
│   ├── connection.py                 # SqliteConnection: 同步 sqlite3 + asyncio.to_thread + WAL
│   ├── schema.py                     # 4 张表 DDL + migration + reset
│   └── backup.py                     # VACUUM INTO → checkpoints/<ts>/conductor.db.bak
├── orchestrator/
│   ├── __init__.py
│   ├── conductor.py
│   ├── agent_role.py
│   ├── message_bus.py                # §10.1 用 events 表 + AUTOINCREMENT seq
│   ├── cursor_store.py               # §13.3 cursors 表 UPSERT
│   ├── shared_state.py
│   ├── scheduler.py
│   ├── score_priors.py               # §9.2 Initial Score Priors 表
│   ├── objective.py                  # §8
│   ├── execution_mode.py             # §3.4 ExecutionMode
│   ├── feature_flags.py              # §3.4.4 build_feature_flags
│   ├── policy.py                     # §10.5.8 PolicyGate (含 quick allowlist)
│   ├── intent_parser.py              # §10.5
│   ├── resource_lock.py              # §3.5 SqliteLeaseBackend + ResourceLockManager
│   ├── task_registry.py              # §13.4 tasks 表 + transitions
│   ├── sub_agent_runner.py           # §11
│   ├── persona.py                    # §5.3 蒸馏（仅 marathon）
│   ├── sage_query_service.py         # §5.1.2 KB 查询服务（quick/guided 用）
│   ├── kb.py                         # §6 全 mode 启用
│   ├── checkpoint.py                 # §13 + storage/backup.py
...
└── tests/
    ├── __init__.py
    ├── conftest.py                    # tmp session_dir + clean db fixture
    ├── test_storage_schema.py         # SQLite WAL + 4 张表创建
    ├── test_resource_lock.py          # acquire_many ROLLBACK 原子性
    ├── test_message_bus.py            # AUTOINCREMENT seq 单调 + concurrent append
    ├── test_task_registry.py          # transition 状态机 + needs_manual_review
    ├── test_cursor_store.py           # UPSERT 幂等
    ├── test_objective.py              # 4 种 Objective 实现
    ├── test_execution_mode.py         # MAX_HOURS → mode
    ├── test_feature_flags.py          # mode → flag 矩阵
    └── e2e/                            # 端到端：resume from crashed task
```
...
| 资源锁死锁 | 中 | 高 | SQLite leases 表 + `BEGIN IMMEDIATE` 单事务原子 multi-lane lease + TTL + heartbeat + 调度器全局检测；死锁触发紧急 release + alert |
| SQLite over NFS 锁可靠性 | 中 | 高 | 路径 A：DB 落 sandbox 本地盘 + 定期 backup 到 NFS（默认）；路径 B：用 `apsw + unix-dotfile` VFS 跑 NFS（备选） |
| 跨 sandbox 重启丢 ≤ 30min 状态 | 中 | 中 | KEEP/integrate 等关键决策后立即 `VACUUM INTO` 强制备份；30min 周期备份兜底 |
...
| 9 (v0.5) | 持久化原子性方案 | SQLite WAL 单库 `storage/conductor.db`，4 张表 leases/events/cursors/tasks，DB 落 sandbox 本地盘 + 定期 backup 到 NFS（路径 A） |
| 10 (v0.5) | 生产 NFS 路径 | `/hyperloom/inference-optimizer-sessions/<session_id>/`；本地 dev/test 通过 `INFERENCE_OPTIMIZER_SESSION_ROOT` 环境变量覆盖 |
...
| ADR-18 | 资源锁模型 4 个 lane（**ADR-33 v0.5 superseded**：底层后端从 durable file lease 换成 SQLite leases 表） | 无锁 / 全局单锁 / 仅内存 asyncio.Lock | 共享 sandbox 必须显式资源模型，否则并行变测量污染；锁需跨进程/恢复可见 |
...
| **ADR-33 (v0.5)** | **leases / events / cursors / tasks 4 类持久化合并到 SQLite WAL 单库 `storage/conductor.db`，DB 落 sandbox 本地盘 + 定期 `VACUUM INTO` backup 到 NFS（路径 A）** | （a）durable file lease + jsonl event log + per-agent cursor 文件 + per-task json 文件（v0.4 方案），（b）DB 直落 NFS 走 `apsw + unix-dotfile` VFS（路径 B），（c）外部 KV（etcd/redis） | 单 SQLite 事务可同时写 cursor + tasks + events + leases，§13.3/§13.4 跨表原子性 v0.4 做不到；标准 stdlib `sqlite3` 在本地盘上的并发/原子性是业界最稳；NFS 写延迟与 fcntl bug 都被规避；30min `VACUUM INTO` + KEEP-after backup 已经覆盖跨 sandbox 重启的"最坏丢多少状态"约束；外部 KV 引入额外运维与凭据管理成本，单 sandbox 不必要 |
...
5. **资源锁**：4 个 lane + SQLite leases 表（v0.5，ADR-33）/ 现有锁接入是否覆盖所有冲突场景？
...
| T9 | SQLite leases 表后端落地（含死锁检测、TTL/heartbeat、reap_expired）+ 路径 A backup 频率调优 | 实施前 | TBD | 未启动（v0.5：底层方案已切换，留接入测量） |
