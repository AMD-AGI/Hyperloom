...
In both modes, all GPU work runs inside the RayJob image where
...
1. Submit a Python driver as one Ray Job (`POST /api/jobs/`) that uses
   `ray.init()` to talk to the local cluster.
...
## ADDENDUM-07: Always bootstrap OOB / GEAK / TraceLens inside the RayJob before kernel-opt

The default RayJob image (e.g. `sglang:*`) is a vanilla framework image. It
does **not** ship the OOB CLI, GEAK CLI, claude CLI, codex CLI, TraceLens, the
...
`kernel-opt/llm.md` (LLM Proxy direct API)" — that violates IR-7b
...
2. Verify with one short follow-up REST job before continuing:
   ...
   {"entrypoint":
     "source /etc/profile.d/hyperloom-env.sh && \
      which oob && which claude && which codex && which geak && which ray && \
      oob --help | head -5"}
   ```

   If any of `oob` / `claude` / `codex` / `geak` / `ray` is missing, re-run
...
4. OOB / GEAK / claude / codex are installed inside the **RayJob pods**, not
...
5. Do not silently fall back to `kernel-opt/llm.md` (LLM Proxy direct API)
...
`mcp_call.js`, `ray_submit.js`, `ray_poll.js`, `call_claude.js`, ad-hoc
...
1. Before writing any helper into `/workspace/`, check whether the same
   ...
   - A script under `$SKILL_ROOT/scripts/` (e.g. `run_baseline.sh`,
     ...
     `geak_ray_submit.py`, `trace_action.py`, `bootstrap.sh`).
...
4. Never call out to LLM gateways from sandbox bash (`call_claude.js` and
   friends). All LLM-backed kernel work goes through the configured
   `KERNEL_OPT_BACKENDS` (OOB / GEAK / Codex / Claude CLI) inside the
...
1. When creating the RayJob via `workload_create`, propagate the current
   ...
   - `GEAK_API_KEY` = current sandbox `SAFE_API_KEY` unless explicitly provided
   ...
   - `GEAK_BASE_URL` = `https://core42.primus-safe.amd.com/api/v1/llm-proxy/v1`
...
3. If the RayJob was already created without credentials and the env file is
   ...
   POST http://<head_ip>:8265/api/jobs/
   {
     "entrypoint": "bash -lc 'export SAFE_API_KEY=\"$SAFE_KEY\" OOB_API_KEY=\"$SAFE_KEY\" AMD_LLM_API_KEY=\"$SAFE_KEY\" LLM_API_KEY=\"$SAFE_KEY\" GEAK_API_KEY=\"$SAFE_KEY\" ANTHROPIC_API_KEY=\"$SAFE_KEY\" OPENAI_API_KEY=\"$SAFE_KEY\" OOB_BASE_URL=\"https://core42.primus-safe.amd.com/api/v1/llm-proxy/v1\" GEAK_BASE_URL=\"https://core42.primus-safe.amd.com/api/v1/llm-proxy/v1\"; bash /wekafs/yunkai/Hyperloom/.cursor/skills/inference-optimization/scripts/bootstrap.sh --force'"
   }
   ```

4. When an OOB / Claude / GEAK command reports a missing API key
   (`no --api-key`, `ANTHROPIC_API_KEY not set`, `apiKeySource: none`, empty
   GEAK config `api_key`), the agent must immediately use the currently
   available sandbox key as above. Do not continue with unauthenticated OOB,
   and do not silently fall back to direct LLM Proxy calls.