...
| **TraceLens not called** | Said "called TraceLens" but didn't actually run analysis | Must read the Analysis Orchestrator skill at `TraceLens/Agent/Analysis/.cursor/skills/analysis-orchestrator.md` and run the full agentic analysis workflow it defines (see `actions/profile.md` Step 2). The orchestrator runs the perf-report CLI and `orchestrator_prepare.py` internally — Hyperloom no longer calls them directly. |
