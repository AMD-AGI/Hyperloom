# Local Mode — Complete Execution Reference

This document contains ALL local-mode-specific instructions. Read this **before starting**
...
IR-7b). This file defines local-specific Iron Rules (IR-12 through IR-16), constants,
...
Selected explicitly by `MODE=local`:
...
The GEAK REST API server (`python -m server.main`) does NOT run in local mode.
...
The OOB MCP server and `oob_client.py` REST flow are GONE in local mode.
...
local task workspace (see "OOB in Local Mode" below).

### IR-16: Orchestrator MUST NOT bypass backends by writing kernels directly

In local mode the orchestrator Agent and the OOB `claude` / `codex` backend
...
| GPU scheduling | Ray local cluster | SaFE cluster scheduler |
| OOB | `oob_ray_submit.py run` (Ray-scheduled, GPU-isolated) | `oob_client.py` (REST) |
| OOB service | None (Ray task per invocation) | Remote REST API |
| TraceLens | CLI (offline) | CLI (offline) |
| Shell commands | Direct | `exec_on_gpu` wrapper |
| RayJob | None | SaFE-managed |
...
[`../kernel-opt/claude.md`](../kernel-opt/claude.md) "Local Execution"
...
**GEAK backend:** Use `geak_ray_submit.py` (see "GEAK in Local Mode" above).

**Codex / Claude backend:** Use `oob_ray_submit.py run` CLI (see "OOB in Local Mode" above).
