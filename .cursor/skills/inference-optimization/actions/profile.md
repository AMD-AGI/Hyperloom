...
This step runs the full agentic analysis workflow through TraceLens's own `analysis-orchestrator` skill. Hyperloom does not call `TraceLens_generate_perf_report_pytorch*` or `orchestrator_prepare.py` itself — the orchestrator handles those internally based on `<analysis_mode>`.
...
**Resolve `analysis_mode` for this run.** Inference-optimization always serves vLLM or SGLang, so:

```bash
ANALYSIS_MODE=inference
INFERENCE_EXEC_MODE=eager   # only the trace file is needed for vLLM/SGLang
```
...
| `<platform>` | `$GPU_TYPE` (`MI300X` / `MI325X` / `MI350X` / `MI355X`) |
| `<analysis_mode>` | `inference` |
| `<inference_exec_mode>` | `eager` |
...
- Write the final report to `$TRACE_DIR/tracelens_output/analysis.md`.

After completion, the following artifacts are available under `$TRACE_DIR/tracelens_output/`:
...
Record tracing end after the orchestrator completes:
```bash
...
## Failure Handling
- TraceLens CLI not installed: install path tried in order — `/hyperloom/TraceLens-internal`, `/opt/TraceLens`, `git clone $TRACELENS_GIT_URL /tmp/TraceLens-internal` — then `pip install -e <dir>` (see Step 2 install block).
- TraceLens orchestrator run fails (perf report, prepare, or any subagent): Hyperloom Step 3 falls back to direct trace parsing on `filtered-TP-0.trace.json.gz` with the 5-tier classification — kernel-opt scoring is independent of the orchestrator run.
- Trace too large: use filtered trace; if still too large, use architecture-based estimation in Step 3.
- vLLM profiling empty: use `/start_profile` HTTP endpoints or skip to architecture-based estimation.
