...
| Model Type | torch.compile | OOB Expected | Primary Strategy |
|-----------|--------------|--------------|-----------------|
| Dense | Try first | High (Inductor kernels) | torch.compile + OOB kernel optimization (Strategy A) |
| MoE without MLA, no SWA | Try first | Medium | torch.compile + OOB if Inductor works |
| **MoE + SWA** | **Incompatible** | **Low** | **CUDA graph coverage + backend exploration + server params** |
| **MoE + MLA** | **Skip** | **Low (~0-2%)** | **Backend exploration → server param tuning + targeted OOB round** |
| **MoE + MLA + custom attention** | **Skip** | **Low** | **Backend exploration → kernel tuning → combined testing** |
| Any model after vendor kernel >50% | — | Low | **Backend exploration → server parameter tuning; use OOB only for source-backed code paths** |
...
- `oob_expected_impact`: high / medium / low / skip
...
- Dense: boost torch.compile and OOB kernel optimization scores, moderate backend scores
- MoE+MLA: zero torch.compile, high backend scores, low OOB kernel optimization scores
