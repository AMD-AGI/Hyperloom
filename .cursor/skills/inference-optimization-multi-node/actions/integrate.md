...
Per-kernel integration phase. Called by `kernel-opt.md` for each OOB result.

## Inputs
- OOB output: optimized kernel source file or source patch
...
def patch_standalone_kernels(kernel_name, optimized_source_path, target_signature_pattern):
    with open(optimized_source_path) as f:
        optimized_body = f.read()

    body_match = re.search(r'(def ' + kernel_name + r'\([^)]+\):)\n(.*)', optimized_body, re.DOTALL)
    if not body_match:
        return 0, 0
    optimized_function_body = body_match.group(2)
    ...
    for root, dirs, files in os.walk(CACHE_DIR):
        for f in files:
            ...
            adapted_body = re.sub(r'xnumel\s*=\s*\d+', f'xnumel = {xnumel}', optimized_function_body, count=1)
...
python3 $SCRIPTS_DIR/patch_inductor.py patch \
    ...
    --optimized-file <optimized_kernel.py> \
