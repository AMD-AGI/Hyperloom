...
### IR-1：通过 OOB 提交所有选中的内核候选

kernel-opt 动作必须将 `OOB_TOP_CANDIDATES`（默认 5 个）候选提交给 `KERNEL_OPT_BACKENDS` 中所有启用的 OOB agent（`codex`、`claude`），并在容量允许时**同时**提交。当存在多个候选时只提交 1 个，或在两个 OOB agent 都启用时串行提交，均属于违规。

### IR-2：OOB 提交前绝不修改内核源码

必须提交**按原样提取**的内核源码。不要移除装饰器、修改 stride、把 `@triton_heuristics` 替换成 `@triton.jit`，也不要做任何“清理”编辑。OOB agent 会在隔离工作区里处理适配。

### IR-3：集成（Phase 8）是强制步骤

OOB 返回优化后的内核后，必须执行 integrate 动作（patch → re-baseline → decide）。跳过该步骤意味着 OOB 结果从未经过端到端验证。re-baseline 使用 `run_baseline.sh`，不存在 `run_benchmark.sh`。详情见 `actions/integrate.md`。
...
**关键：** 当 OOB 优化后的内核修改 block size 或 warp count 时，也必须通过 `--best-config` 传入更新后的 tiling 参数。只 patch 内核 `.py` 而不更新 `.best_config` 会造成数值损坏（输出乱码）。详情见 `actions/integrate.md`。

### IR-7：OOB 是唯一内核优化后端

内核优化必须在 RayJob 内通过 `oob_ray_submit.py run` 使用 OOB。
见 [`modes/REMOTE.md`](modes/REMOTE.md)。

将 OOB 配置视为**只读基础设施**。skill 运行时不得修改任何 OOB 配置文件、设置或参数。具体包括：

- **不要**修改 OOB auth config、workspace settings 或 API configuration
- **不要**写入或改动 OOB config/settings 目录下的任何文件，bootstrap 除外
- **不要**在运行时修改 `KERNEL_OPT_WORKSPACE`、`OOB_ROUND_ITERATIONS` 或其他常量
  （使用上方常量表中的值，或使用用户覆盖值）

唯一允许的内核优化交互方式是 `oob_ray_submit.py run`，它会在 RayJob 执行环境中调用 `oob run -a codex` 或 `oob run -a claude`。

违规（修改后端或绕过 OOB）= 立即判定本次运行无效。

### IR-7b：编排器不得自己编写内核优化代码

所有内核优化都必须通过配置的 `KERNEL_OPT_BACKENDS`（`codex`、`claude`）执行。编排器 Agent 的职责是**准备 prompt、向后端提交任务、验证结果并集成**，绝不能直接编写优化后的 Triton/HIP/CUDA 内核。
...
**其他远程模式专属铁律定义在 [`modes/REMOTE.md`](modes/REMOTE.md)（IR-8 到 IR-11）。**
...
| 常量 | 值 | 说明 |
|----------|-------|-------------|
| `KERNEL_OPT_BACKENDS` | `codex,claude` | 逗号分隔的启用 OOB agent。允许值：`codex`、`claude`。用户可在 prompt 中覆盖。 |
| `OOB_ROUND_ITERATIONS` | 3 | 每轮 Codex/Claude 的迭代次数（提交 → RayJob 内 benchmark/验证 → 反馈 → 重新提交）。最佳结果获胜。 |
| `KERNEL_OPT_IMAGE` | *由 CI 或用户提供* | OOB 运行所在 RayJob 使用的框架镜像。每次运行一个镜像，由框架（SGLang/vLLM）决定。 |
| `KERNEL_OPT_WORKSPACE` | `control-plane-moe` | RayJob 和 OOB 输出所在的 SaFE workspace。用户可覆盖。 |
| `OOB_MAX_RETRIES` | 3 | 每个内核的最大 OOB 提交重试次数 |
| `OOB_TOP_CANDIDATES` | 5 | 要提交的顶部内核/代码候选数量 |
| `OOB_CONSECUTIVE_DISCARDS` | 5 | 连续丢弃达到该次数后停止 |
| `OOB_WALL_CLOCK_MIN` | 30 | kernel-opt 动作的最大墙钟时间（分钟） |
| `OOB_POLL_INTERVAL_S` | 60 | 需要外部轮询时，OOB 任务状态轮询间隔（秒） |
| `OOB_POLL_TIMEOUT_MIN` | 15 | 需要外部轮询时，单个 OOB 任务的最大轮询时间（分钟） |
| `MIN_GPU_PCT` | 3 | 将内核/代码路径视为 OOB 候选的最小 GPU 时间占比 |
| `SERVER_KILL_WAIT_S` | 10 | kill 服务与重启之间的等待秒数 |
| `FILTERED_TRACE_NAME` | `filtered-TP-0.trace.json.gz` | TraceLens 分析首选 trace 文件 |

**所有 OOB 工作都在 RayJob 镜像内执行。** 对于源码存在于镜像中的内核（例如 `/sgl-workspace/aiter/`），在 prompt 中包含 pod 可见路径。对于运行时生成的内核（例如 `torch.compile` 生成的 `/tmp/torchinductor_root/`），不要在 prompt 中包含 `kernel_url`/`kernel_repo`；应将文件复制到共享 NFS，或只依赖 `files[].content`。
...
5. **绝不绕过 OOB。** 见 IR-7。内核优化只能在 RayJob 内通过
   `oob_ray_submit.py run` 调用。

6. **记录所有外部调用的开始/结束时间戳。** 调用任何外部组件（OOB、
   TraceLens 或未来批准的后端）前，运行
...
**DFS 循环动作：** backends、params、kernel-opt、integrate、sweep。它们由启发式打分，并按最高分优先弹出。每个动作可以推入子动作（例如 PROFILE 推入 OOB candidates，BACKENDS 推入组合测试）。agent 沿最有希望的分支深度优先探索，并在分数变化时回溯。

agent 不限于预定义动作。如果 profiling 暴露了意外瓶颈，或 KB 提示了新技术，也可以创建临时动作，并用同一启发式进行打分。

这是单 agent 的顺序循环。每个动作运行完成后才会重新打分。并行发生在动作内部（例如 OOB 将候选同时提交给 Codex/Claude），而不是动作之间。

## 自治规则

**自治执行，不需要人工确认。** 执行以下操作前不要询问用户：
- 在 SaFE 上创建/停止 RayJob（远程模式）
- 在 RayJob 内运行 baseline/profiling 脚本
- 提交 OOB 任务
- 在 RayJob 内部杀掉/重启服务
...
4. **所有 backend 测试完成后：** 推入 `re-profile`（用于发现新的 OOB 目标）
...
| 0.05-0.15 | OOB 生成的内核/代码修改、GEMM 调优 | **是** |
...
### 内核级预检查（可选，仅用于 OOB kernel/code mods）
...
3. **torch.compile 会暴露更多可供 OOB 优化的 Triton 表面。** 启用 compile 时，
   可以提取运行时生成的 Inductor 内核并通过 OOB 提交；未启用时，OOB 应重点处理
   framework source、调度和集成修改。
...
6. **除非有源码，否则不要把 vendor kernel 当作可直接编辑对象。**
   对 `Cijk_*` 或 `aiter::*`，优先使用 framework scheduling/config 修改，或带源码的 OOB 任务。
