#!/usr/bin/env python3
"""Patch Inductor standalone kernel files with an optimized function body.
...
Usage:
    python patch_inductor.py patch \\
        ...
        --optimized-file /path/to/optimized_kernel.py \\
...
def patch_single_file(
    ...
    """
    new_body = extract_function_body(optimized_source, kernel_name)
    if not new_body:
        print(f"ERROR: Could not find function '{kernel_name}' in optimized output")
        return False
    ...
    optimized_sig = extract_function_signature(optimized_source, kernel_name)
    target_sig = extract_function_signature(content, kernel_name)
    if optimized_sig and target_sig and optimized_sig != target_sig:
        print(f"ERROR: Signature mismatch — optimized output has {len(optimized_sig)} params "
              f"({', '.join(optimized_sig[:4])}...) but target has {len(target_sig)} "
              f"({', '.join(target_sig[:4])}...). "
...
def main():
    parser = argparse.ArgumentParser(description="Patch Inductor kernels with optimized output")
    sub = parser.add_subparsers(dest="command")

    p_patch = sub.add_parser("patch", help="Apply optimized kernel patch")
    p_patch.add_argument("--kernel-name", required=True)
    p_patch.add_argument("--optimized-file", required=True, help="Optimized kernel file")
    ...
    if args.command == "patch":
        ...
        optimized_source = open(args.optimized_file).read()
        bc = _json.loads(args.best_config) if args.best_config else None
        ok = patch_single_file(args.kernel_name, optimized_source, args.target_file, bc, args.dry_run)
