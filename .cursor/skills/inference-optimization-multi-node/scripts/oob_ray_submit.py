#!/usr/bin/env python3
"""OOB Ray Submit — schedule oob CLI tasks inside the RayJob.

Run this script from inside the RayJob image, typically by submitting a Ray
Dashboard REST job (`POST /api/jobs/`). Do not run it from the Claw sandbox or
any external client.

It uses `ray.init(address="auto")` by default to connect to the in-cluster Ray
runtime inside the RayJob. This is not the sandbox Ray Client path.

Provides GPU isolation for concurrent OOB (Claude Code / Codex) tasks. Each
task gets an isolated GPU allocated by Ray.
...
_FORWARDED_ENV_KEYS = (
    "OOB_API_KEY", "OOB_BASE_URL", "OOB_CLI", "OOB_HOME",
...
def main():
    parser = argparse.ArgumentParser(
        description="OOB Ray Submit — schedule oob CLI tasks inside the RayJob",
