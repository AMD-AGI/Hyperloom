...
3. Every later REST job that needs the Hyperloom toolchain must
   ...
   gateway `https://core42.example-internal-host.invalid/api/v1/llm-proxy`, not the
...
1. When creating the RayJob via `workload_create`, propagate the current
   ...
   - `OOB_BASE_URL` = `https://core42.example-internal-host.invalid/api/v1/llm-proxy/v1`
   - `ANTHROPIC_BASE_URL` = `https://core42.example-internal-host.invalid/api/v1/llm-proxy`
...
2. After RayJob is `Running` and bootstrap has run, verify inside the RayJob:
   ...
   test "$ANTHROPIC_BASE_URL" = "https://core42.example-internal-host.invalid/api/v1/llm-proxy"
...
3. If the RayJob was already created without credentials and the env file is
   ...
   POST http://<head_ip>:8265/api/jobs/
   {
     "entrypoint": "bash -lc 'export SAFE_API_KEY=\"$SAFE_KEY\" OOB_API_KEY=\"$SAFE_KEY\" AMD_LLM_API_KEY=\"$SAFE_KEY\" LLM_API_KEY=\"$SAFE_KEY\" ANTHROPIC_API_KEY=\"$SAFE_KEY\" OPENAI_API_KEY=\"$SAFE_KEY\" OOB_BASE_URL=\"https://core42.example-internal-host.invalid/api/v1/llm-proxy/v1\" ANTHROPIC_BASE_URL=\"https://core42.example-internal-host.invalid/api/v1/llm-proxy\" ANTHROPIC_CUSTOM_HEADERS=\"Authorization: Bearer $SAFE_KEY\"; bash /wekafs/yunkai/Hyperloom/.cursor/skills/inference-optimization-multi-node/scripts/bootstrap.sh --force'"
   }
   ```
