...
def _build_benchmark_cases_block(candidate: dict[str, Any]) -> str:
    ...
    ``candidate["task_group"]`` is absent so the captured serving shapes still
    reach GEAK (previously this returned "" and GEAK got no shapes). When a
    task_group is present, emits one bullet per TraceLens Operation
    ...
    if not (isinstance(rows, list) and rows):
        # No task_group: still surface the captured serving shapes so GEAK's
        # harness is bound to the real workload (generic fallback).
