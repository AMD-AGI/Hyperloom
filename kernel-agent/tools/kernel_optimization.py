...
def _build_benchmark_cases_block(candidate: dict[str, Any]) -> str:
    ...
    Empty string when no ``task_group`` (legacy per-kernel dispatch). Otherwise one
    bullet per TraceLens row, sorted by aggregate time desc, surfacing operation/args/
    aggregate_time_ms/percent_e2e/count/per_call_ms/flops_per_byte/efficiency/bound.
    """
...
def invoke_backend(
    ...
    try:
        if backend == "geak":
            ...
            # Prefer external --test-command; else derive from benchmark files via the selector/timeout wrapper.
            ...
            if not test_command:
                # Always provide a real test command (GEAK preprocess may invent a bogus path otherwise).
            ...
            # Auto-generate a GEAK-compatible harness when needed (e.g. raw AITER op_test).
            if test_command:
