...
def _try_generate_aiter_harness(
    ...
    for p in test_fn.params:
        ...
        if pl in ("dtype", "input_dtype"):
            ...
            si += 1
        elif pl in int_param_order:
            missing_shape_params.append(p)
    if missing_shape_params:
        log(
            "aiter idiom: candidate shape missing values for fn params "
            f"{missing_shape_params}"
        )
        return None
    if not kwargs:
    ...
    if isinstance(shapes, list):
        ...
        for entry in shapes:
            if not isinstance(entry, dict):
                continue
            dims, _dtype = _parse_shape_string(str(entry.get("shape") or ""))
            if not dims:
                continue
            try:
                call_num = int(entry.get("call_num") or 0)
            except (TypeError, ValueError):
                call_num = 0
            parsed.append((dims, call_num))
        if not parsed:
            return out
        max_ndim = max(len(dims) for dims, _call_num in parsed)
        dims, _call_num = max(
            (item for item in parsed if len(item[0]) == max_ndim),
            key=lambda item: item[1],
        )
        for k, v in zip(("M", "N", "K"), dims[:3]):
            out[k] = v
    return out
