...
class TraceLensSkillRunResult:
    ...
    ``priority_data_path`` is ``None`` when the v0.3 SDK orchestrator
    didn't emit ``priority_data.json``. Per
    ``TraceLens_Report_Interfacing.docx`` §2, ``analysis.md`` is the
    single source of truth — intermediate JSON sidecars are best-effort
    artefacts, not contracted inputs.
    """

    output_dir: Path
    report_path: Path
    priority_data_path: Path | None
...
5. At minimum, ensure these files exist before you finish:
   ...
   The following intermediate sidecars are optional — emit them if your
   orchestrator template already writes them, but do not fail the run if
   they're absent (Hyperloom treats analysis.md as the single source of
   truth per Report_Interfacing.docx §2):
   - {output_dir / "priority_data.json"}                       (optional)
   - {output_dir / "category_data" / "category_manifest.json"} (optional)
...
async def run_tracelens_skill(
    *,
    ...
    # the single source of truth: "no intermediates generated by the
    # agent". ``priority_data.json`` and ``category_data/category_manifest.json``
    # are v0.2-era transitional sidecars that v0.3 still emits for
    # back-compat — we surface their paths when present but never
    # require them. Requiring them here would couple Hyperloom to an
    # intermediate-files contract TraceLens explicitly disowns, so a
    # future TraceLens release that drops the sidecars would
    # gratuitously break Hyperloom.
    ...
    priority_data_optional: Path | None = (
        priority_data_path if priority_data_path.exists() else None
    )
    category_manifest_path = output_dir / "category_data" / "category_manifest.json"
    category_manifest_optional: Path | None = (
        category_manifest_path if category_manifest_path.exists() else None
    )
    ...
    if priority_data_optional is not None:
        artifact_paths["tracelens_priority_data"] = str(priority_data_optional)
    if category_manifest_optional is not None:
        artifact_paths["tracelens_category_manifest"] = str(category_manifest_optional)
    if sdk_error:
    ...
    return TraceLensSkillRunResult(
        ...
        priority_data_path=priority_data_optional,
        raw_text="\n".join(chunks),
