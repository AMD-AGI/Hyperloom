...
def test_78_build_orchestrator_invocation_plain():
    """Without debug, returns the plain prompt invocation."""
    argv = tla.build_orchestrator_invocation(
        prompt="Read .../standalone-analysis-orchestrator.md and follow ALL steps",
        debug=False,
    )
...
def test_78_build_orchestrator_invocation_with_stream_json():
    """With debug + stream_path, argv includes --output-format stream-json."""
    argv = tla.build_orchestrator_invocation(
    ...
    idx2 = argv.index("--output-file")
    assert argv[idx2 + 1] == "/tmp/stream.jsonl"


def test_78_build_orchestrator_invocation_debug_without_stream_path():
    """Debug enabled but no stream_path → still adds stream-json format
    (caller may pipe to stdout)."""
    argv = tla.build_orchestrator_invocation(
        prompt="Read the skill...",
        debug=True,
        stream_path=None,
...
def test_78_debug_flag_via_env(tmp_path, monkeypatch):
    """DEBUG_TRACELENS=true env var enables debug mode at argparse time."""
    import gzip
    import json as _json
    import subprocess
    import sys

    trace = tmp_path / "filtered-TP-0.trace.json.gz"
    with gzip.open(trace, "wt") as f:
        _json.dump({"traceEvents": [{"name": "k", "cat": "kernel", "dur": 1, "ts": 0}]}, f)

    env = dict(__import__("os").environ)
    env["DEBUG_TRACELENS"] = "true"

    proc = subprocess.run(
        [
            sys.executable,
            str(Path(tla.__file__)),
    ...
    runs = list((tmp_path / "kernel-agent" / "runs").iterdir())
    log = next(runs[0].rglob("*.log")).read_text()
    assert "DEBUG_TRACELENS=true" in log, f"debug log marker missing:\n{log}"
    assert "tracelens_agent_stream.jsonl" in log

    status = list((runs[0] / "status").rglob("*.json"))[0]
    payload = _json.loads(status.read_text())
    assert "tracelens_agent_stream" in payload.get("artifact_paths", {}), (
        f"stream artifact path missing: {payload}"
    )
