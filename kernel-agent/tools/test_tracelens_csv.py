...
# ===========================================================================
# Raw-trace hot-kernel fallback (HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK).
# When the steady-state splitter yields no usable chunk, the env-gated
# fallback deterministically surfaces hot EDITABLE reusable-native (.py)
# kernels straight from the raw trace so the Coordinator still gets a
# kernel_id — while dropping collective/comm kernels that over-represent
# on a raw (un-split) trace. Flag OFF must preserve the strict
# trace_split_no_steady_state refuse (analysis.md is the single source of
# truth). Mirrors the validated reference fallback.
# ===========================================================================
def test_derive_shapes_from_workload_moe_model():
    shapes = tla.derive_shapes_from_workload(
        workload={"isl": 4096, "osl": 64, "conc": 64, "tp": 4},
        model_name="/home/sapmajum/models/gpt-oss-120b-BF16",
        candidate={"name": "fused_moe_kernel", "source_file": "fused_moe_triton_config.py"},
    )
    assert shapes
    assert any("E=128" in s or "experts" in s for s in shapes)
    assert any("4096" in s for s in shapes)


def test_enrich_candidates_with_workload_shapes():
    candidates = [{"kernel_id": "k001", "name": "fused_moe", "shapes": []}]
    tla.enrich_candidates_with_workload_shapes(
        candidates,
        workload={"isl": 4096, "conc": 64, "tp": 4, "osl": 64},
        model_name="/home/sapmajum/models/gpt-oss-120b-BF16",
    )
    assert candidates[0]["shapes"]
    assert candidates[0]["shape_provenance"] == "workload_config"


def test_raw_trace_fallback_enabled_env_gating(monkeypatch):
    for truthy in ("1", "true", "TRUE", "Yes", "on", "  on  "):
        monkeypatch.setenv("HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK", truthy)
        assert tla._raw_trace_fallback_enabled() is True, truthy
    for falsy in ("0", "false", "off", "no", "", "   ", "maybe"):
        monkeypatch.setenv("HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK", falsy)
        assert tla._raw_trace_fallback_enabled() is False, falsy
    monkeypatch.delenv("HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK", raising=False)
    assert tla._raw_trace_fallback_enabled() is False


def test_is_collective_comm_kernel_drops_comm_keeps_compute():
    # The validated +23%-run editable compute targets must NOT be flagged
    # as comm collectives.
    for name in (
        "fused_moe_kernel",
        ...
        assert tla._is_collective_comm_kernel(
            {"name": name, "source_file": "/x/triton/foo.py"}
        ) is False, name
    # Collective/comm kernels (matched by name OR source_file) must drop.
    for cand in (
        {"name": "cross_device_reduce", "source_file": ""},
        {"name": "_C::all_reduce", "source_file": ""},
        {"name": "allgather_kernel", "source_file": ""},
        {"name": "reduce_scatter_impl", "source_file": ""},
        {"name": "quickreduce", "source_file": ""},
        {"name": "custom_ar_dispatch", "source_file": ""},
        {"name": "opaque", "source_file": "/x/nccl/foo.py"},
        {"name": "opaque", "source_file": "/x/rccl/bar.cu"},
    ):
        assert tla._is_collective_comm_kernel(cand) is True, cand


# Controlled raw-trace extraction result: one editable .py reusable-native
# compute kernel (admit), one collective comm kernel (drop), one
# non-reusable-native kernel (drop), one C++/.cu reusable kernel (drop —
# the fallback is restricted to source-only .py to avoid sgl-kernel rebuild
# contamination).
_FALLBACK_TRACE_CANDS = [
    {"name": "fused_moe_kernel", "source_file": "/x/sgl/triton/fused_moe.py",
     "reusable_native_kernel": True, "kernel_id": "k001",
     "duration_us": 100.0, "gpu_pct": 50.0},
    {"name": "cross_device_reduce", "source_file": "/x/sgl/comm.py",
     "reusable_native_kernel": True, "kernel_id": "k002",
     "duration_us": 80.0, "gpu_pct": 40.0},
    {"name": "ck_gemm", "source_file": "/x/vendor/ck.py",
     "reusable_native_kernel": False, "kernel_id": "k003",
     "duration_us": 70.0, "gpu_pct": 30.0},
    {"name": "sgl_kernel_moe", "source_file": "/x/csrc/moe.cu",
     "reusable_native_kernel": True, "kernel_id": "k004",
     "duration_us": 60.0, "gpu_pct": 25.0},
]


def _drive_fallback_main(tmp_path, monkeypatch, capsys, *, flag_value, analyze_return):
    """Drive ``tla.main()`` through the no-steady-state splitter path.

    The mocked splitter "succeeds" (rc=0) but writes no ``*_steady_state_*``
    chunk files, so the trace-split raise path fires. The raw-trace fallback
    gate is set to ``flag_value`` (unset when ``None``) and
    ``analyze_trace_files`` is stubbed to ``analyze_return`` for determinism.
    ``write_reports`` / ``load_roofline_results`` are stubbed so the test
    isolates the fallback gate / filter / wiring (those collaborators have
    their own tests). Returns ``(return_code, parsed_result_json)``.
    """
    import gzip
    import json as _json
    from unittest.mock import patch
    ...
    def fake_write_reports(run_dir, **_kw):
        return {"trace_report_path": ""}

    if flag_value is None:
        monkeypatch.delenv("HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK", raising=False)
    else:
        monkeypatch.setenv("HYPERLOOM_KERNELOPT_RAW_TRACE_FALLBACK", flag_value)
    monkeypatch.setattr(tla, "analyze_trace_files", lambda *a, **k: list(analyze_return))
    monkeypatch.setattr(tla, "write_reports", fake_write_reports)
    monkeypatch.setattr(tla, "load_roofline_results", lambda *a, **k: {})
...
def test_raw_trace_fallback_flag_on_admits_editable_drops_comm(
    tmp_path, monkeypatch, capsys,
):
    rc, result = _drive_fallback_main(
        tmp_path, monkeypatch, capsys,
        flag_value="1", analyze_return=_FALLBACK_TRACE_CANDS,
    )
    assert rc == 0, result
    assert result.get("orchestrator_mode") == "raw_trace_fallback", result
    assert result.get("report_source") == "raw_trace_fallback", result
    names = [k["name"] for k in result["hot_kernels"]]
    # Only the editable .py reusable-native COMPUTE kernel survives; the
    # collective comm, the non-reusable, and the C++/.cu candidates drop.
    assert names == ["fused_moe_kernel"], names
    # The trace_split_no_steady_state warning is still surfaced upward.
    assert any(
        isinstance(w, dict) and w.get("code") == "trace_split_no_steady_state"
        for w in result.get("trace_health_warnings", [])
    ), result


def test_raw_trace_fallback_flag_off_still_refuses(
    tmp_path, monkeypatch, capsys,
):
    rc, result = _drive_fallback_main(
        tmp_path, monkeypatch, capsys,
        flag_value=None, analyze_return=_FALLBACK_TRACE_CANDS,
    )
    assert rc == 1, result
    assert result.get("status") == "failed", result
    assert "trace_split_no_steady_state" in result.get("error", ""), result
