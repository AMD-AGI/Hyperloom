...
- **Default ladder (all rewritable kernels)**: `forge,geak,claude,codex,cursor`
  (Forge-GEAK-OOB) — Forge first (Kernel-Forge autonomous loop), falling through
  to GEAK when Forge skips a non-Triton candidate or misses a KEEP, then the OOB
  backends (`claude`, `codex`, `cursor`). An explicit `--backends` or the
  `KERNEL_OPT_BACKEND_ORDER` / `KERNEL_OPT_BACKENDS` env still overrides this
  default as-is. `cursor` is key-gated (dropped from the auto-derived ladder when
  `$CURSOR_API_KEY` is unset to avoid wasted 401 attempts). The kernel type (Triton / HIP-C++ / FlyDSL /
