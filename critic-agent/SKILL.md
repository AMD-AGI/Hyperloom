...
| `evidence_producer` | `explore`, `specialist`, `sweep`, `profile`, `pmc_roofline`, `kernel_opt`, `deep_kernel_analysis`, `operator_tuning`, `vendor_kernel_config`, `assess_remaining_gaps` | Structural — provenance non-empty (specialist or default_grid), action in current phase's allowed set, no contradicting KB prior. **Default approve when KB priors are silent.** |
