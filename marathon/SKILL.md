...
2. **Launch in BACKGROUND** — never foreground:
   ...
   bash $SKILL_ROOT/launcher/run.sh > /tmp/marathon.log 2>&1 &
   echo "marathon_pid=$!"
   ```
...
The directory containing this `SKILL.md` — i.e. `TBO/inference_optimization/marathon/`.
The protocol skills (`SPEC_ROOT`) live in the `skills/` subdirectory.
The tmux launcher lives in the `launcher/` subdirectory.
...
SandboxImage: harbor.oci-slc.primus-safe.amd.com/custom/lmsysorg/sglang:202603270958
...
section, constructs the `bash $SKILL_ROOT/launcher/run.sh > /tmp/marathon.log 2>&1 &`
...
Everything lives under `marathon/`:

```
marathon/
├── SKILL.md           ← you are here (the @-trigger skill)
├── launch.sh          ← convenience wrapper (exports env + calls launcher/run.sh)
├── README.md
├── launcher/          ← tmux launcher
│   ├── run.sh         ← creates tmux session with 3 panes
│   ├── pane_orchestrator.md
│   ├── pane_kernel_mgr.md
│   └── pane_watchdog.md
└── skills/            ← SPEC_ROOT (protocol source of truth)
    ├── SKILL.md       ← orchestrator DFS protocol
    ├── kernel-manager/
    ├── watchdog/
    ├── actions/
    ├── kernel-opt/
...
`run.sh` auto-discovers `SPEC_ROOT` as the sibling `skills/` directory.
Override `SPEC_ROOT` env to point elsewhere if needed.

The three pane prompts (`launcher/pane_*.md`) tell their `claude` CLI to
`Load and follow $SPEC_ROOT/SKILL.md` (and `$SPEC_ROOT/kernel-manager/SKILL.md`
/ `$SPEC_ROOT/watchdog/SKILL.md` respectively), and explicitly IGNORE
`$SPEC_ROOT/modes/CLAW.md` since the pane IS on the GPU host natively.
