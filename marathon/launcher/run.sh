...
#   bash $SKILL_ROOT/launcher/run.sh
#
...
# Protocol skills live in the sibling "skills/" directory.
: "${SPEC_ROOT:=$(cd "$SCRIPT_DIR/../skills" 2>/dev/null && pwd)}"
if [[ ! -d "$SPEC_ROOT" ]]; then
    for cand in \
        "/shared_nfs/nehaprakriya/TBO/inference_optimization/marathon/skills" \
...
if [[ "$MODE" == "sandbox" ]]; then
    ...
    SESSION_ROOT="${WORKSPACE_ROOT:-/shared_nfs/nehaprakriya/marathon-sessions}"
...
for f in "$SPEC_ROOT/SKILL.md" "$SPEC_ROOT/kernel-manager/SKILL.md" "$SPEC_ROOT/watchdog/SKILL.md"; do
    [[ -f "$f" ]] || { echo "ERROR: spec file missing: $f (set SPEC_ROOT env to point at the skills/ directory)" >&2; exit 4; }
done
