...
**Applies to:** Strategy B (framework Triton), Strategy D (dispatch patching),
Strategy E (scheduling optimization), Strategy F (fusion kernel insertion).

### Strategy D/E Integration: Framework Source Patches

For call-stack optimizations that modify framework or aiter Python source:

```bash
# 1. Create rollback snapshot BEFORE any edits
PATCH_BACKUP="$RESULT_DIR/patch_backups/$(date +%s)"
mkdir -p "$PATCH_BACKUP"

# For aiter dispatch changes
AITER_PATH=$(python3 -c "import aiter; import os; print(os.path.dirname(aiter.__file__))")
cp -r "$AITER_PATH" "$PATCH_BACKUP/aiter_backup"

# For SGLang scheduling changes
SGLANG_PATH=$(python3 -c "import sglang; import os; print(os.path.dirname(sglang.__file__))")
cp -r "$SGLANG_PATH/srt/managers" "$PATCH_BACKUP/sglang_managers_backup"

# 2. Apply the patch (agent edits files directly or uses StrReplace)

# 3. Reload without full restart (for Python-only changes)
# Option A: pip install -e . (if in editable install)
cd /sgl-workspace/aiter && pip install -e . --no-deps 2>/dev/null

# Option B: Full server restart (always works, required for scheduling changes)
kill_server && sleep $SERVER_KILL_WAIT_S

# 4. Rollback on regression
cp -r "$PATCH_BACKUP/aiter_backup/"* "$AITER_PATH/"
# or
cd /sgl-workspace/aiter && git checkout -- . && pip install -e . --no-deps
```

**Rollback checklist for framework patches:**
- [ ] Backup created before edit
- [ ] `__pycache__` cleared after revert: `find "$AITER_PATH" -name "__pycache__" -exec rm -rf {} +`
- [ ] `.pyc` files cleared: `find "$AITER_PATH" -name "*.pyc" -delete`
- [ ] Server fully restarted (module reload is not enough for some changes)
- [ ] E2E benchmark confirms revert to pre-patch performance
