...
| Model Type | Full torch.compile | Selective compile | GEAK Expected | Primary Strategy |
|-----------|:--:|:--:|:--:|---|
| Dense | Try first | N/A (full works) | High | torch.compile + GEAK (Strategy A) |
| MoE without MLA, no SWA | Try first | Fallback | High→Medium | torch.compile + GEAK; if fails → selective + GEAK |
| **MoE + SWA** | **Incompatible** | **Try (norm/act)** | **Medium** | **CUDA graph → backends → selective compile + GEAK** |
| **MoE + MLA** | **Skip** | **Try (norm/act/gate)** | **Medium** | **Backends → selective compile + GEAK + server params** |
| **MoE + MLA + custom attention** | **Skip** | **Try (norm/act)** | **Low–Medium** | **Backends → selective compile + GEAK → combined** |
| Any model after vendor kernel >50% | — | — | Skip GEAK | **Backend exploration → server parameter tuning** |

### Step 2b: Compile probing (when full torch.compile fails or is skipped)

When Step 2 marks full `torch.compile` as incompatible/skip, probe which submodule
types CAN compile. This feeds **Strategy C** in `kernel-opt.md`.
...
# Probe individual ops in isolation
probe_results = {}
test_input = torch.randn(1, hidden_size, device="cuda", dtype=torch.bfloat16)

# RMSNorm
try:
    from sglang.srt.layers.layernorm import RMSNorm
    norm = RMSNorm(hidden_size).cuda().bfloat16()
    compiled_norm = torch.compile(norm, fullgraph=False)
    _ = compiled_norm(test_input)
    probe_results["RMSNorm"] = "COMPILABLE"
except Exception as e:
    probe_results["RMSNorm"] = f"FAIL: {str(e)[:80]}"

# SiLU activation
try:
    silu = torch.nn.SiLU()
    compiled_silu = torch.compile(silu, fullgraph=False)
    _ = compiled_silu(test_input)
    probe_results["SiLU"] = "COMPILABLE"
except Exception as e:
    probe_results["SiLU"] = f"FAIL: {str(e)[:80]}"

# Linear projection (non-MoE)
try:
    linear = torch.nn.Linear(hidden_size, hidden_size, bias=False).cuda().bfloat16()
    compiled_linear = torch.compile(linear, fullgraph=False)
    _ = compiled_linear(test_input)
    probe_results["Linear"] = "COMPILABLE"
except Exception as e:
    probe_results["Linear"] = f"FAIL: {str(e)[:80]}"

for name, status in probe_results.items():
    print(f"  {name}: {status}")
```

**Output:** `compile_probe_results` — dict of module type → COMPILABLE/FAIL.
If ANY type is COMPILABLE, set `selective_compile_viable = true` and boost kernel-opt
score by +2 (from base 2 → 4 for MoE+MLA).
...
- `selective_compile_viable`: true / false (from Step 2b probe)
- `compile_probe_results`: dict of module_type → COMPILABLE/FAIL
- `kernel_opt_expected_impact`: high / medium / low / skip
...
- MoE+MLA + selective_compile_viable: zero full-compile, moderate GEAK (4–5), high backend scores
- MoE+MLA + NOT selective_compile_viable: zero full-compile, low GEAK (2), high backend scores
- MoE+SWA: zero torch.compile, high CUDA graph + backend scores, moderate GEAK if selective_compile_viable
