...
**DFS role:** Typically the highest-scored action for MoE+MLA models (prior=9). Backend
switches change actual GPU kernels and can produce super-linear gains when combined.
After this action, ALWAYS push `combined_backends_test` and `re-profile` sub-actions.
