...
if [ "$FRAMEWORK" = "vllm" ]; then
    ...
    # cpu_group now uses RCCL (not gloo), so tune Ring algorithm + 4 channels
    # to match TP size and eliminate hipEventSynchronize stalls on barriers.
...
if [ -n "$TRACE_FOR_ANALYSIS" ]; then
    echo "Filtering trace (removing python_function events for kernel analysis)..."
...
if [ "$TRACE_COUNT" -gt 0 ]; then
    ...
    echo "For kernel analysis: $TRACE_FOR_ANALYSIS"
else
