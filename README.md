...
A GPU node is required to run benchmarks. You can either use a local GPU machine or request an Authoring Pod on **[Primus-SaFE](https://core42.example-internal-host.invalid/authoring)**. For example, an inference optimization workload typically runs on an image like:
