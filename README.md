...
Bind your **[LLM Gateway](https://llm.amd.com/)** key to **[Hyperloom](https://oci-slc.example-internal-host.invalid/hyperloom/)** to obtain your `AK_YOUR_API_KEY`. This key is required for both the Hyperloom UI and the local optimization workflow — it provides access to TraceLens, GEAK, and OOB services.
...
A GPU node is required to run benchmarks. You can either use a local GPU machine or request an Authoring Pod on **[Primus-SaFE](https://oci-slc.example-internal-host.invalid/authoring)**. For example, an inference optimization workload typically runs on an image like:
...
Full setup guide: **[deploy/local/README.md](deploy/local/README.md)** | Design doc: **[deploy/local/DESIGN.md](deploy/local/DESIGN.md)**
