...
# ---------------------------------------------------------------------------
# gfx1250 fused residual-add + RMSNorm (Triton).
#
# On this image the `_C` fused_add_rms_norm op fails to load (ABI mismatch:
# undefined symbol getCurrentHIPStream), so RMSNorm falls back to the eager
# forward_static path. Profiling gpt-oss-120b showed the split, fp32 residual
# add (`x = x.to(f32) + residual`) alone is ~25% of GPU time, plus the
# separate up/down-casts and reduce/rsqrt kernels. This fuses the whole
# residual-add + RMSNorm into one Triton kernel (bf16 I/O, fp32 accumulate),
# matching forward_static semantics. Guarded: any import/shape/dtype we don't
# handle falls back to the original eager path, so correctness is never at risk.
# ---------------------------------------------------------------------------
try:
    import triton
    ...
    def _fused_add_rmsnorm_kernel(
        x_ptr, res_ptr, w_ptr, out_ptr, resout_ptr,
        ...
        s = x + r                       # residual add in fp32 (matches native)
        tl.store(resout_ptr + off, s, mask=mask)   # new residual (rounds to out dtype)
        var = tl.sum(s * s, axis=0) / n_cols
        inv = tl.rsqrt(var + eps)
        norm = s * inv
        w = tl.load(w_ptr + cols, mask=mask, other=0.0).to(tl.float32)
        out = norm * w
        tl.store(out_ptr + off, out, mask=mask)     # rounds to out dtype on store

    _HAVE_FUSED_RMSNORM = True
except Exception:  # pragma: no cover - triton missing / import failure
    _HAVE_FUSED_RMSNORM = False


def _fused_add_rmsnorm(x, weight, residual, eps, hidden_size):
    """One-kernel residual-add + RMSNorm. Returns (out, new_residual), same
    dtype/shape as ``x``. Caller guarantees the fast-path preconditions."""
    orig_shape = x.shape
    # The kernel indexes with a hardcoded row stride of ``hidden_size``, so the
    # 2D views MUST be contiguous. reshape() alone can return a stride-preserving
    # view of a sliced tensor (row stride != hidden_size), and empty_like()
    # would inherit that stride — both corrupt the output. Force contiguity.
    x2d = x.reshape(-1, hidden_size).contiguous()
    r2d = residual.reshape(-1, hidden_size).contiguous()
...
class RMSNorm(CustomOp):
    ...
    def forward_static(
        ...
        # gfx1250 fast path: fuse residual-add + RMSNorm into one Triton kernel
        # for the common decode/prefill case. Strictly guarded — anything we
        # don't handle falls through to the eager code below unchanged.
        if (
            _HAVE_FUSED_RMSNORM
            and residual is not None
            and weight is not None
            and variance_size_override is None
            and x.is_cuda
            ...
            try:
                return _fused_add_rmsnorm(
                    x, weight, residual, variance_epsilon, hidden_size
                )
            except Exception:
                pass  # fall back to the eager path on any kernel error
