...
- `KERNEL_OPT_BACKEND_ORDER=forge` — GEAK does not currently drive kernel
  rewrites on ATOM, so the kernel phase runs the per-kernel KernelForge backend.
...
`optimize`. The opt-in is an **exact** match on `forge`; every other value
(including unset) leaves GEAK owning the whole kernel phase.
...
kernel phase on ATOM spends its budget without producing kernel candidates.
Forge works per kernel and does not depend on that seam.
...
Write both values into `.env` as well when the user wants them to persist across
runs, so a `--resume-from` relaunch keeps the same framework and backend:
...
Before launch, confirm both values are actually set in the launching shell and
report them. Launching without `KERNEL_OPT_BACKEND_ORDER` silently runs GEAK and
produces a run that looks like this demo but is not.
...
- `FRAMEWORK=atom` and `KERNEL_OPT_BACKEND_ORDER=forge` must be set **inside the
  container**, in the same `docker exec` that launches `optimize`. Exporting
  them only on the host does not reach the optimizer.
...
export FRAMEWORK=atom
export KERNEL_OPT_BACKEND_ORDER=forge
echo "framework: ${FRAMEWORK}  kernel backend: ${KERNEL_OPT_BACKEND_ORDER}"
```
...
2. Export `FRAMEWORK=atom` and `KERNEL_OPT_BACKEND_ORDER=forge` in the launching
   shell, after sourcing `kernel-agent.env.sh`, and confirm both values before
   launch. In docker mode, set them inside the same `docker exec` that runs
   `optimize`.
...
9. To recover an unexpected crash, only run `optimize --resume-from "$SESSION_DIR"` against the same session dir, with `FRAMEWORK=atom` and `KERNEL_OPT_BACKEND_ORDER=forge` still set. After the first launch, never start a new `optimize`; that creates a new `<UTC_ts>` session and is forbidden.
