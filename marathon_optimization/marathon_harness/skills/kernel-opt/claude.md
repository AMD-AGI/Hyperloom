...
**Verification uses the same trace-derived test harness** as Codex — see `codex.md`
"Verification Steps" and `kernel-opt.md` Step 1b. Test inputs are extracted from
standalone files or traces, never fabricated. Multi-shape testing catches
occupancy-dependent regressions.
...
Claude Code (via OOB Agent) is the most capable backend for deep call-stack
optimization. Multi-turn reasoning + tool use enables complex multi-file changes.

| Tier | Target | Claude capability | Example |
|------|--------|:-----------------:|---------|
| **T1: Triton/Inductor** | Triton kernel source | **Full rewrite** | Structural optimization with multi-step reasoning |
| **T2: aiter/CK dispatch** | Python dispatch + config | **Deep analysis + rewrite** | Analyze dispatch paths, rewrite for model-specific shapes, tune configs |
| **T3: Framework scheduling** | SGLang/vLLM scheduler | **Multi-file patch** | Modify batch scheduler, token management, prefill/decode overlap (multiple files) |
| **T4: Communication** | NCCL/RCCL + custom AR | **Tune + patch** | Analyze communication patterns, rewrite custom allreduce, tune NCCL params |
| **T5: Compiled binaries** | Vendor BLAS/CK | Config only | Cannot rewrite compiled code; tune via CSV/env |

**Key advantage over Codex:** Claude's multi-turn capability (30 turns vs Codex's
typical 1) makes it the best choice for T3 framework scheduling optimizations that
require reading multiple files, understanding data flow, and making coordinated changes.

**Key advantage over GEAK:** Claude can access and modify the entire framework source
tree, not just isolated kernel files. For deep optimizations that cross module
boundaries (e.g., changing how the scheduler feeds tokens into the MoE dispatch), Claude
is the only backend that can reason about the full picture.
...
- **Best for T3 (framework scheduling)**: Multi-file reasoning is Claude's sweet spot.
