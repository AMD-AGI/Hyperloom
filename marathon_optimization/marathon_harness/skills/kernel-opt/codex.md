...
for i in range(OOB_ROUND_ITERATIONS):
    ...
    speedup, details = micro_benchmark_filter(optimized_code, test_harness)
...
Each iteration appends exactly one line to the feedback context. The `micro_benchmark_filter()`
from `kernel-opt.md` Step 1b provides the result. If the GPU is busy (server running),
compilation success/failure is still reported — full speed measurement is deferred.

```
Iteration 1: speedup=1.32x (avg over xnumel=[4,16,64,256], worst=1.18x at xnumel=256)
Iteration 2: COMPILE_FAIL — NameError: name 'libdevice' is not defined
Iteration 3: CORRECTNESS_FAIL — max diff=0.152
Iteration 4: COMPILED_OK — GPU_BUSY, speedup deferred (server running)
Iteration 5: speedup=1.51x (avg over xnumel=[4,16,64,256], worst=1.41x at xnumel=256)
```
...
- Know that compilation passed even when speedup can't be measured yet
...
**Test inputs MUST come from the test harness generated in `kernel-opt.md` Step 1b.**
Never let the agent fabricate shapes — extract them from the Inductor standalone file
(Strategy A/C) or trace events (Strategy B). This prevents micro-benchmark cheating
where convenient shapes mask real-workload regressions.

| Step | Method | Failure → feedback |
|------|--------|-------------------|
| Compilation | `exec(compile(code, "kernel.py", "exec"))` | `COMPILE_FAIL — {error}` |
| Correctness | Run original + optimized with **trace-derived test harness** inputs, `torch.allclose(orig_out, opt_out, atol=1e-2, rtol=1e-2)` | `CORRECTNESS_FAIL — max diff={diff}` |
| Multi-shape benchmark | `multi_shape_benchmark()` from `kernel-opt.md` — tests at xnumel ×{1,4,16,64}. Reject if ANY shape regresses >5%. | `speedup={avg:.2f}x (worst={worst:.2f}x at xnumel={n})` |

**DeepSeek-R1 lesson:** A kernel showed +44% micro-benchmark but -19.9% E2E due to
register pressure at real occupancy. Multi-shape testing catches this: the regression
appears at higher batch sizes where occupancy matters.

## Full Call-Stack Optimization Capabilities

Codex (via OOB Agent) can read and modify ANY source code — not just Triton kernels.
This enables optimization across the entire call stack.

| Tier | Target | Codex capability | Example |
|------|--------|:----------------:|---------|
| **T1: Triton/Inductor** | Triton kernel source | **Full rewrite** | Dual-loop → single-pass, block tuning |
| **T2: aiter/CK dispatch** | Python dispatch wrappers | **Rewrite dispatch** | Edit `aiter/ops/*.py` to select faster kernel variant for model shapes |
| **T2: aiter/CK config** | GEMM CSVs, env flags | **Tune config** | Generate shape-specific GEMM CSV, toggle FP8 bypass |
| **T3: Framework scheduling** | SGLang/vLLM scheduler code | **Patch scheduling** | Rewrite batch scheduler, prefill/decode overlap logic |
| **T4: Communication** | NCCL/RCCL configs | **Tune parameters** | NCCL_MIN_NCHANNELS, quantized collectives, topology |
| **T5: Compiled binaries** | Vendor BLAS/CK | Config only | Cannot rewrite compiled code; tune via CSV/env |

**Key advantage over GEAK:** Codex can modify framework Python code that GEAK cannot
access (GEAK only has kernel source in its pod). For aiter dispatch rewrites and
framework scheduling changes, Codex is the preferred backend.
...
- For T2-T3 tasks, Codex excels at focused single-file patches (dispatch logic, config files)
