...
**DFS role:** Medium-priority action (prior=5-7). Parameter tuning is low-risk (most
params have accuracy_risk=0) and can yield large gains when the model has suboptimal
defaults (e.g., CUDA graph coverage, decode-steps). Test parameters on top of the
current best config (backends + any prior wins), not on the original baseline.
