...
### Step 2b: Run compile probe (if full torch.compile failed)

When full-model compile fails, run the compile probing from `classify.md` Step 2b to
determine which submodule types CAN compile. This enables Strategy C (selective
submodule compilation) in `kernel-opt.md`, which generates Inductor Triton targets
for GEAK even on MoE+MLA models.

Record `compile_probe_results` in state. If any submodule type is COMPILABLE, set
`selective_compile_viable = true` — this boosts kernel-opt score from 2 → 4–5
for non-compile models.

### Step 3: Record baseline throughput

```bash
# Extract baseline throughput from the result file
baseline_tput=$(python3 -c "
import json, glob
f = sorted(glob.glob('$RESULT_DIR/baseline_*.json'))[-1]
d = json.load(open(f))
print(d['output_throughput'])
")
...
- If torch.compile failed AND selective_compile_viable: moderate GEAK scores (Strategy C targets available)
- If torch.compile failed AND NOT selective_compile_viable: reduce GEAK scores, boost backend exploration and server param scores
