"""Unit tests for the two metrics providers (``local`` + ``robust``).

These complement the existing factory/integration coverage by exercising
the parsing + ring-buffer logic in isolation. We patch the subprocess /
httpx integration points so tests run without real ROCm or a
Primus-Robust-Internal endpoint.
"""
...
class TestRingBuffer:
    def test_push_then_get_within_window(self):
        ...
        # window_seconds=2 → keep the 3 most recent (delta <= 2 from latest=14).
    ...
    def test_max_age_drops_stale_entries(self):
        ...
        buf.push(_snapshot(0, ts=10.0))  # forces cutoff = 5.0
        buf.push(_snapshot(0, ts=12.0))
        # ts=0.0 should have been popped (older than cutoff at push time).
...
class TestLocalProvider:
    ...
    async def test_get_gpu_metrics_parses_rocm_smi_json(self, monkeypatch):
        ...
        # History records the timestamped snapshot.
    ...
    async def test_get_gpu_metrics_falls_back_to_nvidia_when_rocm_empty(
        ...
        provider = LocalProvider()
        snaps = await provider.get_gpu_metrics()
        # Bad row dropped, one valid snapshot retained.
        assert len(snaps) == 1
    ...
    async def test_get_process_list_parses_ps_output(self, monkeypatch):
        ...
        # First row valid; second has malformed rss; third is too short.
        assert any(p.pid == 100 for p in procs)
        # Second is still accepted because rss_kb falls back to 0.
...
class _FakeClient:
    def __init__(self, responses: dict[tuple[str, str | None], Any]):
        # responses keyed by (path, optional query short-hand) → payload or
        # exception instance.
    ...
    async def get(self, path: str, params=None, timeout=None):
        params = params or {}
        # Try path with params signature, then bare path.
        candidates = [path]
        if "query" in params:
...
class TestRobustProvider:
    ...
    async def test_get_fault_events_parses_payload(self, provider_with_responses):
        ...
        # Bad row drops; one valid event remains.
        assert len(events) == 1
