...
"""CLI entry point for `kernel-agents forge-fuse`.

Usage:
    kernel-agents forge-fuse --trace <kineto.json[.gz]> --model-path <dir> \\
...
# The command is registered on the kernel-agents CLI as `forge-fuse`; this alias
# keeps `python -m kernel_agents.fusion.command` working for direct debugging.
main = run
