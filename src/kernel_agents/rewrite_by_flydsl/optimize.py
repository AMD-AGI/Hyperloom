...
def _forge_loop_argv() -> list[str]:
    ...
    Prefer ``sys.executable -m kernel_agents.cli`` — the exact entry the
    ``kernel-agents`` console script maps to (``kernel_agents.cli:main``) — so an
    ...
    """
    if sys.executable:
        return [sys.executable, "-m", "kernel_agents.cli"]
    exe = shutil.which("kernel-agents")
    return [exe] if exe else ["kernel-agents"]
...
def run_optimize(
    ...
    *,
    ...
    try:
        ...
        # launched"): a missing kernel-agents on PATH, a bad interpreter, or a
