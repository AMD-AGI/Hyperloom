...
from kernel_agents.knowledge.implementation_identity import (
    canonical_owner_framework,
)
from kernel_agents.durable_io import atomic_write_text, fsync_directory
...
def _force_jit_rebuild(workspace_dir, kernel, source_files) -> None:
    ...
    from kernel_agents.loop.jit_rebuild import force_jit_rebuild_for_changes
...
def _record_measured_speedup(
    ...
    *,
    ...
    from kernel_agents.rewrite_by_flydsl.agent_kb import (
        KernelRecipeKB,
        kb_store_secrets,
    )
...
def kb_warmstart(*, config, kernel, driver, workspace_dir, fellow,
    ...
    if resume:
        ...
        return result
    try:
        from kernel_agents.knowledge.experience_reader import read_top_solutions
