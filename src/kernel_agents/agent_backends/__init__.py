...
standalone wheel that must never import ``kernel_agents``; it has since been
...
``kernel_agents.agent_providers``, and a third-party plugin registered against
...
``kernel_agents.agent_backends.registry`` and friends by path, not only the
