...
from kernel_agents.mcp_server.probe_stdio_server import (
...
_PROBE_CHILD_ENV_VARS = (
    # The child's own launch: `python -m kernel_agents...` from a checkout.
    "PYTHONPATH",
    "PYTHONHOME",
    "VIRTUAL_ENV",
    "CONDA_PREFIX",
...
def _device_lock_path(workspace: Path) -> Path:
    ...
    Imported here rather than at module scope: ``kernel_agents.loop`` imports
    ...
    def mcp_servers(self) -> dict[str, StdioMcpServer]:
        ...
        return {
            _PROBE_SERVER_KEY: StdioMcpServer(
                ...
                args=("-m", "kernel_agents.mcp_server.probe_stdio_server"),
                env=self.server_env(),
