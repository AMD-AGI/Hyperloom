...
from kernel_agents.mcp_server.pr_stdio_server import TOOL_NAMES as PR_TOOL_NAMES
...
def make_agent_fn(
    ...
    When a :class:`~kernel_agents.tracker.usage.UsageAccumulator` is supplied
    ...
    # binary (command not found). Mirrors kernel_agents.rtk.wrap_command, which
...
"""
    ...
    if pr_kb_repo and backend.capabilities.mcp:
        pr_mcp_servers["pr_monitor"] = StdioMcpServer(
            ...
            args=("-m", "kernel_agents.mcp_server.pr_stdio_server"),
...
"""
        ...
        if gate_enabled:
            from kernel_agents.loop.insession_gate import InSessionGate
            gate = InSessionGate(
...
def _make_session_summarizer(
    *,
    ...
    from kernel_agents.loop.lessons import SUMMARIZER_ROLE
