...
- **Any FP8 path is lossy by construction** → gate on **task accuracy** (gsm8k / mmlu / perplexity), not
  ...
  (`aiter/test_common.py:482`) is the convention used across aiter tests.
...
- amax/scale, zero-scale clamp, err-ratio gate: `ROCm/aiter@b467ce342:aiter/ops/quant.py`, `aiter/test_common.py`, `gradlib/gradlib/GemmTuner.py`.
