...
| backend | status | card |
|---|---|---|
| aiter | 🟢 sota (live `fused_moe`, DB-driven) | [aiter.md](aiter.md) |
| ck | 🟢 sota (stage-2 `moe_ck2stages_*`, block-scale) | ck |
| hip | 🟢 (stage-1 hand-tuned asm `fmoe_stage1_*`; editable) | hip |
| triton | 🟡 competitive (vLLM/sglang Triton fused-MoE; per-shape JSON tuned) | triton |
| flydsl | 🟢 sota (fp4 a4w4 2-stage + fused act/requant, CDNA4) | flydsl |
...
## Sources
- aiter `fused_moe` (two-stage grouped GEMM, quant routing, kernel names): `ROCm/aiter@b467ce342:aiter/fused_moe.py`, `aiter/configs/tuned_fmoe.csv`.
