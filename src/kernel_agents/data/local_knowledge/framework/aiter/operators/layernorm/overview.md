...
updated: 2026-07-14
sources:
  - ROCm/aiter@b467ce342:aiter/ops/triton/normalization/norm.py
  - ROCm/aiter@b467ce342:aiter/ops/norm.py
...
| backend | status | card |
|---|---|---|
| triton | 🟢 sota (aiter's own impl, two-pass) | triton |
| aiter | 🟢 sota (CK/asm `layernorm2d_fwd` + fused) | [aiter.md](aiter.md) |
| hip | 🟢 sota (vLLM `layer_norm` kernel) | hip |
| vllm_kernels | 🟢 sota (HIP + AITER wiring) | vllm_kernels |
| miopen | 🟡 competitive (primitive exists; idle for LLM) | miopen |
| flydsl | 🧪 experimental (two-statistic reduce **primitive** `make_block_reduce_add2` for μ/σ²; no standalone tuned op) | flydsl |
...
## Sources
- aiter Triton layernorm (two-pass mean/var, blocked, fused add/quant): `aiter/ops/triton/normalization/norm.py`.
- aiter C++/CK/asm (`layernorm2d_fwd`, `_with_add`, `_with_smoothquant`): `aiter/ops/norm.py`.
