...
updated: 2026-07-14
sources:
  - ROCm/aiter@b467ce342:aiter/ops/rope.py
  - ROCm/aiter@b467ce342:aiter/ops/triton/rope/rope.py
...
| backend | status | card |
|---|---|---|
| aiter | 🟢 sota (HIP/C++ rope_fwd family + fused QK-norm+RoPE) | [aiter.md](aiter.md) |
| triton | 🟢 sota (aiter Triton rope + vLLM Triton rope) | triton |
| hip | 🟢 sota (vLLM `rotary_embedding` HIP) | hip |
| vllm_kernels | 🟢 sota (HIP pos_encoding + AITER rope) | vllm_kernels |

## Fusion neighbors
`QK-norm + RoPE + KV-cache write + quant` → aiter `fused_qk_norm_rope_cache_quant_shuffle` (fused_norm_quant,
...
## Sources
- aiter rope ops (cached/uncached, 1c/2c, thd, 2d, positions/offsets): `aiter/ops/rope.py`.
- aiter Triton rope (`_rope_fwd`, BLOCK_D, BLOCK_S=32, num_warps=4): `aiter/ops/triton/rope/rope.py`.
