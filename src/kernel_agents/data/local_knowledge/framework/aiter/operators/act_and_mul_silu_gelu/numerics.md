...
## 3. Gate/up ordering (which half is gated)
`silu_and_mul` gates the **first** half: `silu(x[:d])·x[d:]`. The concat order of the fused gate/up Linear
decides correctness — if the checkpoint packs up||gate instead of gate||up, the activation is applied to
the wrong half. Confirm against the model's `gate_up_proj` packing.
...
## Sources
- aiter activation variants (silu/gelu/gelu_tanh/scaled): `aiter/ops/activation.py`.
- act+quant (fp8 group, mxfp4 e8m0 block=32): `aiter/ops/triton/activation.py`.
