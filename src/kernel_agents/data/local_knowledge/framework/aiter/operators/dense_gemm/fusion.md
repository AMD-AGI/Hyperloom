...
- Fusing changes the lookup key (bias/act/scale flags) → must capture/tune the *fused* shape, or the
  tuned DB misses (same failure mode as bias mismatch in [aiter.md](aiter.md)).
...
- bias/scale as epilogue args on live path: `ROCm/aiter@b467ce342:aiter/tuned_gemm.py`.
