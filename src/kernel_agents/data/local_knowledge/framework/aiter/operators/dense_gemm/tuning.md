...
(see [aiter.md](aiter.md)); everything below is the knob space that aiter/CK/triton
...
2. Race kernels: `csrc/gemm_a16w16/gemm_a16w16_tune.py --libtype all --indtype bf16 --mp <ngpus>`
   (multi-backend; add `--with-hipblaslt` for hipBLASLt; gate `err_ratio<0.05`).
3. Deploy by env `AITER_CONFIG_GEMM_BF16=<tuned.csv>`, verify `grep -c 'is tuned on cu_num'`.
Full recipe in [aiter.md](aiter.md). Validated **+2.23% e2e** on Qwen3.5-27B/sglang.
...
- Live tuning recipe + +2.23%: `ROCm/aiter@b467ce342`, perf_knowledge e2e run 2026-06-08 (see backends/aiter.md).
