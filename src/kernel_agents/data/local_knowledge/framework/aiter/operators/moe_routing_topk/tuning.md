...
- **gfx1250 (wave32)**: the fused gate `topk_gating_kernels.cu` adds a **wave32 multi-wave-per-token** path
  (dispatched under `get_warp_size_func()==32`), gated to the token ranges measured to win (e.g. E=256 WPT=8
  is a low-token decode win at T≤256, ~4.1–6.0µs vs reg ~5.6–6.3µs); above the gate it falls through to the
  reg kernel. `topk_gating` also validates `score_func ∈ {sqrtsoftplus, sigmoid, softmax}`.
...
- on-box kernel: `ROCm/aiter@b467ce342:csrc/kernels/topk_gating_kernels.cu` (gfx1250 wave32 gate), `csrc/kernels/moe_align_block_size_kernels.cu`, `topk_softmax_kernels_group.cu`.
