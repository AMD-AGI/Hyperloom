...
5. **Re-assemble before comparing.** Never trust a pre-existing `.co` on a remote node across sessions. Always re-assemble from the current `.s` source to avoid chasing phantom divergences from stale binaries.
