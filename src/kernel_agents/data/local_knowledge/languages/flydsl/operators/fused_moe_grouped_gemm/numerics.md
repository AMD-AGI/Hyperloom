...
## TL;DR
Grouped GEMM is parity-safe (fp32 accumulate) until you quantize; the risks are fp8/int8/fp4 quant error and the ordering of activation / routed-weight / combine. The #1 silent bug is the wrong fp8 dialect (fnuz on gfx942 vs OCP on gfx950 → off by 2×); always gate quant paths with an end-to-end eval, not the DB `err`.
