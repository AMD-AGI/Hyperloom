...
## TL;DR
Two AMD-specific levers on top of elementwise bandwidth: the cross-lane reduce shape (per-thread → **wave64 6-step shuffle** → LDS → grid) and whether to **split the reduced axis** to fill 304 CUs (essential for few-rows/huge-axis; unneeded for many-rows). Combine split partials via atomicAdd (fast, nondeterministic) or two-call (deterministic); block 256, 128-bit loads, fp32 accumulate.
