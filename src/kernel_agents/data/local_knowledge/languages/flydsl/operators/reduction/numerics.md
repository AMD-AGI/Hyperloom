...
## TL;DR
FP add is not associative, so tree/sequential/atomic reduces differ in low bits — **accumulate in fp32 always**; atomic-split sums are nondeterministic run-to-run (fine for sum/mean if a task gate passes, but never on an argmax/logits path — use the deterministic two-call path there); AMD `v_max/min` return the **non-NaN** operand (unlike torch). Don't expect bitwise parity across backends.
