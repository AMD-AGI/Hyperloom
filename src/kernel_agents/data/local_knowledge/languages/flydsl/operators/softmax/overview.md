...
| backend | status |
|---|---|
| triton | 🟢 sota (aiter online-softmax impl) |
| aiter | 🟢 sota (Triton + fused-into-attn/topk) |
| hip | 🟢 sota (hand-written wave64 reduce) |
| flydsl | 🧪 experimental (max/sum reduce **primitives** `reduce_vec_max`/`reduce_vec_sum`/`make_block_reduce`; no standalone op) |
