...
## TL;DR
A fusion-first op: fold `act(gate)·up` into the up/gate GEMM epilogue (write `[M,d]` not `[M,2d]`), then fuse output fp8/fp4 quant so the down-proj reads ½–¼ the bytes; in production it lives entirely inside fused-MoE stage-1 with no HBM materialization.
