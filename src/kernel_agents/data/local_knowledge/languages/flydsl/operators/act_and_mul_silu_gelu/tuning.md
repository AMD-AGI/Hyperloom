...
## TL;DR
Memory-bound elementwise op: saturate bandwidth with 128-bit strided loads of the two halves and a small `BLOCK_M` (≤8, wide rows); but the real lever is not tuning it standalone — fuse `act(+quant)` into the up/gate GEMM epilogue or fused-MoE stage-1.
