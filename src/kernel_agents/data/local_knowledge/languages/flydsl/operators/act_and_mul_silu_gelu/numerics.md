...
## TL;DR
Compute the activation in fp32 even for bf16 IO; **match the checkpoint's GeLU variant** (exact-erf vs tanh-approx) and the **gated half** (`silu_and_mul` vs `mul_and_silu`) or it is a silent accuracy bug; fp8/fp4 fused-quant output must use the correct dialect (fnuz on gfx942, FP4 only on gfx950) and be gated at the task level, not allclose.
