...
| backend | status |
|---|---|
| aiter | 🟢 sota (CK/asm `silu_and_mul` + flydsl fused quant) |
| triton | 🟢 sota (aiter act+quant Triton kernels) |
| hip | 🟢 sota (vLLM `silu_and_mul` HIP) |
| vllm_kernels | 🟢 sota (HIP act.cu + AITER wiring) |
| flydsl | 🟢 sota (SiLU·mul + fused MXFP4/MXFP8 quant `silu_and_mul_fq`) |
