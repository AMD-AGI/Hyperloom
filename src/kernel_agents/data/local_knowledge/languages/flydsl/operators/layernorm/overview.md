...
| backend | status |
|---|---|
| triton | 🟢 sota (aiter's own impl, two-pass) |
| aiter | 🟢 sota (CK/asm `layernorm2d_fwd` + fused) |
| hip | 🟢 sota (vLLM `layer_norm` kernel) |
| vllm_kernels | 🟢 sota (HIP + AITER wiring) |
| miopen | 🟡 competitive (primitive exists; idle for LLM) |
| flydsl | 🧪 experimental (two-statistic reduce **primitive** `make_block_reduce_add2` for μ/σ²; no standalone tuned op) |
