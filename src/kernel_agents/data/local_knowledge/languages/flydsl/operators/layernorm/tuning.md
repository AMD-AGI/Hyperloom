...
## TL;DR
Same bandwidth-bound playbook as RMSNorm plus a second statistic (mean) and a bias add; the one new decision is **two-pass on-chip** (N ≤ block — simplest and fastest) vs **one-pass Welford** (N > block — avoids a second HBM read). Keep `num_warps` 2–4, vectorize x, cache γ/β in LDS.
