...
- **Sweep in BOTH directions.** Every literal on the host dispatch path is a search variable, not a
  ...
  an ordinary module constant in an ordinary file (see `[[optimization/edit_surface_reach.md]]`).
...
## See also
- `[[optimization/edit_surface_reach.md]]` — which constants and files are in reach at all.
- `[[optimization/autotuning_methodology.md]]` — searching a structured config space once the cheap
  one-constant questions are answered.
- `[[profiling/benchmarking_methodology.md]]` — noise band, warmup, same-session A/B.
