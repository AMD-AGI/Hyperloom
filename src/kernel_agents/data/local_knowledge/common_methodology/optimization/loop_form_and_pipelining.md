...
- **Stream pipelining** (`[[optimization/memory_pipelining.md]]`) is what overlaps
...
- **Direct-to-LDS async copy** (`global_load_lds` / `buffer_load ... lds`, `s_wait_asynccnt`) needs its
  ...
  also frees the staging VGPRs (`[[optimization/occupancy_and_registers.md]]`), so losing it costs
...
- **LDS and registers.** The static form stages a full tile per iteration and `num_stages>1`
  ...
  (`[[optimization/lds_and_bank_conflicts.md]]`, `[[optimization/occupancy_and_registers.md]]`).
...
(`[[profiling/benchmarking_methodology.md]]`).
...
- **ISA dump** (`AMDGCN_ENABLE_DUMP=1`,
  `[[languages/triton/skills/optimize/triton_levers/isa_verify.md]]`): a pipelined loop issues the
...
- **Profiler**: exposed global latency shows as memory-wait stalls before the consumer with HBM far
  ...
  (`[[profiling/reading_a_kernel_bottleneck.md]]`, `[[optimization/roofline_and_bottlenecks.md]]`).
...
## See also
- `[[optimization/memory_pipelining.md]]` — what the pipeline does once the loop form allows it;
  `num_stages`, prefetch distance, `global_load_lds`, CDNA3 vs CDNA4.
- `[[optimization/roofline_and_bottlenecks.md]]` — classify first; this lever is for latency-bound and
  memory-bound loops, not for a compute-bound body.
- `[[optimization/cheap_sweeps.md]]` — one command per `num_stages` point.
- `[[optimization/numerical_stability.md]]` — the gate the mask has to survive.
- `[[languages/triton/skills/optimize/triton_levers/deep_codegen.md]]` — the AMD stream pipeliner and
