...
(`[[optimization/numerical_stability.md]]`). Confirm the symbol actually reached the module by reading
...
see `[[optimization/autotuning_methodology.md]]`), and confirm engagement from the log or a marker
...
## See also
- `[[optimization/cheap_sweeps.md]]` — once a constant is in reach, measure it in one command.
- `[[optimization/autotuning_methodology.md]]` — lookup keys, engagement checks, tuned tables.
- `[[optimization/numerical_stability.md]]` — the gate any injected fast path must survive.
