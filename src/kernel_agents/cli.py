...
"""CLI entry point for kernel-agents."""
...
from kernel_agents.knowledge.experience_integration import (
...
from kernel_agents.loop.recovery import (
    atomic_write_json,
...
if TYPE_CHECKING:
    ...
    from kernel_agents.knowledge.pr_monitor_refs import PRRefsResult
...
def _persist_declared_spec(invocation_spec_file: str, driver: str) -> None:
    ...
    if not invocation_spec_file:
        return
    from kernel_agents.loop.task_preparer import _materialize_invocation_spec
...
def _collect_pr_references(
    *,
    ...
    """
    from kernel_agents.knowledge.pr_monitor_refs import (
        PR_KB_RECOVERABLE,
        collect_references,
    )
...
@click.group()
@click.version_option(package_name="kernel-agents")
def main():
...
def _make_lane_agent_factory(
    *,
    ...
    from kernel_agents.loop.aiter_cache import child_cache_environment
...
def forge_loop(kernel, driver, workspace_dir, snr_threshold,
    ...
    if producer:
        from kernel_agents.knowledge.kernel_identity import KERNEL_RECIPE_PRODUCERS
    ...
    from kernel_agents.knowledge.experience_integration import git_head
    from kernel_agents.loop.campaign_config import (
        CampaignConfigStore,
        derive_campaign_implementation_contract,
    )
    from kernel_agents.loop.run_state import WorkspaceLock, WorkspaceLockError
    ...
    from kernel_agents.loop.runner import IterationLoop
    ...
    if prepare_task and not resume:
        from kernel_agents.loop.task_preparer import (
            declared_case_ids,
            preflight_task,
            prepare_task_sync,
        )
        ...
        if pf.ok:
            ...
            from kernel_agents.loop import task_preparer as _tp
    ...
    from kernel_agents.orchestrator.analysis import make_analysis_agent_service
    ...
    from kernel_agents.orchestrator.supervisor import make_supervisor_fn
...
def _validate_rewrite_framework(_ctx, _param, value):
    ...
    from kernel_agents.rewrite_by_flydsl.protocol import SUPPORTED_FRAMEWORKS
...
def _emit_rewrite_capabilities(ctx, _param, value):
    ...
    if not value or ctx.resilient_parsing:
        return
    from kernel_agents.rewrite_by_flydsl.protocol import capabilities
...
def _emit_rewrite_applyback_contract(ctx, _param, value):
    ...
    if not value or ctx.resilient_parsing:
        return
    from kernel_agents.rewrite_by_flydsl.protocol import applyback_contract_example
...
def forge_rewrite(source_kernel, driver, prepare_driver, invocation_spec_file,
    ...
    Example:
        kernel-agents forge-rewrite-by-flydsl --source-kernel softmax.py \\
    ...
    from kernel_agents.rewrite_by_flydsl import run_rewrite
...
def _register_forge_fuse() -> None:
    ...
    Importing it lazily keeps `kernel-agents --help` free of the fusion
    pipeline's import cost, which pulls in the trace and validation stack.
    """
    from kernel_agents.fusion.command import run as forge_fuse
