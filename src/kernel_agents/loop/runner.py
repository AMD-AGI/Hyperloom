...
from kernel_agents.learning.auto_evolve import AutoEvolver
from kernel_agents.loop.canonical_correctness import accept_candidate
from kernel_agents.loop.validation import run_validation_pipeline
from kernel_agents.loop.experience import ExperienceLedger
from kernel_agents.loop.lessons import (
    SUMMARY_MIN_SECONDS,
    UNDISPROVEN_CLAIM,
    LessonScope,
    LessonStore,
...
from kernel_agents.loop.archive import CandidateArchive, CandidateRecord
from kernel_agents.loop.handoffs import HandoffStore, IterationHandoff
from kernel_agents.loop.jit_rebuild import (
    force_jit_rebuild,
    tracked_source_changes,
)
from kernel_agents.loop.analysis_runtime import AnalysisRuntimeMixin
from kernel_agents.loop.search_policy import (
...
from kernel_agents.loop.round_budget import (
    admit_dispatch,
    admit_round,
    estimate_measurement_sec,
)
from kernel_agents.loop.run_state import (
...
from kernel_agents.orchestrator.orchestration import (
    OrchestrationInfrastructureError,
)
from kernel_agents.orchestrator.supervisor import (
    clear_latest_supervisor_ruling,
    latest_supervisor_ruling_path,
    load_latest_supervisor_ruling,
    persist_supervisor_ruling,
)
from kernel_agents.loop.prompt_view import (
    MAX_RECENT_ATTEMPT_LINES,
    render_long_horizon_header,
)
from kernel_agents.loop.reporting import BestResultPublisher
from kernel_agents.rtk import smart_wrap
from kernel_agents.mcp_server.tools.bench import (
    CaseCoverageError,
    calculate_mean_case_speedup,
    calculate_measurement_case_speedups,
    measure_wallclock,
)
from kernel_agents.mcp_server.tools._subprocess import communicate_process_group
from kernel_agents.loop.new_path_allowlist import (
    matches_commit_new_paths,
    normalize_commit_new_paths,
)
from kernel_agents.durable_io import atomic_write_text
from kernel_agents.loop.scoring import (
...
from kernel_agents.loop.baseline_reference import (
    BASELINE_DRIFT_TOLERANCE,
    BASELINE_DRIFT_TOLERANCE_ENV,
    check_baseline_against_reference,
)
from kernel_agents.loop.device_hazard import DeviceHazard, DeviceHazardLog
from kernel_agents.loop.fanout import LanePlan, LaneResult, run_lanes
from kernel_agents.loop.merge_candidates import (
...
from kernel_agents.mcp_server.tools.registers import check_registers
from kernel_agents.tracker import ExperimentTracker, Experiment
...
class SigmaResolution:
    ...
    ``sigma`` is what :func:`~kernel_agents.loop.scoring.required_keep_speedup`
...
class IterationLoop(AnalysisRuntimeMixin):
    ...
    def _merge_attempt_refusal(self) -> str:
        ...
        :func:`~kernel_agents.loop.merge_candidates.eligible_candidates` skips.
    ...
    async def _resolve_keep_sigma(
        ...
        When :func:`~kernel_agents.loop.scoring.attribute_sigma` names a
        ...
        is what
        :data:`~kernel_agents.loop.scoring.SIGMA_REMEASURE_MAX_ROUNDS` bounds.
    ...
    def _persist_lane_queue(self) -> None:
        ...
        """
        from kernel_agents.loop.recovery import atomic_write_json
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            try:
                from kernel_agents.loop.aiter_cache import (
    ...
    async def _run_locked(
        ...
        *,
        ...
        if self.ic.pr_kb_snapshot:
            from kernel_agents.knowledge.pr_monitor_refs import commit_snapshot
            from kernel_agents.knowledge.pr_query_context import REASON_LOCAL_FAILURE
        ...
        if supervisor_fn is not None:
            from kernel_agents.loop.supervisor import SupervisionMonitor
        ...
        while True:
            ...
            # cycles :mod:`kernel_agents.loop.round_budget` is calibrated on.
            self._close_round()
