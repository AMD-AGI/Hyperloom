...
from kernel_agents.learning.tuning_db import (
    TransferRule,
    TuningDatabase,
    TuningEntry,
)
...
def test_tuning_entry_keys_and_roundtrip():
    e = TuningEntry(
        operation="gemm", backend="ck", gpu_target="gfx950", dtype="bf16",
        shape={"N": 4096, "M": 1024}, config={"BLOCK_M": 128}, wall_ms=1.0,
    )
...
def test_transfer_rule_roundtrip():
    r = TransferRule(rule_id="r1", description="d", scope="all",
                     parameter="wpe", recommended_value=2)
    d = r.to_dict()
...
def test_log_returns_entry_but_does_not_persist(tmp_path):
    ...
    entry = db.log(operation="gemm", backend="ck", gpu_target="gfx950",
                   dtype="bf16", shape={"M": 4096}, config={"BLOCK_M": 64},
                   wall_ms=1.0)
...
def test_all_entries_skips_blank_lines(tmp_path):
    db = TuningDatabase(tmp_path)
    with open(db._entries_path, "w") as f:
        f.write(json.dumps({"operation": "gemm", "backend": "ck",
                            "gpu_target": "gfx950", "dtype": "bf16",
                            "shape": {"M": 4096}, "config": {}, "wall_ms": 1.0}) + "\n")
        f.write("\n")  # blank line ignored
...
def test_suggest_configs_all_levels(tmp_path):
    ...
    _seed_golden(db, {
        "attention_bwd|ck|gfx950|bf16|seq_len=8192": {
            "config": {"BLOCK_M": 128}, "wall_ms": 80.0},
    })
    # entries drive level 2 (similar shape) + level 3 (related op)
    _seed_entries(db, [
        dict(operation="attention_bwd", backend="ck", gpu_target="gfx950",
             dtype="bf16", shape={"seq_len": 8192}, config={"BLOCK_N": 64},
             wall_ms=82.0, passed_correctness=True),
        dict(operation="attention_fwd", backend="ck", gpu_target="gfx950",
             dtype="bf16", shape={"seq_len": 8192}, config={"wpe": 2},
             wall_ms=9.0, passed_correctness=True),
    ])
    # transfer rule for level 4
    _seed_rules(db, [dict(rule_id="r", description="d", scope="attention_*",
                          parameter="wpe", recommended_value=2, confidence=0.6)])

    suggestions = db.suggest_configs("attention_bwd", "ck",
                                     shape={"seq_len": 8192}, max_suggestions=10)
...
def test_suggest_configs_dedup_and_max(tmp_path):
    db = TuningDatabase(tmp_path)
    _seed_entries(db, [
        dict(operation="gemm", backend="ck", gpu_target="gfx950", dtype="bf16",
             shape={"M": 4096}, config={"BLOCK_M": 128}, wall_ms=1.0,
             passed_correctness=True),
        dict(operation="gemm", backend="ck", gpu_target="gfx950", dtype="bf16",
             shape={"M": 4096}, config={"BLOCK_M": 128}, wall_ms=1.2,
             passed_correctness=True),  # duplicate config -> deduped
    ])
    suggestions = db.suggest_configs("gemm", "ck", shape={"M": 4096},
                                     max_suggestions=1)
...
def test_add_transfer_rule_new_and_update(tmp_path):
    ...
    _seed_rules(db, [])
    db.add_transfer_rule(rule_id="r1", description="first", scope="all",
                         parameter="wpe", recommended_value=2,
                         evidence=["e1", "e2"])
    ...
    _seed_rules(db, [dict(rule_id="r1", description="first", scope="all",
                          parameter="wpe", recommended_value=2, anti_value=None,
                          evidence=["e1"], confidence=0.2)])
    db.add_transfer_rule(rule_id="r1", description="updated", scope="all",
                         parameter="wpe", recommended_value=3,
                         evidence=["e2"])
...
def test_discover_transfer_rules_too_few_entries(tmp_path):
    ...
    _seed_entries(db, [
        dict(operation="gemm", backend="ck", gpu_target="gfx950", dtype="bf16",
             shape={"M": 4096}, config={"wpe": 2}, wall_ms=1.0,
             passed_correctness=True),
    ])
...
def test_discover_transfer_rules_finds_winner(tmp_path):
    ...
    for op in ["attention_fwd", "attention_bwd", "sla_fwd"]:
        entries.append(dict(operation=op, backend="flydsl", gpu_target="gfx950",
                            dtype="bf16", shape={"seq_len": 4096},
                            config={"wpe": 2}, wall_ms=8.0,
                            passed_correctness=True))
        entries.append(dict(operation=op, backend="flydsl", gpu_target="gfx950",
                            dtype="bf16", shape={"seq_len": 4096},
                            config={"wpe": 3}, wall_ms=12.0,
                            passed_correctness=True))
...
def test_discover_transfer_rules_single_value_skipped(tmp_path):
    ...
    entries = [
        dict(operation=f"op{i}", backend="ck", gpu_target="gfx950", dtype="bf16",
             shape={"M": 4096}, config={"wpe": 2}, wall_ms=1.0,
             passed_correctness=True)
        for i in range(6)
    ]
...
def test_context_for_task_with_data(tmp_path):
    ...
    _seed_golden(db, {
        "gemm|ck|gfx950|bf16|M=4096": {
            "config": {"BLOCK_M": 128}, "wall_ms": 0.5,
            "pmc_diagnosis": "COMPUTE-BOUND"},
    })
    _seed_rules(db, [dict(rule_id="r", description="use wpe=2", scope="all",
                          parameter="wpe", recommended_value=2, anti_value=3,
                          confidence=0.6)])
