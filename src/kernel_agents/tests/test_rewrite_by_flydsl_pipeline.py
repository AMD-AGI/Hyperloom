...
from kernel_agents.rewrite_by_flydsl import port_loop
from kernel_agents.rewrite_by_flydsl.attempt import create_attempt_workspace
from kernel_agents.rewrite_by_flydsl.applyback import ApplybackResult
from kernel_agents.rewrite_by_flydsl.kb import RewriteKbReadResult
from kernel_agents.rewrite_by_flydsl.spec import RewriteSpec
...
def test_optimize_argv_uses_current_interpreter():
    ...
    assert argv[-2:] == ["-m", "kernel_agents.cli"]
...
def test_optimize_argv_falls_back_to_console_script(monkeypatch):
    ...
    monkeypatch.setattr(optimize.shutil, "which", lambda name: "/usr/bin/kernel-agents")
    assert optimize._forge_loop_argv() == ["/usr/bin/kernel-agents"]
...
def test_optimize_returns_empty_on_launch_failure(tmp_path, monkeypatch):
    ...
    def _boom(cmd, **kw):
        raise FileNotFoundError("kernel-agents not found")
...
def _install_agent(monkeypatch, kernel_text):
    ...
    import kernel_agents.orchestrator.agent as agent_mod
