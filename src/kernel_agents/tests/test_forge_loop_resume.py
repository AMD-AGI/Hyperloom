...
import kernel_agents.cli as cli_module
import kernel_agents.loop.recovery as recovery_module
import kernel_agents.orchestrator.agent as agent_module
import kernel_agents.orchestrator.analysis as analysis_module
import kernel_agents.orchestrator.orchestration as orchestration_module
import kernel_agents.orchestrator.supervisor as supervisor_module
...
from kernel_agents.loop.runner import IterationConfig, IterationLoop
...
def _install_cli_fakes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("kernel_agents.loop.runner.IterationLoop", FakeLoop)
    monkeypatch.setattr("kernel_agents.tracker.ExperimentTracker", FakeTracker)
...
def test_resume_reuses_persisted_framework_without_inference(
    ...
    monkeypatch.setattr(
        "kernel_agents.loop.campaign_config.infer_source_owner_framework",
        fail_inference,
    )
    result = CliRunner().invoke(
        main,
...
def test_fresh_campaign_captures_post_prep_driver_digest_and_base(
    ...
    monkeypatch.setattr(
        "kernel_agents.loop.task_preparer.preflight_task", fake_preflight
    )
    monkeypatch.setattr(
        "kernel_agents.loop.task_preparer.prepare_task_sync", fake_prepare
    )
