...
from kernel_agents.loop import analysis_evidence
from kernel_agents.loop import fanout
from kernel_agents.loop import runner as runner_module
from kernel_agents.loop.analysis_refresh_policy import AnalysisRefreshDecision
from kernel_agents.loop.archive import CandidateRecord
from kernel_agents.loop.device_hazard import MAX_BLOCKED_ITERATIONS
from kernel_agents.loop.reporting import _round_budget_lines
...
def test_a_snapshot_write_failure_records_degradation_and_continues(
    ...
    monkeypatch.setattr(
        "kernel_agents.knowledge.pr_monitor_refs.commit_snapshot",
        fail_snapshot,
    )
...
def _attach_lessons(loop, workspace):
    from kernel_agents.loop.lessons import LessonStore
...
def test_record_lesson_carries_an_undisproven_cannot_into_the_scope(
    ...
    from kernel_agents.loop.lessons import UNDISPROVEN_CLAIM
...
def test_record_lesson_carries_a_refuted_cannot_as_an_open_direction(
    ...
    from kernel_agents.loop.lessons import is_claim_disproved
...
def test_an_unscored_case_does_not_reopen_a_stored_negative_in_the_prompt(
    ...
    from kernel_agents.loop.lessons import LessonScope, LessonStore
...
def test_handoff_carries_the_scope_of_the_iterations_lesson(
    ...
    from kernel_agents.loop.handoffs import HandoffStore
...
def test_decision_label_matches_every_outcome():
    from kernel_agents.loop.runner import _decision_label
