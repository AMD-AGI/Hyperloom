...
from kernel_agents.knowledge.pr_monitor_search import PRReference
...
class _StubClient:
    ...
    def get_many(self, requests, *, budget_sec=None):
        from kernel_agents.knowledge.pr_monitor_client import FetchOutcome
...
def test_the_whole_lookup_fits_inside_one_end_to_end_budget(monkeypatch, tmp_path):
    ...
    for module in ("pr_monitor_refs", "pr_monitor_search"):
        monkeypatch.setattr(
            f"kernel_agents.knowledge.{module}.time.monotonic", _now
        )
    monkeypatch.setattr(
        "kernel_agents.knowledge.pr_monitor_client.urllib.request.urlopen", slow
    )
...
def test_the_deadline_starts_before_local_snapshot_loading(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "kernel_agents.knowledge.pr_monitor_refs.load_snapshot",
        slow_snapshot,
    )
