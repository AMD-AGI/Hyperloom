...
def test_bench_once_returns_complete_suite(monkeypatch):
    import kernel_agents.mcp_server.tools.bench as bench
...
def test_correctness_once_true(monkeypatch):
    import kernel_agents.mcp_server.tools.test as test_mod
...
def test_kb_warmstart_reference_only_when_patch_empty(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "kernel_agents.knowledge.experience_reader.read_top_solutions", fake_read)
    monkeypatch.setattr(
        integ,
        "_bench_once",
        lambda *_a, **_k: {
...
def test_kb_warmstart_swallows_reader_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "kernel_agents.knowledge.experience_reader.read_top_solutions", boom)
    warm = integ.kb_warmstart(
        config=object(), kernel=str(repo / "kernel.py"), driver="d.py",
        workspace_dir=str(repo), fellow="triton-fellow")
    assert warm == {
