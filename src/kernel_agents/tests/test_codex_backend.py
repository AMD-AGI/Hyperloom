...
from kernel_agents.cli import _agent_runtime_overrides
...
def test_codex_backend_materializes_custom_agent_roles(
    ...
    spec = replace(
        ...
        mcp_servers={
            "gpu": StdioMcpServer(
                ...
                args=(
                    "-m",
                    "kernel_agents.mcp_server.pr_stdio_server",
                ),
    ...
    assert ('mcp_servers.gpu.args=["-m", "kernel_agents.mcp_server.pr_stdio_server"]') in overrides
...
def test_outer_gate_counts_only_incremental_resume_target_edits(
    ...
    monkeypatch.setattr(agent_module, "create_registered_backend", fake_factory)
    monkeypatch.setattr(
        "kernel_agents.loop.insession_gate.InSessionGate._on_stop",
        fake_on_stop,
    )
    kernel = tmp_path / "kernel.py"
    kernel.write_text("VALUE = 1\n")
...
def test_codex_writable_author_restore_failure_is_not_swallowed(
    ...
    """
    from kernel_agents.agent_backends import codex as codex_module
