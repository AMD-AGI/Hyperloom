...
"""Gluon fellow registration, the triton<->gluon knowledge pairing, and detection.
...
first would send every such kernel to the wrong fellow.
"""
...
from kernel_agents.fellows.base import build_single_fellow_prompt
from kernel_agents.fellows.constants import (
    FELLOW_BACKENDS,
    resolve_language_dir,
    resolve_language_dirs,
)
from kernel_agents.loop.campaign_config import infer_fellow
...
def test_gluon_renders_a_forge_loop_prompt(config):
    assert build_single_fellow_prompt(config, "gluon-fellow")
...
class TestLanguagePairing:
    ...
    @pytest.mark.parametrize("fellow", ["gluon-fellow", "triton-fellow"])
    def test_both_prompts_carry_both_layers(self, config, fellow):
        prompt = build_single_fellow_prompt(config, fellow)
        assert "languages/gluon" in prompt, f"{fellow}: no Gluon knowledge layer"
        assert "languages/triton" in prompt, f"{fellow}: no Triton knowledge layer"
...
class TestKnowledgeBuilderAcceptsASequence:
    ...
    def test_a_bare_string_still_works(self, config):
        from kernel_agents.knowledge import build_forge_knowledge
...
class TestKnowledgeTree:
    ...
    def test_index_is_loaded_into_the_prompt(self, config):
        ...
        prompt = build_single_fellow_prompt(config, "gluon-fellow")
...
class TestInferFellow:
    ...
    def test_a_gluon_kernel_infers_the_gluon_fellow(self, tmp_path):
        path = tmp_path / "kernel.py"
        path.write_text(_GLUON_KERNEL)
        assert infer_fellow([path]) == "gluon-fellow"
    ...
    def test_merely_mentioning_gluon_does_not_infer_gluon(self, tmp_path):
        ...
        assert infer_fellow([path]) == "triton-fellow"
    ...
    def test_the_aiter_framework_arm_still_outranks_the_language(self, tmp_path):
        ...
        ``infer_fellow`` picks the FRAMEWORK fellow for anything under aiter,
        ...
        ``--fellow gluon-fellow`` explicitly for such a campaign, or accept that
        ...
        path.parent.mkdir(parents=True)
        path.write_text(_GLUON_KERNEL)
        assert infer_fellow([path]) == "aiter-fellow"
...
class TestTritonEscalationHint:
    ...
    def triton_prompt(self, config) -> str:
        return build_single_fellow_prompt(config, "triton-fellow")
...
class TestAiterFellowRoutesToAuthoringKnowledge:
    """aiter-fellow has no language layer, so it must at least name the route.
    ...
    def aiter_prompt(self, config) -> str:
        return build_single_fellow_prompt(config, "aiter-fellow")
...
class TestGluonPromptDiscipline:
    """What the Gluon prompt must carry, beyond the shared fellow contract."""

    @pytest.fixture()
    def gluon_prompt(self, config) -> str:
        return build_single_fellow_prompt(config, "gluon-fellow")
