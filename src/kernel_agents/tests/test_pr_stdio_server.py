...
from kernel_agents.mcp_server import pr_stdio_server as server
...
def test_find_references_returns_ranked_results(monkeypatch):
    from kernel_agents.knowledge.pr_monitor_search import PRReference, SearchOutcome
...
def test_find_references_accepts_a_bare_string_keyword(monkeypatch):
    ...
    def fake_discover(client, context, **kwargs):
        from kernel_agents.knowledge.pr_monitor_search import SearchOutcome
...
def test_server_speaks_json_rpc_over_stdio():
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "kernel_agents.mcp_server.pr_stdio_server"],
