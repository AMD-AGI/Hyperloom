...
"""Fellow system-prompt assembly for the iteration loop."""
...
def build_single_fellow_prompt(
    config: Config,
    fellow_name: str,
    *,
    task_type: str = "",
    ...
    """Build ONE fellow's system prompt for the autonomous forge-loop (no network).

    One fellow runs per kernel, so the prompt carries that fellow's role and
    ...
    layers for the task.
      ...
      * ``framework/aiter/`` — when the target is an AITER operator (whole-repo
        task with an ``aiter`` path component) or the fellow is the aiter fellow.
      ...
      * ``languages/<lang>/`` — the kernel's implementation language(s), resolved
        from the fellow backend by ``resolve_language_dirs`` (aiter / hipblaslt
    ...
    Returns the prompt text, or "" for an unknown fellow.
    """
    if not fellow_name.endswith("-fellow"):
        return ""
    backend = fellow_name.removesuffix("-fellow")
    module_path = FELLOW_PROMPT_MODULES.get(backend)
