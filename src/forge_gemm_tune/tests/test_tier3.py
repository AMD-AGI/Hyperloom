...
from forge_gemm_tune.router import TunerSpec
from forge_gemm_tune.tier3 import (
    build_mandate,
    contract,
    coverage_gaps,
    judge_candidates,
...
def clock(monkeypatch):
    c = _Clock([])
    monkeypatch.setattr("forge_gemm_tune.tier3.referee.time.perf_counter", c.perf_counter)
    return c
...
class TestReferee:
    ...
    def test_interference_on_one_side_only_is_refused(self, clock):
        ...
        from forge_gemm_tune.tier3 import referee
