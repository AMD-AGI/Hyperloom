...
from forge_gemm_tune.tier3 import gate, ledger, sandbox
from forge_gemm_tune.tier3.coverage import CoverageGap
from forge_gemm_tune.tier3.runner import attempt_generated_tuner
...
class TestTheWholeChain:
    ...
    def _writes(self, tmp_path, rows, candidates):
        ...
        def _fake_generate(mandate, work_dir, **kw):
            from forge_gemm_tune.tier3.generate import GeneratedTuner
...
def test_the_generator_degrades_when_no_agent_provider_is_installed(tmp_path, monkeypatch):
    ...
    from forge_gemm_tune.tier3.generate import generate_tuner
    from forge_gemm_tune.tier3.mandate import build_mandate

    monkeypatch.setitem(sys.modules, "forge_llm.agent_backends.registry", None)
