...
from forge_gemm_tune import cli as cli_mod
from forge_gemm_tune.cli import main
...
def _stub_preflight(monkeypatch):
    monkeypatch.setattr(
        "forge_gemm_tune.aiter_preflight.collect",
...
def test_cli_resolves_auto_once_and_records_effective_gpu(tmp_path, monkeypatch):
    from forge_gemm_tune import router
...
def test_cli_auto_detection_failure_aborts_before_model_or_tuning(tmp_path, monkeypatch):
    from forge_gemm_tune import model_analyzer, router
