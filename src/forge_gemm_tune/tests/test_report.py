...
from forge_gemm_tune.tuners.base import TuneResult
from forge_gemm_tune.report import build_report
...
class TestBuildReport:
    def test_all_skipped(self):
        report = build_report(
            ...
            profile=_make_profile(), framework="sglang", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64, 128], started_at="2026-01-01T00:00:00Z", total_elapsed_s=1.0,
        )
    ...
    def test_candidate(self):
        result = TuneResult(
            tuner_name="fmoe_ck", status="ok",
            artifact_path="/path/to/csv", env_var="AITER_CONFIG_FMOE",
            env_value="/path/to/csv", improved_shapes=2, total_shapes=5,
            best_micro_speedup=1.15, avg_micro_speedup=1.08,
        )
        report = build_report(
            results=[result], skipped=[],
            profile=_make_profile(), framework="sglang", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=256,
            tokens=[64, 128, 256], started_at="2026-01-01T00:00:00Z", total_elapsed_s=70.0,
        )
    ...
    def test_candidate_with_env_vars(self):
        result = TuneResult(
            tuner_name="vllm_dense_tunableop", status="ok",
            ...
            improved_shapes=3, total_shapes=3, candidate=True,
        )
        report = build_report(
            results=[result], skipped=[],
            profile=_make_profile(), framework="vllm", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=80.0,
        )
        assert report.micro_decision == "candidate"
    ...
    def test_no_improvement(self):
        result = TuneResult(
            ...
            improved_shapes=0, total_shapes=10,
            best_micro_speedup=1.0, avg_micro_speedup=1.0,
        )
        report = build_report(
            results=[result], skipped=[],
            profile=_make_profile(), framework="sglang", precision="fp8",
            quant_type="blockscale", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=120.0,
        )
        assert report.status == "ok"
    ...
    def test_forced_candidate_promoted_despite_no_improvement(self):
        ...
        result = TuneResult(
            tuner_name="a8w8_blockscale", status="no_improvement",
            artifact_path="/path/to/splitk.csv",
            env_var="AITER_CONFIG_GEMM_A8W8_BLOCKSCALE", env_value="/path/to/splitk.csv",
            improved_shapes=0, total_shapes=48,
            best_micro_speedup=1.0, avg_micro_speedup=1.0, candidate=True,
        )
        report = build_report(
            results=[result], skipped=[],
            profile=_make_profile(), framework="vllm-aiter", precision="fp8",
            quant_type="blockscale", gpu_type="mi355x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=120.0,
        )
        assert report.requires_e2e_validation is True
    ...
    def test_partial_failure_not_fatal(self):
        ...
        """
        failed = TuneResult(tuner_name="vllm_moe_triton", status="failed",
                            error="unsupported", error_class="api_error")
        ok = TuneResult(tuner_name="fmoe_ck", status="ok",
                        improved_shapes=0, total_shapes=3,
                        best_micro_speedup=1.0, avg_micro_speedup=1.0)
        report = build_report(
            results=[failed, ok], skipped=[],
            profile=_make_profile(), framework="sglang", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=10.0,
        )
        assert report.status == "ok"
    ...
    def test_all_failed(self):
        ...
        report = build_report(
            results=[failed1, failed2], skipped=[],
            profile=_make_profile(), framework="sglang", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=5.0,
        )
    ...
    def test_candidate_wins_over_failure(self):
        ...
        candidate = TuneResult(
            tuner_name="fmoe_ck", status="ok",
            artifact_path="/csv", env_var="AITER_CONFIG_FMOE", env_value="/csv",
            improved_shapes=1, total_shapes=3, best_micro_speedup=1.1,
        )
        report = build_report(
            results=[failed, candidate], skipped=[],
            profile=_make_profile(), framework="sglang", precision="bf16",
            quant_type="none", gpu_type="mi300x", tp=1, conc=64,
            tokens=[64], started_at="2026-01-01T00:00:00Z", total_elapsed_s=80.0,
        )
