...
from forge_gemm_tune.aiter_preflight import classify, collect, is_aligned, main, serve_aiter_path
...
def test_collect_falls_back_to_package_version_when_commit_unset(monkeypatch, tmp_path):
    ...
    from forge_gemm_tune import aiter_preflight as ap
...
def test_collect_no_serve_aiter_is_hard(monkeypatch):
    monkeypatch.setattr("forge_gemm_tune.aiter_preflight.serve_aiter_path", lambda: None)
    st = collect({})
    assert st["aligned"] is False
...
def test_splitk_trial_key_matches_blockscale_tuner():
    ...
    from forge_gemm_tune.tuners._aiter_dense_common import SPLITK_TRIAL_SCRIPT_KEY
    from forge_gemm_tune.tuners.a8w8_blockscale import A8W8BlockscaleTuner
