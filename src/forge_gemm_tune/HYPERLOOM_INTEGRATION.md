# Hyperloom Integration Guide for forge-gemm-tune

## Overview

`forge-gemm-tune` is designed as a standalone CLI that Hyperloom calls as a subprocess.
...
With forge-gemm-tune supporting bf16 MoE, this gate should be relaxed:
...
After forge-gemm-tune returns `micro_decision: "candidate"`:
...
Hyperloom Orchestrator
    ...
    v
forge-gemm-tune run --model-path ... --framework ... --output-dir ...
...
- forge-gemm-tune is preferred when available (check via `which forge-gemm-tune`)
