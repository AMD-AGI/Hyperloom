...
```bash
cd /wekafs/xiaofei/KernelForge/src/forge_gemm_tune
pip install -e .
```
...
forge-gemm-tune plan --model-path /wekafs/models/Qwen3-30B-A3B --framework sglang --precision bf16
...
forge-gemm-tune run ... --global-timeout 7200

# Individual tuner cap (per-tuner, default 1h)
forge-gemm-tune run ... --timeout 1800 --global-timeout 7200
```
...
Key points:
- Hyperloom calls `forge-gemm-tune run` as a subprocess
