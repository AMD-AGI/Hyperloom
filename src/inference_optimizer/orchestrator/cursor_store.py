...
tables, this gives the cross-table atomicity that ADR-33 promises.
"""
...
class CursorState:
    ...
    def empty(cls, agent: str) -> "CursorState":
        return cls(agent=agent, last_processed_seq=0,
                   last_processed_msg_id="", processed_at=_now_iso())
...
class CursorStore:
    ...
    async def advance(
        ...
        *,
        ...
        async with self.db.transaction() as cur:
            ...
            if seq <= current:
                # cursor never goes backwards — safety net
                return CursorState(
                    agent=agent,
    ...
    async def is_already_processed(self, agent: str, seq: int) -> bool:
        cursor = await self.load(agent)
        return seq <= cursor.last_processed_seq
