...
Topics + priorities are validated against an allowlist (DESIGN §10.2)
before insert.
"""
...
class MessageBus:
    ...
    async def replay_for(self, to_agent: str, *, after_seq: int) -> list[Message]:
        """Used at resume — return events in monotonic seq order."""
    ...
    async def append_batch(self, messages: Iterable[Message]) -> list[int]:
        """Bulk-insert as a single transaction. Useful in tests."""
        seqs: list[int] = []
        async with self.db.transaction() as cur:
            for msg in messages:
                if msg.topic not in TOPIC_ALLOWLIST:
