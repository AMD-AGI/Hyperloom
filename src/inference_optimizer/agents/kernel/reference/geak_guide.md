# GEAK Guide (kernel agent reference)

> Migrated from `src/inference_optimizer/kernel_opt/geak.md` as part of
> Plan A (kernel agent ownership). The kernel agent reads this on
> demand from `actions/run_optimization.md` (and `select_kernels.md`
> for the kernel-classification table). The original SKILL.md
> references in this file refer to the **executor's** SKILL.md +
> action descriptions; for the **kernel agent's** workflow see
> `agents/kernel/SKILL.md` and `actions/`.
