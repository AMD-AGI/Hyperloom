...
- [x] 8a.1 `paths.py::asset_root() / asset_scripts_dir() / asset_actions_dir() / asset_kernel_opt_dir()` —— 统一 package-local 资产路径解析（`INFERENCE_OPTIMIZER_ASSET_ROOT` env override）
...
- [x] 8a.10 `cli.py` 默认 `_build_action_registry()` 从 `asset_actions_dir()` 加载；`--no-action-registry` / `--actions-dir` 可选关闭/覆盖；启动 banner 显示 `actions: N loaded`
...
- [x] 8a.14 删除旧 stub，真版本随 `src/inference_optimizer/scripts/` package assets 提供
...
- [x] 8a.16 `tests/test_scripts_skeletons.py` 重写 —— 验证 package 脚本存在 + executable + 缺 env 时 `set -u` 拒跑
- [x] 8a.17 SKILL.md 作为薄入口，runtime assets 收敛到 `src/inference_optimizer/`
...
> 文件位于 `src/inference_optimizer/orchestrator/system_prompts/`。`AgentRole.system_prompt()`（带缓存）按角色名加载。
