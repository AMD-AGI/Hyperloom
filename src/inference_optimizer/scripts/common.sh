...
# --- InferenceX benchmark_lib.sh defaults (set -u guards) ---
# benchmark_lib.sh references several env vars without ${VAR:-default}
# defaults of its own. Our run_baseline / run_sweep / run_profile wrappers
# all use ``set -u``, so an unbound var bombs the entire script before any
# real work happens. Pin safe defaults here once for every consumer.
export EVAL_ONLY="${EVAL_ONLY:-false}"
export DP_ATTENTION="${DP_ATTENTION:-false}"
export SPEC_DECODING="${SPEC_DECODING:-false}"
export RESULT_FILENAME="${RESULT_FILENAME:-}"
export EVAL_RESULT_DIR="${EVAL_RESULT_DIR:-}"
