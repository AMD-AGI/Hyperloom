...
# Profiling is controlled by the explicit /start_profile and /stop_profile
# calls in this wrapper. Do not also pass benchmark_serving.py --profile,
# or SGLang sees a nested start_profile and can hang while stopping traces.
unset PROFILE
...
TRACE_COUNT=0
TP=${TP:-1}
# NB: ``set -o pipefail`` + ``ls X 2>/dev/null | wc -l || echo 0`` produces
# ``"0\n0"`` (wc outputs 0 on empty input, then ``|| echo 0`` fires too because
# the failed ls makes the pipeline exit non-zero). Guard the failing leg with
# a subshell + ``|| true`` so the pipe never trips pipefail.
for i in $(seq 1 60); do
    TRACE_COUNT=$( ( ls "$TRACE_DIR"/*.json.gz 2>/dev/null || true ) | wc -l )
    TRACE_COUNT=${TRACE_COUNT:-0}
    CUR_SIZE=$( ( du -sb "$TRACE_DIR" 2>/dev/null || true ) | awk '{print $1+0}' )
    CUR_SIZE=${CUR_SIZE:-0}
