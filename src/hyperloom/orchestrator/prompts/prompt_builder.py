...
def _section_decision_framework(*, kernel_enabled: bool, phase: str = "", transport: str = "") -> list[str]:
    ...
    lines.extend(
        [
            ...
            "   ``pct`` share. In EXPLORE a ``force_exit`` line adds",
            "   ``phase_remaining_pct``; as it falls toward ``pct_thresh``,",
            "   prefer lower-cost / known-good actions (explore over kernel_opt).",
            "   The Plateau advisory block is informational only for KERNEL;",
            "   a detected FRAMEWORK_AGENT or EXPLORE plateau, by contrast,",
            "   deterministically advances the phase. EXPLORE advances",
            "   to KERNEL_AGENT (``reason=explore_no_more_leverage``) and",
            "   FRAMEWORK_AGENT to EXPLORE (``reason=framework_agent_plateau``)",
            "   at the next phase-compute. When you judge the current phase exhausted,",
