...
def _append_composite_perf_section(lines: list[str], summary: dict[str, Any]) -> None:
    """Render composite perf axes when baseline perf data is available."""
    from hyperloom.common.perf_metric import composite_metric_enabled, composite_score, perf_snapshot_from_mapping
    ...
    lines.append("## Composite perf (input / intvty p90 / output)")
    lines.append("")
    lines.append(
        f"- baseline input tput : `{baseline['input_throughput']:.1f}` tok/s"
    )
    lines.append(
        f"- baseline output tput: `{baseline['output_throughput']:.1f}` tok/s"
    )
    lines.append(
        f"- baseline intvty p90 : `{baseline['intvty_p90']:.1f}` tok/s/user"
    )
    if cb_snap:
        lines.append(
            f"- current_best input  : `{cb_snap['input_throughput']:.1f}` tok/s"
        )
        lines.append(
            f"- current_best output : `{cb_snap['output_throughput']:.1f}` tok/s"
        )
        lines.append(
            f"- current_best intvty : `{cb_snap['intvty_p90']:.1f}` tok/s/user"
        )
        score = composite_score(cb_snap, baseline)
        lines.append(f"- current_best score  : `{score:.4f}`")
    if composite_metric_enabled():
        lines.append("- grading mode        : `composite_v1` (flag on)")
    else:
        lines.append("- grading mode        : `output_throughput` (composite flag off)")
