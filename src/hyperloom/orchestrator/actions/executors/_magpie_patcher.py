...
"""Compatibility patches for the pinned Magpie/InferenceX benchmark scripts.

Two independent gaps between the pinned Magpie/InferenceX revision and what
Hyperloom needs, applied at install time (``install.sh``) and re-asserted
defensively right before a benchmark launches:

- SGLang custom-tokenizer trust gating for ``MAGPIE_TRUST_REMOTE_CODE=1``
  (Magpie's client call sites never forward the ``trust`` flag upstream).
- The redundant ``--concurrent-requests`` flag InferenceX's ``run_lm_eval``
  rejects: stripped from every executed script, with the parser also taught
  to tolerate it as a defence-in-depth belt for the run-time re-copy.
...
class MagpiePatchStatus:
    ...
    def ok(self) -> bool:
        ...
        Returns:
            ``True`` only when the remote-trust and eval-flag-strip checks
            both succeeded.
        """
...
def magpie_scripts_patch_status(
    ...
    """Apply and report the SGLang trust and eval-concurrency compatibility patches.
    ...
    Returns:
        A ``MagpiePatchStatus`` carrying the remote-trust and eval-flag outcomes.
    """
    with _file_lock(_LOCK_PATH):
