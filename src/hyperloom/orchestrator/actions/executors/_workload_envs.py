...
# gfx942 / CDNA3 dies (MI300X, MI308X, MI325X) that ship the aiter CK gemm_a8w8_bpreshuffle kernel.
_GFX942_GPU_TYPES = frozenset({"mi300x", "mi308x", "mi325x"})


# Value is optional so a bare, value-less flag (an operator typo, or a flag left dangling at end-of-string) is
# stripped too rather than surviving into a retry that is supposed to launch without it.
_MOE_RUNNER_BACKEND_RE = re.compile(r"(?:^|\s)--moe-runner-backend(?:(?:[=\s]+)(?!--)\S+)?")

# Profile-phase capture defaults.
_DEFAULT_PROFILE_MAX_STEPS = 128
# AgentX counterpart of the cap above.
_AGENTX_PROFILE_MAX_ITERS = 8
# Default profile OSL ceiling when --profile-osl / PROFILE_OSL is unset: the profile reuses min(served OSL, this) so
# its trace stays light.
_PROFILE_DEFAULT_OSL = 1024
# SGLang graph-capture profiling flag shipped by the profile YAML, and the eager flag that makes it a no-op.
_SGLANG_PROFILE_CUDA_GRAPH_FLAG = "--enable-profile-cuda-graph"
_SGLANG_DISABLE_CUDA_GRAPH_FLAG = "--disable-cuda-graph"
# ``HYPERLOOM_PROFILE_DEGRADED_REASON`` value meaning the TraceLens runtime patch was attempted and did not apply.
_TRACELENS_PATCH_UNAVAILABLE = "tracelens_runtime_patch_unavailable"
_AGENTX_TRUE_VALUES = frozenset({"1", "true", "yes", "on"})

# Quality-reference env names, in resolution order.
...
def apply_agentx_switch(
    ...
    *,
    ...
    # The Magpie benchmark config's flat wall-clock cap (``benchmark.timeout_seconds``, e.g. 7200s from
    # baseline_vllm.yaml) is one deadline over server boot + warmup + the measurement window + result export.
    ...
    # WEKA_LOADER_OVERRIDE is upstream's own per-recipe corpus pin, so it has no AGENTX_ prefix and would not survive
    # the loop below. aiperf_client.sh documents it as a supported knob; without forwarding it only works when the
    # benchmark process happens to inherit the full parent environment, which is exactly the kind of silent difference
    # this path exists to remove.
    for key, value in os.environ.items():
        if key.startswith("AGENTX_") or key in ("AIPERF_BIN", "WEKA_LOADER_OVERRIDE"):
            envs[key] = value
    # ...but AGENTX_WARMUP_GRACE_PERIOD must not be forwarded raw.
...
def _apply_custom_runtime_defaults(
    ...
    *,
    ...
    # A variant's own overrides are applied after this hook, so they still win for keys the operator did not pin;
    # setdefault only fills the gaps.
    ...
    # Also under the framework-agnostic name, and in the config rather than only in os.environ.
...
def _visible_gpu_count() -> int:
    ...
    if shutil.which("rocm-smi"):
        ...
        if proc.returncode == 0:
            # ``rocm-smi --showid`` emits multiple ``GPU[N]`` lines per device; dedup by index.
            indices: set[str] = set()
...
# ``$FRAMEWORK`` (lowercased) -> shipped Magpie YAML, relative to ``asset_root()``.
...
def _finalize_framework_server_args(
    ...
    *,
    ...
    # 5. vLLM/atom argparse dedup: collapse repeated single-value flags to last-wins (vLLM crashes EngineCoreProc on a
    # duplicate); no-op for sglang.
...
def materialize_config_with_envs(
    ...
    *,
    ...
    if effective_inferencex_path:
        # Persist the resolved InferenceX checkout so Magpie's runtime checkout matches Hyperloom's patch target.
    ...
    # RANDOM_RANGE_RATIO is a float feeding the steady-state formulas below; do NOT coerce to int.
    ...
    # Auto-clamp TP to the visible GPU count.
    ...
    # Last-resort fallbacks kept in sync with the CLI workload defaults (parser.DEFAULT_ISL/OSL/CONC); normally the
    # CLI has already projected the resolved values into these envs before materialization.
    isl_val = int(envs.get("ISL") or 1024)
    osl_val = int(envs.get("OSL") or 1024)
    conc_val = int(envs.get("CONC") or 64)

    # Steady-state window for profiling configs (detected by YAML ``benchmark.envs.PROFILE`` or
    # ``profiler.torch_profiler.enabled``, not the process env).
    is_profile = str(envs.get("PROFILE", "")).strip() == "1" or (
        bench.get("profiler", {}).get("torch_profiler", {}).get("enabled") is True
    )
    # Scriptable image frameworks (xDiT diffusion) have no LLM decode steady-state window; the OSL/steady-floor math
    # below is a serving concept xDiT never consumes, so skip it.
    ...
    # ``(sentinel, flag)`` pairs remembered so the re-assertion at the very end of this function can restore exactly
    # the profiler flags that some later step dropped, without re-stating the ones that survived.
    pending_vllm_profiler_flags: list[tuple[str, str]] = []
    # The serialization-safe capture cap computed below, so the re-assertion can reject a max_iterations that is above
    # it (or non-positive, which vLLM reads as no limit) instead of accepting the flag on its name alone.
    ...
    if is_profile and not _is_scriptable_profile:
        ...
        # Cap captured decode steps at a serialization-safe default so the torch-profiler trace can be written without
        # starving the engine RPC.
        ...
        # Steady-state floor: minimum captured decode steps for the splitter to isolate a steady-state window (mirrors
        # TraceLens find_steady_state_window).
        ...
        if delay_iters < 0:
            delay_iters = 0
        # The iteration-based delay assumes the client streams a predictable number of decode steps before steady
        # state.
        if agentx_enabled():
            delay_iters = 0
            # ...and the bound itself has to come down, because the cap above is sized in DECODE STEPS against the
            # synthetic OSL.
            if max_iters > _AGENTX_PROFILE_MAX_ITERS:
                if cap_explicit:
                    # The operator asked for this cap explicitly (e.g. to widen the steady-state window); silently
                    # overriding it with no trace of the original value would hide why a deliberate
        ...
        # Operator hard-override of captured steps (e.g. a small eager FlyDSL profile).
        _ovr = os.environ.get("HYPERLOOM_PROFILE_MAX_ITERS", "").strip()
        if _ovr.isdigit() and int(_ovr) > 0:
            max_iters = int(_ovr)
            # Raising the capture bound must not revive the delay the AgentX branch above zeroed; the two knobs are
            # documented together.
            if agentx_enabled():
            ...
            # The AgentX clamp above is a HOST RAM bound, and this override silently undoes it.
        ...
        # NUM_PROMPTS must let the engine reach ``delay_iters + max_iters`` decode steps before running out of prompts
        # (N prompts ≈ N * OSL / CONC iters; invert + 2x buffer).
        ...
        # atom's profiler is HTTP-driven via atom_mi*x.sh, so this layer sets no atom profiler envs and must NOT
        # inject --profiler-config.* flags.
        is_atom = "atom" in fw
        # TraceLens profiler flags exist only in patched vLLM / SGLang builds; try to patch, fall back to the safe set
        # on failure.
        ...
        if is_atom:
            # atom's profile window lives only in Magpie's atom_mi*x.sh (ATOM_PROFILE_OSL / ATOM_PROFILE_NUM_PROMPTS);
            # defer to Magpie.
            ...
            # vLLM's AsyncLLM-side profiler tracks no iterations and captures the whole start_profile..stop_profile
            # range, so it has to stay off wherever we bound the worker-side one.
            ...
            # Injected unconditionally so the computed cap WINS over anything the YAML pins, via the repeated-flag
            # last-wins that vLLM's argparse and this layer both rely on.
            profiler_args = " ".join(
                flag
            ...
            try:
                ...
                extra_body = {}
            # Always override start_step/num_steps (the template has CONC=8 placeholders).
            extra_body["start_step"] = delay_iters
            extra_body["num_steps"] = max_iters
            # shape_discovery balloons an eager+with_stack trace; allow disabling it via env for eager profiles.
            _shape_disc = os.environ.get(
                "HYPERLOOM_PROFILE_SHAPE_DISCOVERY",
                "1",
            ).strip().lower() not in {"0", "false", "no", "off"}
            # Gemma2 + shape-discovery crashes CUDA-graph capture, so disable shape-discovery for Gemma2.
            ...
            # Both capture options are annotation-only and need TraceLens server-side support to land: without it the
            # trace carries no ``kernel_shape_profiler`` events (trace-health check 5), so asking for them pays the
            # capture cost for data nothing downstream reads.
            ...
            # NOTE: this write happens before the per-task ``extra_envs`` merge, so an ``extra_envs`` entry for
            # PROFILE_EXTRA_BODY can still drop start_step/num_steps the way ``args_mode="replace"`` used to drop
            # vLLM's --profiler-config bounds.
            envs["PROFILE_EXTRA_BODY"] = _json.dumps(extra_body)
            if tracelens_patch_ok and _shape_disc:
                # TraceLens-patched SGLang exposes --enable-shape-discovery-for-cuda-graph-profile; unpatched SGLang
                # errors on it.
    ...
    if not _is_scriptable_profile:
        # NUM_PROMPTS / NUM_WARMUPS are serving-request concepts; xDiT drives its own iteration count, so leave them
        # untouched.
        if profile_num_prompts is not None:
            # Profile mode: force-override NUM_PROMPTS to reach the steady-state window.
    ...
    # ── reference-script base (lowest priority) ──────────────────────────── Seed the framework server-args env +
    # envs from a reference recipe below the YAML base and any per-task extra_server_args (reference flags leftmost,
    ...
    if server_args:
        # Merge into (not overwrite) the framework env so the profile path's graph-capture flags aren't dropped.
        from ._grid_runner import merge_server_args
        ...
        if replace_args:
            ...
            # For MoE backend exploration/tuning, the candidate value must replace the baseline's injected default
            # (usually triton) rather than relying on duplicate last-wins flags.
        ...
        if not replace_args and existing:
            ...
            envs[framework_env] = server_args
    # Magpie forwards only ``benchmark.envs``.
    ...
    # ── aiter tuned-config lookup logging ──────────────────────────────────── aiter logs a line for every tuned-GEMM
    # table lookup it MISSES unconditionally, but the corresponding HIT line only when this is set.
    ...
    framework_env = server_args_env_name(bench.get("framework"))
    # Final dedup after reference/server_args merges: collapse repeated vLLM/atom single-value flags to last-wins so
    # recipe/variant values override earlier ones (no-op for sglang).
    _final_args = str(envs.get(framework_env, "")).strip()
    if _final_args:
        envs[framework_env] = dedup_vllm_server_args(_final_args, bench.get("framework"))
    # An eager profile cannot capture a CUDA graph, so the profile YAML's ``--enable-profile-cuda-graph`` is dead
    # weight the server still sets up for.
    if framework_env == "EXTRA_SGLANG_ARGS":
    ...
    # ── Quality-reference wiring (scriptable / server-less workloads) ────── Magpie forwards only ``benchmark.envs``
    # to the wrapper subprocess, so re-inject the quality reference here (the single scriptable choke point) or the
    # shipped empty YAML default silently skips the gate.
    ...
    # ── xDiT-only wiring ─────────────────────────────────────────────────── These three are read by the xDiT runner
    # and by nothing else, so they are keyed on the framework rather than on scriptability: an operator-supplied
    # ``custom`` workload must not have its baseline altered by settings it never declared, least of all an attention
    # backend.
    if str(bench.get("framework") or "").strip().lower() == "xdit":
        # The xDiT runner resolves models via MODEL_REGISTRY keys, not filesystem paths.
        envs["XDIT_MODEL_ARG"] = os.environ.get("XDIT_MODEL_ARG", "").strip() or "name"
        # If set, the baked hyperloom_local_aliases map each registered name to a local snapshot dir rooted at
        # $XDIT_MODEL_ROOT/<slug>.
        _xdit_model_root = os.environ.get("XDIT_MODEL_ROOT", "").strip()
        if _xdit_model_root:
            envs["XDIT_MODEL_ROOT"] = _xdit_model_root
        # For the baseline only, force the operator-pinned backend (default 'aiter', the MI300X-verified path) so an
        # invalid agent override cannot poison the reference measurement.
        if establish_quality_ref:
            envs["XDIT_ATTENTION_BACKEND"] = os.environ.get("XDIT_ATTENTION_BACKEND", "").strip() or "aiter"
    # ── Per-model MI300X baseline work-arounds ───────────────────────── A handful of flagship models SIGABRT during
    # CUDA-graph capture on the sglang ROCm image because their DEFAULT fused kernels are buggy on gfx942.
    _model_basename = Path(str(model_path or os.environ.get("MODEL_PATH", ""))).name.lower()
    if "kimi-k2" in _model_basename:
        # Kimi K2.x at tp8 takes sglang's ROCm fused-decode-MLA path, whose RoPE kernel aborts during CUDA-graph
        # capture.
        envs.setdefault("SGLANG_ROCM_FUSED_DECODE_MLA", "0")
    if "mimo-v2" in _model_basename:
        # MiMo-V2.x's DEFAULT aiter attention backend SIGABRTs during CUDA-graph capture on gfx942.
        from ._grid_runner import merge_server_args
        ...
        # vLLM registers this checkpoint under MiMoV2FlashForCausalLM but the HF config declares MiMoV2ForCausalLM,
        # which the pod-local vLLM build rejects at boot.
    ...
    # Single choke point every benchmark path funnels through: the final server-arg guards, applied at the FINAL
    # framework env so any operator-pinned flag is honored and never doubled.
    ...
    # ── Client trust-remote-code (model-agnostic) ───────────────────────── The MI300X bench scripts always launch
    # the SERVER with --trust-remote-code, so a custom-tokenizer model's CLIENT must load the same remote code or
    # transformers raises mid-warmup.
    for _trust_key in (
    ...
    # Server-side trust-remote-code for custom-code models (Qwen3.6 MoE): the SERVER must also load remote code or it
    # refuses the arch at boot.
    ...
    # Persist eval task/limit into the YAML so they become part of the fingerprint-able contract and bypass_runner
    # reads them from a stable source.
    ...
    # PD (router-fronted): lm_eval's default token-id prompts are rejected by the sglang_router's /v1/completions
    # (HTTP 422, StringOrArray), collapsing the accuracy eval.
    ...
    if not _eval_tok_env:
        try:
            ...
            # Don't degrade silently: if this IS a PD run, failing to resolve the topology leaves
            # MAGPIE_EVAL_TOKENIZED_REQUESTS unset, lm_eval keeps sending token-id prompts, and the sglang_router 422s
            # every request -> accuracy reads 0 with no other clue pointing back here.
            log.warning(
    ...
    # KernelForge fp8 block-scale CK backend switch: SGLANG_FP8_BLOCKSCALE_CK_MAX_M only takes effect on a
    # KernelForge-patched sglang fp8_utils.py.
    _fw = str(bench.get("framework") or "").lower()
    ...
    # FlyDSL folds only same-directory helpers into its JIT cache key, so a patched helper one directory over is
    # served from a stale binary.
    if flydsl_source_dirs:
        _flydsl_dirs = flydsl_extra_source_dirs()
        if _flydsl_dirs:
            envs.setdefault(ENV_FLYDSL_EXTRA_SOURCE_DIRS, _flydsl_dirs)

    # sglang FP8 per-channel/per-token CK fast path: a dense FP8 checkpoint with per-channel weight + per-token
    # (dynamic) activation falls into the slow unfused _apply_fallback_scaled_mm in sglang's apply_fp8_linear unless
    # SGLANG_USE_AITER_FP8_PER_TOKEN=1 flips use_per_token_if_dynamic on and routes the GEMM to aiter's CK
    ...
    if pending_vllm_profiler_flags and framework_env == "EXTRA_VLLM_ARGS":
        # The profile path's iteration bounds have to be the LAST word on this env, because three separate steps above
        # can drop them: a candidate carrying ``args_mode="replace"`` (which writeback sets as soon as a KEEP needs
        # ``remove_args``) overwrites the whole flag string, ``extra_envs`` can override it outright, and
        # ``remove_args`` itself strips flags by name. vLLM reads a missing ``max_iterations`` as "profile until
        # stop_profile", and the worker then accumulates every profiler event in host RAM at ~60 MiB/s until the
        # cgroup OOM-killer takes it out mid-roofline.
        from ._grid_runner import merge_server_args
        ...
        if restored:
            ...
            # _finalize_framework_server_args already ran, so re-apply the sink-side guard it ends with rather than
            # shipping an unvalidated string.
