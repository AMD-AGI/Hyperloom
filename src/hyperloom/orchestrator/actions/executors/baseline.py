...
class BaselineExecutor:
            ...
            if result.get("status") == "succeeded":
                ...
                if defer_accuracy_until_after_measure:
                    ...
                    if keep_policy is not None:
                        ...
                        skipped_accuracy = {
                            "status": "skipped",
                            "reason": "intvty_regression" if graded.vetoed else "performance_keep_not_eligible",
    ...
    async def _run_single_benchmark(
        ...
        *,
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        # A reused output_dir may still hold a prior attempt's server.log, whose terminal init markers would
        # misclassify THIS attempt as ``server_init_dead``.
        ...
        # And the ready stamp beside it, for the same reason: a prior attempt's would make this attempt's boot look
        ...
        # When the server's engine/worker bootstrap dies, the root cause is in server.log, not Magpie's stdout/stderr
        # tail, and the liveness watchdog may have reaped the hung parent with ``SERVER_DEAD_RETURNCODE``.
        ...
        if not measurement.get("valid_measurement"):
            # cuda-graph capture failure wins over server_init_dead so the one-shot disable-cuda-graph retry is armed
            # even when both co-occur.
            if cuda_graph_capture_failed:
                error_class = "cuda_graph_capture_failed"
