...
# Deliberately shares no substring with the auto-stash tag: _find_hyperloom_auto_stash matches by message, and a
# quarantined merge must never be picked up and popped back.
...
# Two-sided band, in percent of the pre-patch base, that a switch-off parity leg must land inside.
...
def _parse_framework_switches(
    *,
    ...
    if not raw:
        return [], []
    # Env the benchmark already defines is reserved: a "switch" colliding with it would be toggled by unrelated
    # configuration rather than by the lever.
...
def _with_skipped_setup_reason(reason: str, setup_result: dict[str, Any]) -> str:
    ...
    # ``_run_setup_commands`` already stores the sanitised form, so for every production caller this is a no-op.
...
def _is_allowlisted_setup_command(cmd: str) -> bool:
    ...
    if not text:
        return False
    # Reject command substitution / backticks / newlines outright — these can smuggle an arbitrary payload regardless
    # of tokenization.
    if re.search(r"[`\n]|\$\(", text):
        return False
    # Guard against genuine shell chaining/redirection while allowing pip/pkg version specifiers that legitimately
    # contain ``>``/``<`` (e.g. ``transformers>=4.58``).
    scrubbed = text
    # Drop quoted segments (their contents cannot act as shell operators).
    scrubbed = re.sub(r"'[^']*'", " ", scrubbed)
    scrubbed = re.sub(r'"[^"]*"', " ", scrubbed)
    # Drop an unquoted pip-style version comparison only when it is attached to the package token and the version
    # starts with a digit (``pkg>=4.58``).
    ...
    # Match on the executable's basename, but ONLY for an absolute path under a system prefix.
...
def _run_setup_commands(commands: list[str], *, cwd: Path, log_dir: Path) -> dict[str, Any]:
    ...
    for cmd in commands:
        if not _is_allowlisted_setup_command(cmd):
            # Sanitised HERE, not at the reporting sites.
            safe_cmd = _sanitize_setup_command(cmd)
            skipped.append(safe_cmd)
            # Also carried into the round's ``reason`` by _with_skipped_setup_reason: a warning alone left the caller
            # with an outcome and no link to the cause, so the same proposal was re-authored and re-dropped until the
            # budget ran out.
...
def _resolve_framework_root(
    ...
    if has_patch_input:
        ...
        # The allowlist does not necessarily hold it: it discovers the unprefixed env var, while the session root also
        # answers to <FRAMEWORK>_REPO_PATH and <FRAMEWORK>_DIR.
...
def _git_quarantine_unmerged(framework_root: Path, paths: list[str]) -> tuple[bool, str]:
    ...
    if quit_cp is None or quit_cp.returncode != 0:
        # Nothing was banked that a later revert cannot reach, but leaving MERGE_HEAD standing would mislabel the next
        # KEEP, so refuse rather than proceed on a repo that is still mid-merge.
...
def _git_restore_to_head(framework_root: Path, paths: list[str] | None = None) -> tuple[bool, str]:
    ...
    # `checkout --force HEAD --` resolves an unmerged index, which plain `checkout -- .` refuses to touch.
    cp = _run_git_cp(
...
def _git_stash_if_dirty(framework_root: Path) -> tuple[str, str]:
    ...
    if not cp.stdout.strip():
        return "clean", ""
    # git refuses to stash while an unresolved merge stands, so every later candidate would abort here forever.
...
def _git_restore_stash_if_needed(
    ...
    if cp.returncode == 0:
        ...
        return ""
    # A pop that cannot merge leaves conflict markers in source.
...
def _resolve_artifact_target(rel_target: str) -> tuple[Path, str, Path] | None:
    ...
    if not roots:
        return None
    # An absolute target is accepted only when it resolves strictly inside an allowlisted framework root.
...
class IntegratePatchExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        # Both are set per round by _stage_apply and tell the revert the tree was written even when no patch landed in
        # ``applied``.
    ...
    async def __call__(self, ctx) -> dict[str, Any]:
        ...
        # Provision an attempt-scoped runtime AFTER the Critic gate (in _stage_resolve) and BEFORE any patch apply /
        # setup replay.
        ...
        # Localize a merged-PR / vendored closure into the source tree.
        ...
        # The apply and the gate both mutate the framework tree behind the operator's auto-stash, and both cross
        # awaits while it is on the stack -- the apply stage writes a KB record on each of its failure verdicts.
        try:
    ...
    async def _stage_resolve(
        ...
        # Upstream-PR mode has no specialist to look up and no specialist verdict to enforce: the Critic verdict on
        # this proposal is the gate.
        ...
        # Rebind from live current_best so a task queued before an Explore KEEP still measures against the real stack
        # top.
        ...
        # Critic-verdict gate — enforced BEFORE any side effect (setup replay, stash, patch/artifact apply, pod
        # fan-out).
    ...
    def _stage_resolve_upstream_pr(
        ...
        if shared_state is not None:
            # Same rebind the specialist lane does: a task queued before a KEEP landed must still measure against the
            # real stack top.
            inject_stack_base_params(params, shared_state, anchor=True, overwrite=True)

        # A queued or resume-dispatched task is not re-validated by PolicyGate, and this lane fetches a remote diff
        # into the live framework tree.
    ...
    async def _stage_provision_attempt_runtime(
        ...
        # Record the attempt venv root on the action so KEEP can persist it and resume/GC can find it.
    ...
    async def _stage_localize_source(
        ...
        # Allowlist gate: touched paths must resolve under the source-file allowlist or the attempt-local root only
    ...
    async def _stage_apply(
        ...
        # Prepend localized closure patches (applied first, before this round's patch) so the enablement composes on
        # top of the localization.
        ...
        if undeclared_gates:
            ...
            # Only the manifest feeds switch_env, so an enablement round may instead arm its gate through the
            # proposal.
        ...
        if switch_manifest and not patch_paths:
            # A manifest without a patch describes switches that gate code which was never delivered.
        ...
        if (
            ...
            # Forward grounding-drop details so framework.py can surface them in the next round's mandate.
        ...
        if patch_paths and framework_root is None:
            ...
            if explicit_framework_root:
                # A non-allowlisted root is refused on that ground alone; only an allowlisted one that simply lacks
                # the files is the patches' fault.
            ...
            # A set spanning two trees is exactly what fails root resolution here, so this is the result that has to
            # carry the reason onward.
        ...
        # The stash is on the stack and the tree is about to be mutated, so ``__call__``'s undo has to be able to see
        ...
        for patch in patch_paths:
            # ``vet_patches`` runs at authoring time inside the specialist runner, so a patch from anywhere else --
            # ``params.patches``, and every remotely-fetched ``upstream_pr`` diff -- arrives unvetted.
            try:
        ...
        # The tree-mutation values are already published above, as the tree took them; what is left is what only the
        # gate reads.
        self._publish_gate_state(
            ctx,
            output_root=output_root,
    ...
    async def _stage_gate(
        ...
        *,
        ...
        # Activate the provisioned attempt runtime by threading its YAML-layer override into params so both bench
        # wirings pick it up.
        ...
        # Bound both bench legs by the session wall-clock, as the sweep and explore arms already are: the declared cap
        # answers "how long before this counts as hung", not "how much budget is left", so without this a patch
        # benched near the end of a run could outlive the run itself.
    ...
    async def _gate_enablement(
        ...
        *,
        ...
        # eval-origin only: a score with no task/metric did not come from a real eval, so it cannot clear the gate.
        if (
            eval_origin
            and correctness_ok
        ...
        if not runs:
            ...
            return _with_stash_restore(
                ...
                stash_note,
                {
                    ...
                    # The round ran and the boot still did not come up.
        ...
        kept_result: dict[str, Any] = {
            ...
            # Captured from the variant this leg launched, so a revalidation replays the graded configuration rather
            # than a re-derived one.
            "enablement_effective_config": dict(bench_result.get("effective_config") or {}),
            **eval_provenance,
        }
        # Record the KEEP'd attempt runtime so it survives rearm and every later bench in this session re-activates
        # it.
        ...
        # Editable-refresh the localized closure + snapshot a manifest that survives rearm so the closure is recorded
        # and not re-fetched.
    ...
    def _finalize_localization_keep(
        ...
        *,
        ...
        # Editable-refresh so localized Python changes take effect in the attempt runtime (no-op for plain wheel trees
        # like atom).
        try:
            from ...framework.adapters import get_adapter
    ...
    async def _gate_perf(
        ...
        *,
        ...
        # ``new_tput`` is reported as ``output_throughput``; the KEEP gate is graded on whichever axis this session
        # uses, both sides from one resolver.
        new_tput = bench_result.get("output_throughput")
        ...
        if switch_manifest:
            # The parity leg is an additional full bench, so it needs the same session bound the first bench got.
            session_deadline_sec, variant_expected_sec = session_grid_bounds(shared_state)
            ...
            if not parity.get("ok"):
                # An unmeasurable parity leg reverts under its own verdict: the patch was never shown to break, so
                # neither the log line nor the KB lesson may say that it did.
        ...
        if not gate_pass:
            # Two-tier verdict for a framework-rewrite patch.
        ...
        # The commit is what makes a KEEP survive the next candidate's ``checkout --force HEAD -- . && clean -fd``;
        # nothing replays the source snapshot in-session.
        commit_failure: str = ""
        ...
        if framework_root is None or not _is_git_tree(framework_root):
            ...
            try:
                ...
                if not ok:
                    ...
                    # Only a real commit advances HEAD, and ``_git_commit_kept`` signals that with an empty note.
                    keep_committed = note == ""
        ...
        if commit_failure:
            ...
            return _with_stash_restore(
                ...
                stash_note,
                {
                    # Applied, benched, then rolled back: the same terminal shape every other post-apply rollback
                    # reports.
                    "status": "reverted",
        ...
        try:
            ...
            if framework_root is not None:
                ...
                # An artifact installed into a sibling tree is not addressable by a rel path under this root, so it
                # belongs to no snapshot here.
                inside_root = [
                    str(a["rel_target"])
                ...
                if snap:
                    ...
                    # Only harvest when a real commit landed.
                    if keep_committed:
        ...
        return _with_stash_restore(
            ...
            stash_note,
            {
                "status": "kept",
                "specialist_task_id": specialist_task_id,
                # Proposal ownership must survive delegated-result persistence so resume replay cannot replace it with
                ...
                "framework_root": str(framework_root or ""),
                "base_sha": source_base_sha,
                # The bundle cleared the gate, so its switches join the running configuration and are registered as
                # levers that are already on.
    ...
    async def _switch_off_parity(
        ...
        *,
        ...
        if not isinstance(parity_tput, (int, float)) or parity_tput <= 0:
            # No measurement is not evidence of a behavioural change.
            return {
                "ran": True,
                "ok": False,
                "inconclusive": True,
    ...
    async def _keep_inert_switches(
        ...
        *,
        ...
        return _with_stash_restore(
            ...
            {
                "status": "kept_inert",
                # True when the bundle moved the output with every switch on.
                "quality_unverified": accuracy_pass is False,
                "specialist_task_id": specialist_task_id,
                "patches_applied": [str(p) for p in applied],
                "patches_reverted": [],
                "artifacts_applied": applied_artifacts,
                # Empty on purpose: the code is present but dormant, so nothing may enter current_best.
                "config_changes_applied": {},
                "extra_server_args_applied": "",
                "extra_envs_applied": {},
                "output_throughput": new_tput,
    ...
    def _upstream_pr_kb_proposal(params: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        return {
            ...
            # Names the lever, matching ``lever_kind``.
    ...
    def _revert_patches(
        ...
        if not self._apply_attempted and not applied and not self._ip_base_artifact_replayed:
            return reverted
        # Restoring to HEAD is the revert, not a fallback for one.
                ...
                applied.append(
                    {
                        ...
                        "existed": existed,
                        "backup": backup_path,
                        # Re-install source for the next round's base replay and for the archived copy in
                        # enablement_setting.sh.
    ...
    async def _bench_patch(
        ...
        *,
        ...
        # Last rung of the precedence: without it a task with no params model_path and no MODEL_PATH in env resolves
        # to the empty string.
        ...
        # Eval-origin enablement needs a raw accuracy for its runnable gate, so RUN_EVAL=true must survive any variant
        # overlay.
        ...
        for name in unset_envs or []:
            ...
            if not key:
                continue
            # Removing it from the variant env too: ``unset_envs`` drops inherited values, but a key present in both
            # would otherwise be re-added here.
        ...
        # Ray-managed GPU execution: hold a serving lease (num_gpus=TP + serving_slot) for the whole run_grid so the
        # patch benchmark serializes against other serving on the whole-machine mutex instead of colliding with a
        # concurrently-running GPU specialist server on the same card (the observed ``reverted_smoke_fail`` root
        # cause).
        ...
        if results:
            ...
            bench = {
                ...
                # Launch evidence is the immutable proof of the server that produced this measurement.
        ...
        # lm-eval writes to ``$EVAL_RESULT_DIR`` under the grid slot, not inside the ``benchmark_*`` workspace.
    ...
    def _grade_accuracy(
        ...
        # Accept numeric strings in addition to int/float; non-numeric / missing values fall back to 0.0 (skip).
        try:
