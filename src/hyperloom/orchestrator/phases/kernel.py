...
class KernelPhase(PhaseHandler):
        ...
        for cand in candidates:
            ...
            if decision == "KEEP" and new_tput > running_tput and not apply_blockers:
                ...
                lifted = self._lift_to_current_best(
                    ...
                    entry_extra={
                        "tuned_file": (env.get(cand["env_var"]) or next(iter(env.values()), "")),
                        "gain_pct": gain_pct,
                        "backend": backend,
                        "source": "kernel_entry_auto",
                    },
    ...
    async def _settle_collective_integration(
        ...
        *,
        ...
        integ["integration_status"] = "complete" if integration_complete else "recovery_required"
        integ["integration_recovery_action"] = "" if integration_complete else "revert"
        return decision
    ...
    async def _integrate_collective(self, result: dict) -> None:
                ...
                integ.update(
                    {
                        ...
                        "integration_status": ("complete" if revert_complete else "recovery_required"),
                        "integration_recovery_action": ("" if revert_complete else "revert"),
                    }
                )
                decision = "REVERT"
        ...
        if decision == "KEEP":
            ...
            finalize_complete = _collective_recovery.patch_lifecycle_complete(finalize_result)
            integ["integration_status"] = "complete" if finalize_complete else "recovery_required"
            integ["integration_recovery_action"] = "" if finalize_complete else "finalize"
            self.shared_state.record_collective_integration(
                integ,
                self.session_dir,
                integration_id=integration_id,
