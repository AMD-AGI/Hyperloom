...
def kernel_work_pending(state: Any) -> bool:
    ...
    Short-circuits in order: an accepted-but-unintegrated KEEP answers first,
    ahead of the terminal probes, so neither of them can strand one; a terminal
    rewrite controller then answers False, because it owns operator selection
    and has stopped; a terminal GEAK phase answers on its own (True only while
    an ``ok`` result has an ``awaiting_rebench`` pending with a revalidation
    task, else False); then the ``untried_hot_reusable_kernels`` capability
    probe, whose failure is treated as 'not available'; then the kernel_opt
...
def exit_normal_kernel(
    ...
    *,
    ...
    """
    # ``kernel_work_pending`` answers for outstanding integrations before it
    # short-circuits on a terminal Controller, so asking it here keeps this exit
    # from stepping over an unintegrated KEEP. The idle streak below still
    # bounds the phase if that work can never be drained.
