...
# Every ``tasks`` row is dispatched and awaited by the Coordinator's orchestration loop, and the table carries no
# requester column, so a heartbeat can only ever attest to this one agent.
...
class SubAgentRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # Live SharedState threaded into each executor's ctx.extra so gain-computing executors can recover a baseline
        # anchor when params['base_tput'] is absent.
    ...
    async def run_task(
        ...
        *,
        ...
        try:
            ...
            # Running an action whose lanes nobody holds would run it unserialised, so a missing lease is a caller
            # bug, not a fallback.
            ...
            # Workspace prep is inside the terminal-writing block: an ENOSPC there is a task that failed, not a task
            # still running.
            try:
                ...
                # Stopped from outside -- shutdown, or a wall-clock budget that ran out while this was running.
                await self._write_terminal(
                    task.task_id,
                    "cancelled",
