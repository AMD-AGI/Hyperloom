...
class Coordinator(metaclass=_CoordinatorMeta):
    ...
    def recorder(self) -> ResultRecorder:
        ...
        The ``_record_*`` / ``_build_*`` / research-evidence methods live on
        :class:`ResultRecorder`; the methods remaining here are thin forwarding
        shims. Built lazily and cached.
        """
    ...
    # Methods extracted into collaborator objects are delegated back by name
    # here. Each value is the name of the property returning the owning
    # collaborator.
    _DELEGATED = {
        # router
    ...
    def _collaborator(self, attr: str, factory):
        """Lazily build + cache a collaborator object (uses ``__dict__``)."""
