...
# The GEAK e2e whole-pipeline optimizer's ``kernel_journey.json`` labels its
# kernel backend plainly ``geak`` — which is now the CANONICAL token for this
# optimizer (formerly ``geak_v4`` / perfskills). The legacy per-kernel backend
# uses the distinct ``geak_v3`` token, so the two provenances no longer collide
# in ``session_breakdown.json`` (versions map / trace) and the e2e journey is
# folded into the breakdown verbatim — no relabel is required.
...
class Coordinator(metaclass=_CoordinatorMeta):
    ...
    # GEAK e2e. Pure param-search (explore/sweep) is excluded.
