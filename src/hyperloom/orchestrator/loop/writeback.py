    ...
    async def _promote_to_shared_state(
        ...
        *,
        ...
        if task_kind == "baseline":
            ...
            promoted = False
            # A post-resume revalidation task confirms the existing cumulative
            # stack rather than adding a variant, so it never "promotes":
            # reconcile the watermark + clear ``resume_pending_revalidation``,
            # but only when the rebench produced a valid measurement.
