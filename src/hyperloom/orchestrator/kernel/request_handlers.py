...
# Explicit per-kernel fallback ladder used only when a user asks for legacy
# native backends. The default phase-level backend is the whole-pipeline GEAK
# delegate (``geak``), so this ladder is not used unless selected explicitly.
