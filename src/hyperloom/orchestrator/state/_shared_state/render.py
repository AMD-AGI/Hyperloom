...
class _RenderMixin:
    ...
    def _format_search_state(search: dict[str, Any] | None) -> str:
        """Multi-line render of a ``*_search`` dedup ledger; counts on the head line, body enumerates every ``tested`` entry (KEEPs first, no truncation) so a variant never reads as untried."""
        if not search:
            return "(none)"
        ...
        # Surfaced on the head line because the per-variant bodies are capped, which can hide a whole round reaped by
        # the overtime gate.
