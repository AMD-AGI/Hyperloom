...
def _warm_recipe_source(
    ...
    *,
    ...
    When the replayable champion is borrowed from a same-architecture sibling,
    the final applied config comes from the donor's path, so prefer the donor's
    ``best_config`` source when a separate donor (tier other than ``self``)
    supplied it. Otherwise attribute to the identity match's ``best_config``
    backend, then its first contributing source, then the dispatcher's remote type.
    ...
    """
    # A borrowed config is the final replayed config; attribute to the donor.
