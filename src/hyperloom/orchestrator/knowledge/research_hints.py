...
# ---------------------------------------------------------------------------
# Built-in advisory priors now live as HUMAN-EDITABLE MARKDOWN under
# ``advisory_kb/`` (generic/ + per-framework folders), loaded via
# ``advisory_kb.hints_from_markdown(framework)``. Routing is by folder: generic/
# reaches all frameworks, ``<framework>/`` reaches only that framework. ADVISORY
# ONLY: they render into the specialist "KB CONTEXT" section and seed advisory
# gaps[]; the Critic still gates every final answer, and nothing here can reject
# a config. Every entry carries a ``source`` so it survives ``_coerce_hint``.
# ---------------------------------------------------------------------------


def builtin_hints(framework: str = "") -> list[dict[str, Any]]:
    """Return the advisory priors for a framework from the markdown KB.

    Reads ``advisory_kb/generic/`` plus ``advisory_kb/<framework>/`` (the folder
    is the routing gate) and normalizes each entry through ``_coerce_hint``.
    ...
    Returns:
        The normalized advisory hint dicts (source-backed; advisory only).
    """
    from . import advisory_kb as _advisory_kb

    out: list[dict[str, Any]] = []
    for raw in _advisory_kb.hints_from_markdown(framework):
...
def ensure_builtin_hints_seeded(session_dir: Path, framework: str = "") -> int:
    """Merge the framework's advisory markdown priors into the session hints.
    ...
    artifacts already exist. Routing is by folder — ``generic/`` reaches every
    framework, ``<framework>/`` only that one.
    ...
    Returns:
        The number of advisory hints newly added this call.
    """
    existing = load_hints(session_dir)
    seen = {_hint_key(h) for h in existing}
    added = 0
    for h in builtin_hints(framework):
        key = _hint_key(h)
        if key not in seen:
            seen.add(key)
            existing.append(h)
...
def write_hints_skeleton(session_dir: Path, framework: str = "") -> None:
    """Ensure both hint artifacts exist AND the advisory priors are seeded (PRELUDE invariant; preserves prior hints).

    Args:
        session_dir: Session directory whose hint artifacts are ensured.
        framework: The run's framework, used to route which advisory-KB folders
            are seeded (``generic/`` + ``<framework>/``).
    """
    # Seed the human-editable advisory markdown priors (generic + this framework)
    # so they are present from PRELUDE even before the scout runs.
    # Order-independent: works whether or not the md artifact already exists.
    ensure_builtin_hints_seeded(session_dir, framework)
