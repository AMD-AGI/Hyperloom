...
- Which framework-specific seed grid the `explore` action falls
  ...
  `src/hyperloom/orchestrator/actions/executors/explore.py` seeds
  atom, xDiT and sglang; vllm and unknown frameworks continue to
  rely on the orchestration LLM emitting
  `provenance='default_grid'` variants and will fail with
  `error_class="empty_grid"` on a cold-start with no LLM input.
  sglang's seed is chosen from the checkpoint's declared
  quantisation, not from `--precision`, because a BF16 checkpoint
  served with `--quantization fp8` reports fp8 while behaving like
  BF16. Checkpoints declaring a non-fp8 quantisation (awq, gptq,
  mxfp4, quark) get no seed, and neither does an fp8 `--precision`
  whose checkpoint cannot be read, since that label alone cannot
...
atom has the broadest programmatic cold-start seed grid
(`_atom_default_grid`: `atom_level_{2,3}`, `atom_prefix_cache`,
`atom_kv_fp8` on FP8, model-class-gated `atom_ep` / `atom_dp_attn` /
`atom_mtp_{1,3}`, `atom_cudagraph_bracket`), against the one or two
variants sglang seeds — vllm fails `error_class="empty_grid"` on a cold
start with no LLM variants.
