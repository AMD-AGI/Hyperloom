...
| Latency SLA | `--max-latency-ms` | Pass any per-request latency ceiling the operator states ("must stay under 200 ms", "p50 latency can't regress past X"). Off by default. A **constraint, not a target**: it sits outside the `--target-*` group and combines with them. Enforced on every KEEP and fails closed — a candidate the benchmark did not time is refused — so omitting it does not merely lose a preference, it removes the SLA from the search and the session may KEEP a configuration that violates it. |
