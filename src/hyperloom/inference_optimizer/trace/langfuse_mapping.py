...
# Langfuse observation levels we emit.
LEVEL_DEFAULT = "DEFAULT"
LEVEL_ERROR = "ERROR"

# ``llm_trace.LLM_STATUS_OK``, re-declared rather than imported: this module is a pure projection layer that
# ``llm_trace`` reaches back into via the emitter, so importing it here would close an import cycle.
_STATUS_OK = "ok"
...
def decision_to_scores(decision_row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if predicted is not None:
        try:
            ...
            pass
    # Calibration signal: the proposal_scorer's pre-decision rating (mean across raters) as its own NUMERIC score.
