...
# ``ray`` is imported lazily inside ``_fanout_to_all_nodes`` so the --local
# (dynamo SSH) path runs on pods without ray installed.
...
def main() -> int:
    ...
    # --local: dynamo SSH backend patches only this pod (no ray).
    if args.local:
        r = _apply_on_pod(
