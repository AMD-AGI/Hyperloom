...
"""Estimate no-run Hyperloom uplift and token/runtime savings from Recipe KB.

Pulls per-session documents (or reads a local JSON list) and prints the
forecast ``min(H, P50(capture)*H)`` plus spend priors. Existing store records
usually lack ``value.roofline`` / ``value.token_usage`` / ``elapsed_minutes``;
the JSON ``coverage`` and ``limitations`` fields say so instead of inventing
savings.
...
::
    ...
    python -m hyperloom.inference_optimizer.tools.estimate_no_run \\
        --input prior_sessions.json --live-headroom-tok-s 18000
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Live remaining roof gap H (tok/s). Overrides P50 of prior remaining.",
    )
    parser.add_argument(
        "--live-baseline-tok-s",
        type=float,
        default=None,
        help="Live baseline throughput; used to express the forecast as gain %.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = estimate_from_sessions(
        documents,
        live_headroom_tok_s=args.live_headroom_tok_s,
        live_baseline_tok_s=args.live_baseline_tok_s,
