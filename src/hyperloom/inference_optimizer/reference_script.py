...
def render_reference_script(
    *,
    ...
    Args:
        ...
        artifacts: List of ``{archive_path, target}`` dicts describing
            whole-file installs to replay.  ``archive_path`` is relative to the
            script directory; ``target`` is the absolute install path.
    ...
    if artifacts:
        lines.append("")
        lines.append("# Whole-file artifact installs (generated by enablement; re-run to restore)")
        for art in artifacts:
            arc = str(art.get("archive_path") or "").strip()
            tgt = str(art.get("target") or "").strip()
            if arc and tgt:
                lines.append(
                    f"install -D {shlex.quote('$SCRIPT_DIR/' + arc)} {shlex.quote(tgt)}"
                )
