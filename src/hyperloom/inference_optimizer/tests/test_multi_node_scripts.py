...
@pytest.fixture(params=["infera", "rayjob"])
def pod_launch_env(request, monkeypatch, tmp_path):
    """Capture the actual child env after PID1/worker recovery, without launching."""
    script = "launch_infera_node.py" if request.param == "infera" else "launch_multinode.py"
    mod = _load_script_module(f"pod_env_{request.param}", script)
    captured = []
    monkeypatch.setattr(mod.os, "environ", {})

    def unexpected(*_args, **_kwargs):
        raise AssertionError("unexpected process, network, or GPU operation")

    monkeypatch.setattr(mod.subprocess, "run", unexpected)
    monkeypatch.setattr(mod.subprocess, "Popen", unexpected)
    monkeypatch.setattr(mod.os, "kill", unexpected)
    if request.param == "infera":

        def pid1_read(path):
            assert str(path).replace("\\", "/") == "/proc/1/environ"
            return b"\0".join(f"{key}={value}".encode() for key, value in harness.base.items())

        monkeypatch.setattr(mod.Path, "read_bytes", pid1_read)
        monkeypatch.setattr(mod, "_kill_prior", lambda *_args: None)
        monkeypatch.setattr(mod, "_kill_gpu_sampler", lambda *_args: None)
        monkeypatch.setattr(mod, "_start_gpu_sampler", unexpected)
        monkeypatch.setattr(mod, "_wait_health", unexpected)
        monkeypatch.setattr(mod, "_ray_start", unexpected)
        monkeypatch.setattr(mod, "_resolve_pod_ip", lambda env: env.get("POD_IP", "192.0.2.1"))
        monkeypatch.setattr(mod, "_detach_launch", lambda cmd, log, pid, env: captured.append(dict(env)) or 42)
        monkeypatch.setattr(
            sys,
            "argv",
            [
        ...
        monkeypatch.setattr(mod, "_probe_mec_firmware_lt_177", lambda: False)
        monkeypatch.setattr(
            mod, "_detach_framework_launch", lambda cmd, log, pid, env, rank: captured.append(dict(env)) or 42
        )

        def launch():
            return mod._spawn_remote(
                framework="sglang",
                model="/m",
                tp=8,
                nnodes=2,
    ...
    harness = types.SimpleNamespace(
        ...
        captured=captured,
        launch=launch,
        base={"PATH": "/opt/venv/bin:/usr/bin", "POD_IP": "192.0.2.1", "LWS_WORKER_INDEX": "0"},
    )

    def run(overrides=None, unset=(), *, control=None, wire=None):
        mod.os.environ.clear()
        mod.os.environ.update(harness.base)
        if wire is not None:
            mod.os.environ.update(wire)
        else:
            mod.os.environ.update(overrides or {})
            mod.os.environ[_LAUNCH_ENV_CONTROL] = (
                control if control is not None else json.dumps({"set": list(overrides or {}), "unset": list(unset)})
            )
        launch()
        return captured[-1]
...
def test_pod_launch_env_tuning_override_wins_recovery_and_defaults(pod_launch_env):
    pod_launch_env.base.update(SGLANG_USE_AITER="pid1", SGLANG_ROCM_FUSED_DECODE_MLA="0")
    env = pod_launch_env.run({"SGLANG_USE_AITER": "variant", "SGLANG_ROCM_FUSED_DECODE_MLA": "1"})
    assert env["SGLANG_USE_AITER"] == "variant"
    assert env["SGLANG_ROCM_FUSED_DECODE_MLA"] == "1"
    assert _LAUNCH_ENV_CONTROL not in env


def test_pod_launch_env_unset_survives_recovery_and_defaults(pod_launch_env):
    pod_launch_env.base.update(SGLANG_USE_AITER="pid1", SGLANG_AITER_MLA_PERSIST="1")
    env = pod_launch_env.run(unset=["SGLANG_USE_AITER", "SGLANG_AITER_MLA_PERSIST"])
    assert "SGLANG_USE_AITER" not in env
    assert "SGLANG_AITER_MLA_PERSIST" not in env
    assert _LAUNCH_ENV_CONTROL not in env


def test_pod_launch_env_set_wins_same_layer_unset(pod_launch_env):
    env = pod_launch_env.run({"SGLANG_USE_AITER": "explicit"}, unset=["SGLANG_USE_AITER"])
    assert env["SGLANG_USE_AITER"] == "explicit"
    assert _LAUNCH_ENV_CONTROL not in env


@pytest.mark.parametrize("key", ["LWS_LEADER_ADDRESS", "POD_IP", "KUBERNETES_SERVICE_HOST", "PATH", "PYTHONPATH"])
def test_pod_launch_env_rejects_protected_control_keys(pod_launch_env, key):
    pod_launch_env.base[key] = "platform"
    with pytest.raises(ValueError, match="launch env control"):
        pod_launch_env.run({key: "variant"}, unset=[key])
    assert pod_launch_env.captured == []


@pytest.mark.parametrize("payload", ["{bad", "[]", '{"set": {}, "unset": []}', '{"set": [], "unset": [1]}'])
def test_pod_launch_env_rejects_malformed_control(pod_launch_env, payload):
    with pytest.raises(ValueError, match="launch env control"):
        pod_launch_env.run(control=payload)
    assert pod_launch_env.captured == []


def test_pod_launch_env_keeps_platform_values_with_tuning(pod_launch_env):
    platform = {
        "LWS_LEADER_ADDRESS": "192.0.2.10",
        "POD_IP": "192.0.2.1",
        "KUBERNETES_SERVICE_HOST": "192.0.2.20",
        "PATH": "/opt/venv/bin:/usr/bin",
        "PYTHONPATH": "/platform/python",
        "VIRTUAL_ENV": "/opt/venv",
    }
    pod_launch_env.base.update(platform)
    env = pod_launch_env.run({"SGLANG_USE_AITER": "variant"})
    assert all(env[key] == value for key, value in platform.items())


def test_pod_launch_env_unset_beats_shape_profiler_and_buffer_defaults(pod_launch_env, tmp_path):
    tool = tmp_path / "TraceLens" / "TraceUtils" / "kernel_shape_tool"
    tool.mkdir(parents=True)
    pod_launch_env.base.update(
        TRACELENS_ROOT=str(tmp_path),
        HYPERLOOM_SGLANG_SHAPE_MODE="sitecustomize",
        HYPERLOOM_MN_PROFILE_TRACE_DIR=str(tmp_path / "traces"),
        SGLANG_TORCH_PROFILER_DIR="old",
    )
    env = pod_launch_env.run(unset=["TRACELENS_SHAPE_DISCOVERY", "SGLANG_TORCH_PROFILER_DIR", "PYTHONUNBUFFERED"])
    assert str(tool) in env["PYTHONPATH"]
    assert "TRACELENS_SHAPE_DISCOVERY" not in env
    assert "SGLANG_TORCH_PROFILER_DIR" not in env
    assert "PYTHONUNBUFFERED" not in env


def test_pod_launch_env_unset_shape_root_disables_activation(pod_launch_env, tmp_path):
    (tmp_path / "TraceLens" / "TraceUtils" / "kernel_shape_tool").mkdir(parents=True)
    pod_launch_env.base.update(TRACELENS_ROOT=str(tmp_path), HYPERLOOM_SGLANG_SHAPE_MODE="sitecustomize")
    env = pod_launch_env.run(unset=["TRACELENS_ROOT"])
    assert "TRACELENS_ROOT" not in env
    assert "kernel_shape_tool" not in env.get("PYTHONPATH", "")


def test_pod_launch_env_rejects_missing_set_value(pod_launch_env):
    with pytest.raises(ValueError, match="launch env control"):
        pod_launch_env.run(control='{"set":["MISSING_KEY"],"unset":[]}')
    assert pod_launch_env.captured == []


def test_pod_launch_env_controller_wire_uses_snapshot_and_blocks_forged_control(pod_launch_env, monkeypatch):
    ...
    forged = json.dumps({"set": ["PATH"], "unset": ["KUBERNETES_SERVICE_HOST"]})
    monkeypatch.setenv(_LAUNCH_ENV_CONTROL, forged)
    monkeypatch.setenv(
        "HYPERLOOM_MN_EXTRA_FWD_ENV",
        json.dumps(
            {
                "SGLANG_USE_AITER": "variant",
                "SECRET_TEST_VALUE": "private-token",
                _LAUNCH_ENV_CONTROL: forged,
                "LWS_LEADER_ADDRESS": "bad",
                "POD_IP": "bad",
                "KUBERNETES_SERVICE_HOST": "bad",
                "PATH": "/evil",
            }
        ),
    )
    monkeypatch.setenv("HYPERLOOM_MN_UNSET_FWD_ENV", '["SGLANG_AITER_MLA_PERSIST","POD_IP"]')
    if pod_launch_env.backend == "infera":
        snapshot = infera._collect_launch_env()
    else:
        snapshot = mncli._collect_ray_launch_env({}, log_dir="/test/logs")
    identity = snapshot.digest
    monkeypatch.setenv("HYPERLOOM_MN_EXTRA_FWD_ENV", '{"SGLANG_USE_AITER":"later"}')
    if pod_launch_env.backend == "infera":
        sent = []
        monkeypatch.setattr(mncli, "_read_pod_script", lambda name: "# test script\n")
        monkeypatch.setattr(
            mncli,
            "_infera_ssh_run_script",
            lambda *args, **kw: sent.append(kw["env"]) or types.SimpleNamespace(returncode=0, stdout="{}", stderr=""),
        )
        for name in ("_read_pod_script", "_infera_ssh_run_script"):
            monkeypatch.setattr(infera._mn_cli, name, getattr(mncli, name))
        infera._infera_fanout_launch(
            {"ssh_port": 2222},
            "--model /m",
            [{"podIP": "192.0.2.1"}],
            label="test",
        ...
        wire = mncli._forward_runtime_env(snapshot)["env_vars"]
    control = json.loads(wire[_LAUNCH_ENV_CONTROL])
    assert "private-token" not in wire[_LAUNCH_ENV_CONTROL]
    assert _LAUNCH_ENV_CONTROL not in control["set"]
    assert "PATH" not in wire and "POD_IP" not in wire
    pod_launch_env.base.update(SGLANG_USE_AITER="pid1", SGLANG_AITER_MLA_PERSIST="1")
    env = pod_launch_env.run(wire=wire)
    assert env["SGLANG_USE_AITER"] == "variant"
    assert env["SECRET_TEST_VALUE"] == "private-token"
    assert env["POD_IP"] == "192.0.2.1"
    assert "SGLANG_AITER_MLA_PERSIST" not in env
    assert _LAUNCH_ENV_CONTROL not in env
    assert snapshot.digest == identity


@pytest.mark.parametrize("pod_launch_env", ["infera"], indirect=True)
@pytest.mark.parametrize("key", ["AITER_REBUILD", "HYPERLOOM_MN_SERVER_LOG_DIR"])
def test_pod_launch_env_explicit_values_override_generated_controller_values(pod_launch_env, monkeypatch, key):
    from hyperloom.inference_optimizer.multi_node import cli as mncli
    from hyperloom.inference_optimizer.multi_node.commands import infera

    monkeypatch.setenv(key, "/old/logs" if key.endswith("LOG_DIR") else "1")
    monkeypatch.setenv("HYPERLOOM_MN_EXTRA_FWD_ENV", json.dumps({key: "explicit-value"}))
    snapshot = infera._collect_launch_env()
    env = pod_launch_env.run(wire=mncli._launch_env_transport(snapshot))
    assert env[key] == "explicit-value"


def test_pod_launch_env_standalone_protection_matches_controller():
    from hyperloom.common.env_safety import BLOCKED_UNTRUSTED_ENV_NAMES
    from hyperloom.inference_optimizer.multi_node import cli as mncli

    expected = BLOCKED_UNTRUSTED_ENV_NAMES | {"VIRTUAL_ENV", _LAUNCH_ENV_CONTROL}
    for script in ("launch_infera_node.py", "launch_multinode.py"):
        mod = _load_script_module(f"protect_{script}", script)
        assert mod._PROTECTED_LAUNCH_ENV == expected
        assert mod._PLATFORM_ENV_PREFIXES == mncli._PLATFORM_ENV_PREFIXES
        assert all(not mncli._is_launch_env_key_allowed(key) for key in expected)
