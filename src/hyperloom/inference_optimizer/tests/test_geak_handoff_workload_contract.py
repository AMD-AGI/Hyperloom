...
def _coord(tmp_path: Path, *, framework: str = "sglang", agentx: bool = True) -> Coordinator:
...
async def test_agentx_geak_output_is_only_a_proposal_proxy(
    ...
    monkeypatch.setenv("E2E_METRIC", "total")
    monkeypatch.setattr(
        "hyperloom.orchestrator.kernel.request_handlers._kernel_agent_tool_path",
    ...
    assert (handoff["e2e_metric"], captured_env["E2E_METRIC"]) == ("output", "output")
