...
def test_aborts_when_forge_is_not_importable(tmp_path: Path) -> None:
    # A partial install: the wheel is there but its forge tree is not.
...
def test_missing_codex_runtime_is_a_warning_only(tmp_path: Path) -> None:
    # It matters only to an OpenAI-only deployment; a claude deployment is fine without it, so this must never abort a
    # working install.
...
def test_static_probe_installs_nothing() -> None:
    # forge ships in this distribution; a pip install here would mean something upstream failed to install it, and
    # papering over that is how the old $FORGE_PATH-resolved side install went stale.
    body = _extract_fn("_check_kernelforge_ready")
...
def test_static_probe_is_called_from_both_install_paths() -> None:
    # Packaged-wheel and editable installs are separate branches of ensure_inference_optimizer(); the probe must guard
    # both.
...
def test_static_gemm_tune_probe_installs_nothing() -> None:
    ...
    # Code only: the comments deliberately name the resolver they replaced, and a guard that cannot tell an
    # explanation from an instruction is a guard that punishes writing the explanation down.
