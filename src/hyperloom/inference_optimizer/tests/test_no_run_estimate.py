...
from hyperloom.orchestrator.knowledge.remote_recipe.sanitize import sanitize_shared_knowledge
...
def _session(
    *,
    ...
    value = {
        ...
        "kernel": {},
        "patch_timeline": [],
        "do_not_repeat": skips if skips is not None else [{"kind": "kernel", "kernel_id": "k1"}],
        "roofline": {
            "baseline": {
                "theoretical_peak_tok_per_sec": peak,
                "achieved_tok_per_sec": baseline,
            },
            "optimized": {
                "theoretical_peak_tok_per_sec": peak,
                "achieved_tok_per_sec": optimized,
            },
        },
    }
    ...
    if minutes is not None:
        value["elapsed_minutes"] = minutes
    return {
        "canonical_id": "inference:m:mi355x:sglang:mt:a:v:bf16",
        "session_id": f"s-{gain}",
        "schema_version": 2,
        "knowledge": {
            "knowledge_schema_version": 1,
...
def test_project_session_capture_is_fraction_of_baseline_roof_gap() -> None:
    ...
    assert row["capture"] == 0.3
    assert row["remaining_headroom_tok_s"] == 70.0


def test_estimate_forecast_and_savings() -> None:
    docs = [
        _session(gain=20.0, peak=200.0, baseline=100.0, optimized=120.0, tokens=1000, minutes=60.0),
        _session(gain=40.0, peak=200.0, baseline=100.0, optimized=140.0, tokens=3000, minutes=120.0),
    ]
    report = estimate_from_sessions(docs, live_headroom_tok_s=80.0, live_baseline_tok_s=100.0)
    assert report["historical"]["p50_capture"] == pytest.approx(0.3)
    assert report["forecast"]["forecast_tok_s"] == pytest.approx(24.0)
    assert report["forecast"]["forecast_gain_pct"] == pytest.approx(24.0)
    assert report["savings_prior"]["p50_token_grand_total"] == 2000.0
    assert report["savings_prior"]["p50_elapsed_minutes"] == 90.0
    assert report["coverage"]["with_token_usage"] == 2
    assert report["limitations"] == []


def test_estimate_legacy_sessions_report_missing_spend() -> None:
    docs = [
        {
    ...
    report = estimate_from_sessions(docs)
    assert report["historical"]["p50_validated_e2e_gain_pct"] == 30.0
    assert report["forecast"]["forecast_tok_s"] is None
    assert report["coverage"]["with_token_usage"] == 0
    assert any("token_usage" in item for item in report["limitations"])
    assert any("elapsed_minutes" in item for item in report["limitations"])


def test_sanitize_keeps_token_usage_object() -> None:
    cleaned = sanitize_shared_knowledge(
        {
            "value": {
                "token_usage": {
                    "session_total": {"grand_total": 99, "calls": 2},
                    "api_token": "secret",
                },
    ...
    assert cleaned["value"]["token_usage"]["session_total"]["grand_total"] == 99
    assert "api_token" not in cleaned["value"]["token_usage"]
    assert cleaned["value"]["elapsed_minutes"] == 12.0
...
def test_extract_parallelism_reads_boolean_switches_and_moe_knobs() -> None:
    knobs = extract_parallelism("--ep-size 4 --enable-dp-attention --dp-size 8 --enable-dp-lm-head --moe-dense-tp-size 1")
    assert knobs == {
        "ep": "4",
        "dp": "8",
        "dp_attention": "on",
...
def test_estimate_flags_winners_only_visibility() -> None:
    ...
    assert any("do_not_repeat" in item for item in report["limitations"])
...
def test_credentials_prefer_flags_then_file_then_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _, token = estimate_no_run.resolve_credentials(
        _args(kb_store_token="flag-token", kb_store_token_file=token_file)
    )
...
def test_report_never_echoes_the_token(tmp_path: Path) -> None:
    src = tmp_path / "sessions.json"
    src.write_text(json.dumps([_session(gain=10.0, peak=200.0, baseline=100.0, optimized=110.0)]), encoding="utf-8")
    out = tmp_path / "report.json"
...
def test_estimate_cli_offline(tmp_path: Path) -> None:
    payload = [_session(gain=30.0, peak=200.0, baseline=100.0, optimized=130.0, tokens=500, minutes=45.0)]
    src = tmp_path / "sessions.json"
    ...
    assert rc == 0
    report = json.loads(out.read_text(encoding="utf-8"))
    assert report["forecast"]["forecast_tok_s"] == pytest.approx(21.0)
    assert report["savings_prior"]["p50_elapsed_minutes"] == 45.0
