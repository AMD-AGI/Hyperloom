...
def spec(isl: int = 1024, osl: int = 128, **kw) -> ServingSpec:
    return ServingSpec(framework="vllm", model_path="/models/gpt-oss-120b", isl=isl, osl=osl, **kw)


def test_inside_the_validated_box_says_nothing():
    notes = extrapolation_notes(spec(isl=1024, osl=128), replica_gpus=8, calibrated=True)
    assert notes == []


def test_context_past_the_validated_range_is_flagged():
    notes = extrapolation_notes(spec(isl=65536, osl=4096), replica_gpus=8, calibrated=True)
...
def test_the_boundary_itself_is_not_flagged():
    ...
    notes = extrapolation_notes(spec(isl=VALIDATED_CONTEXT_TOKENS, osl=0), replica_gpus=8, calibrated=True)
...
def test_context_counts_what_the_run_will_hold_not_just_the_prompt():
    ...
    notes = extrapolation_notes(spec(isl=512, osl=131072), replica_gpus=8, calibrated=True)
    assert any("context" in n for n in notes)
...
def test_notes_accumulate_rather_than_shadowing_each_other():
    notes = extrapolation_notes(spec(isl=131072, osl=1024), replica_gpus=32, calibrated=False)
    assert len(notes) == 3


@pytest.mark.parametrize("isl,osl", [(0, 0), (None, None)])
def test_missing_lengths_do_not_raise(isl, osl):
    assert extrapolation_notes(spec(isl=isl, osl=osl), replica_gpus=8, calibrated=True) == []
