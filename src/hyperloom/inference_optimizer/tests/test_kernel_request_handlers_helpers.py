...
def test_removed_per_kernel_ladder_stays_removed(monkeypatch) -> None:
    """The per-kernel dispatch machinery is gone; only the route question remains.

    ``_raw_kernel_backend_order`` joins the two names this already guarded: with
    Hyperloom no longer selecting operators there is nothing to order, and
    ``geak_selected`` answers the one question left -- which owner the phase has.
    """
    monkeypatch.delenv("KERNEL_OPT_BACKEND_ORDER", raising=False)
    assert not hasattr(krh, "_backend_order")
    assert not hasattr(krh, "_run_backend_ladder")
    assert not hasattr(krh, "_raw_kernel_backend_order")
