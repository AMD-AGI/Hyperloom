...
def test_post_prelude_target() -> None:
    assert ps._post_prelude_target(kernel_enabled=True) == ps.PHASE_FRAMEWORK_AGENT
    assert ps._post_prelude_target(kernel_enabled=True) == ps.PHASE_KERNEL_AGENT
    assert ps._post_prelude_target(kernel_enabled=False) == ps.PHASE_SWEEP
