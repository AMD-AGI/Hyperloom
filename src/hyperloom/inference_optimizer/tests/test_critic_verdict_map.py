...
def test_the_per_variant_shape_the_gate_teaches_carries_the_cited_rule(gate):
    """The gate's hint is where the per-variant entry shape is spelled out for the emitter, so it is where a variant learns it can name the rule its verdict rests on rather than leaving that to prose."""
    with pytest.raises(PolicyDenied) as exc:
...
async def test_a_held_reject_is_not_a_landing_permit(coord):
    """``advise`` is an integrate_patch permit, so mirroring the held verdict turned "the Critic rejected this patch" into "the Critic waved it through" -- over a formatting rule, and irreversibly."""
...
async def test_a_held_variant_mirrors_the_verdict_the_critic_wrote(coord):
    """The mirror is a landing permit for the specialist's patches, and the per-variant path reaches it the same way the single one does: what the Critic wrote is mirrored, whatever the hold made of it for this round."""
    coord.shared_state = _PatchVerdictSharedState()
    pending = PendingProposal(
...
async def test_a_reject_that_also_names_a_second_risk_is_not_held(coord):
    """The hold answers "the whole reject was this one rule"; a verdict that also refuses on its own merits keeps both halves of the sentence."""
    pending = PendingProposal(
...
async def test_a_reject_still_asking_for_evidence_is_not_held(coord):
    """An outstanding evidence request is a ground of its own: the Critic is not complaining about a field, it is saying it cannot judge yet."""
    pending = PendingProposal(
...
async def test_a_rule_the_critic_cleared_is_not_the_grounds_for_its_reject(coord):
    """The prose scan used to fire on any word-bounded mention, so a Critic that checked the advisory rule, found it clean, and refused for a real reason had that reason read as the formatting complaint -- and the proposal ran."""
    pending = PendingProposal(
...
def test_a_rule_a_verdict_enumerates_or_quotes_is_not_the_ground_it_rests_on(reasoning):
    """A model that walks the rule list, or quotes a rule to say it does not apply, writes the code in exactly the shapes a citation would take."""
...
def test_a_citation_opening_the_verdict_still_holds_it():
    """The shape the field verdict used stays readable: the code opens the prose and a colon introduces the finding."""
    entry = {
        "verdict": "reject",
        "reasoning": f"`{QUANTITATIVE_CLAIM_REASON_CODE}`: the payload carries predicted_gain_pct.",
    }
...
def test_a_citation_in_either_field_the_entry_states_its_grounds_in_is_read():
    """The two prose keys are the same speaker's grounds for the same verdict -- ``reasoning`` is how a single verdict spells them and ``rationale`` how a variant does -- and nothing establishes a priority between them."""
    entry = {
        "verdict": "reject",
...
def test_a_rule_named_in_a_remediation_note_is_not_a_citation():
    """``notes`` is where a model writes what to do next, including "this is not a <code> problem"; grounds are stated in ``reasoning``."""
    entry = {
        "verdict": "reject",
...
def test_a_code_the_colon_does_not_follow_is_not_a_citation(reasoning):
    """Nothing may sit between the code and the colon but a backtick and an ASCII gap, so an identifier the code merely starts is not a citation."""
    entry = {"verdict": "reject", "reasoning": reasoning}
...
def test_findings_stated_outside_the_shape_that_counts_them_are_not_one_ground(risks):
    """The hold is confined to a reject naming at most one risk, which means counting the ``risks`` list."""
    entry = {
        "verdict": "reject",
        "reasoning": f"{QUANTITATIVE_CLAIM_REASON_CODE}: the payload carries predicted_gain_pct.",
        "risks": risks,
...
def test_an_empty_findings_slot_states_nothing(findings):
    """A verdict serialised with its list slots padded out has stated what is in them, not how many there are -- the same reading ``serialize_verdict_advisory`` takes of the field set downstream."""
    entry = {
        "verdict": "reject",
        "failure_reason_code": QUANTITATIVE_CLAIM_REASON_CODE,
        **findings,
...
async def test_a_verdict_whose_findings_cannot_be_counted_still_decides_its_proposal(coord, findings):
    """A number where the schema puts a list used to raise ``TypeError`` out of the ground count."""
    pending = PendingProposal(
...
async def test_a_held_reject_never_lands_the_patch_it_rejected(coord):
    """The rules the hold enforces are about specialist proposal payloads, and ``advise`` means "dispatch may proceed": holding an ``integrate_patch`` reject would execute the patch the Critic refused, with the propose-time PolicyGate patch gate already behind it."""
    coord.shared_state = _PatchVerdictSharedState()
    pending = PendingProposal(
...
async def test_a_reject_of_a_proposal_the_rules_do_not_govern_stands(coord, action_name):
    """Every advisory rule is about a specialist-authored proposal payload, so a reject of anything else cannot rest on one however the verdict is worded."""
    pending = PendingProposal(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A code no rule declares as advisory — e.g. the safety hard guard, which critic.md keeps at ``reject``.
...
async def test_a_variant_citing_its_rule_in_the_key_the_entry_carries_is_held(coord):
    """A ``verdict_map`` entry is ``{verdict, rationale?}`` — the shape PolicyGate documents and every fixture uses."""
...
async def test_a_variant_reject_keeps_the_grounds_the_payload_states_for_it(coord):
    """A ``verdict_map`` entry is ``{verdict, rationale?, failure_reason_code?}`` -- it has no slot for ``required_evidence`` or ``risks``, which the schema puts on the payload."""
...
async def test_a_reject_code_the_payload_declares_outranks_a_variants_advisory_prose(coord):
    """Declared-code precedence has to reach the batch path too: a ``failure_reason_code`` naming a rule that asked for a reject is the Critic's own citation, and a variant naming an advisory rule in prose cannot soften it."""
...
async def test_the_one_risk_a_batch_states_is_not_the_rule_its_variants_cite(coord):
    """A single stated risk is allowed beside a citation because on the single path both come from one statement by one author."""
...
async def test_a_variant_resting_only_on_the_cited_rule_still_gives_up_its_reject(coord):
    """A finding is what holds a variant, not the batch's whole advisory context: remediation notes and evidence pointers say what to do next and where to look, so a variant resting on nothing but its cited rule still gives up its reject."""
...
async def test_the_verdicts_own_prose_does_not_supply_a_variants_citation(coord):
    """Grounds stated once for the whole review are inherited; a citation is not."""
...
async def test_the_batchs_declared_advisory_code_does_not_supply_a_variants_citation(coord):
    """The batch's citation is not the variant's either."""
...
async def test_a_batch_blocker_binds_every_variant_whatever_name_its_prose_carries(coord, rejected, risks):
    """A batch states its blockers once, in prose written for the whole set, and nothing asks the Critic to file one
    under a variant's key.
    """
...
async def test_a_batch_finding_is_not_disowned_by_a_sibling_named_in_a_neighbouring_field(coord):
    """A finding is a dict of several fields, and only one of them states what the finding *is*."""
...
async def test_evidence_the_batch_still_wants_holds_a_variant_resting_on_a_cited_rule(coord):
    """``required_evidence`` is the stronger of the two inherited findings -- one item is a ground where ``risks`` needs two -- and a batch states it in the same place, for variants whose own shape has no slot for it."""
...
async def test_a_variant_that_states_a_finding_of_its_own_still_answers_for_the_batchs(coord):
    """Filing a risk under a variant's key says that risk is about it; it does not say the blocker the batch filed under no key is about someone else."""
...
async def test_a_variant_stating_no_grounds_does_not_borrow_the_batchs_advisory_citation(coord):
    """An entry that states nothing but its verdict has cited no rule."""
...
async def test_a_batch_code_no_rule_declares_withholds_the_downgrade(coord):
    """A code the Critic filled in with something no rule defines is not a citation the hold can act on, and it is not permission to dispatch either: it withholds the downgrade, at the cost this reading accepts -- the set keeps the reject it would otherwise have given up."""
...
def test_an_entry_inherits_only_a_code_that_can_hold_its_reject(entry, payload, expected):
    """A finding the batch states is read where it is stated, as a hold on the whole set; moving one onto an entry would put it back in the count the ``<= 1`` allowance runs on, which is what identified the batch's ground with the entry's rule."""
...
def test_a_batch_that_states_nothing_readable_leaves_its_entry_on_its_own_grounds():
    """Both halves of the batch reading -- what an entry inherits and what holds it -- ask a payload what it states, so both have to answer for one that is not a payload at all."""
    entry = {
        "verdict": "reject",
        "rationale": f"{QUANTITATIVE_CLAIM_REASON_CODE}: carries a self-reported gain field.",
    }
...
def test_a_map_of_verdicts_none_of_which_decides_asks_for_review():
    """``redirect`` is a legal verdict with no place in the collapse order, so a map carrying only those decides nothing and must fall back to review."""
...
def test_specialist_prompt_renders_proposal_target_and_ceiling():
    ...
    # Section 8 states both numbers; Section 1 repeats them so the target is visible before the specialist starts
    # working, not only at exit time.
