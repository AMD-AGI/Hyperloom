...
ALLOWED_FILES: dict[str, str] = {
    # Canonical compat helper: the single reader of
    ...
    # hyperloom.inference_optimizer.compat, the transition shim for this helper,
    # has been removed -- all call sites now
    ...
    # Standalone kernel-agent shim: tools/ must
    ...
    "field names",
    "src/hyperloom/orchestrator/actions/executors/_grid_base.py": "GridVariant(extra_sglang_args=...) back-compat kwarg (tree-reform.MD P2.2: GridVariant extracted from _grid_runner.py to the _grid_base sibling)",
    "src/hyperloom/orchestrator/state/shared_state.py": "_migrate_legacy_extra_sglang_args_keys walker + state.json transform",
    "src/hyperloom/orchestrator/knowledge/research_hints.py": "priors-match scorer builds a token blob from variant fields and "
    ...
    "src/hyperloom/inference_optimizer/tests/test_payload_aliases.py": "compat helper test surface",
    "src/hyperloom/inference_optimizer/tests/test_back_compat_legacy_field_name.py": "back-compat regression tests",
    "src/hyperloom/inference_optimizer/tests/test_coordinator_kb_writes.py": "recipe write-back regression test asserts best_config / "
    ...
    "src/hyperloom/orchestrator/kernel/_kernel_decisions.py":
    ...
    # by design.
    "src/hyperloom/inference_optimizer/tests/test_kernel_request_handlers_units.py": "test_server_args_read_from_per_framework_env_key + "
    ...
    "src/hyperloom/orchestrator/actions/executors/profile.py": "watermark-refresh inheritance comments name the legacy "
    "extra_sglang_args channel for the downstream reader",
    "src/hyperloom/inference_optimizer/tests/test_profile_and_kernel_handlers.py": "profile/kernel handler tests exercise the legacy "
    ...
    "src/hyperloom/inference_optimizer/tests/test_extra_sglang_args_merge.py": "cumulative extra_sglang_args merge/dedupe back-compat tests",
