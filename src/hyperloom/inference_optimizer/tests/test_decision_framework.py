...
@pytest.mark.asyncio
async def test_run_optimization_handler_rejects_runtime_symbol_source_mismatch(
    session_dir,
    tmp_path,
):
    """A shared Python launcher cannot stand in for the observed device kernel."""
    from hyperloom.orchestrator.kernel.request_handlers import (
        run_optimization_handler,
    )

    wrapper = tmp_path / "attention.py"
    wrapper.write_text(
        "def unified_attention_with_output(*args):\n    return None\n",
        encoding="utf-8",
    )
    result = await run_optimization_handler(
        {
            "kernel_id": "k_attention",
            "candidate": {
                "kernel_id": "k_attention",
    ...
    assert result["status"] == "failed"
    assert result["error_class"] == "runtime_symbol_source_mismatch"
    assert result["device_kernel_name"] == "kernel_paged_attention_2d"


def test_native_guard_accepts_python_source_that_defines_runtime_symbol(
    tmp_path,
    monkeypatch,
):
    from hyperloom.orchestrator.kernel import request_handlers as krh

    source = tmp_path / "chunked_prefill_paged_decode.py"
    source.write_text(
        "def kernel_paged_attention_2d(*args):\n    return None\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(
        krh,
    ...
    assert krh._validate_reusable_native_kernel(payload) is None
