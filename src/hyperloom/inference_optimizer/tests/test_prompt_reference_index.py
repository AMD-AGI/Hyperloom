...
def test_failure_recovery_present_in_every_phase(refs_dir):
    ...
    failure_path = refs_dir / "failure_recovery.md"
...
def test_read_reference_granted_to_orchestration():
    ...
    tools = gate.allowed_tools_for_agent("orchestration")
    assert "read_reference" in tools


def test_read_reference_returns_doc(refs_dir):
    """Calling read_reference with a valid name returns the document text."""
    ...
    # Wire a live reader pointing at the real refs dir.
    from hyperloom.orchestrator.loop.conversation import ConversationCollaborator

    class _FakeCoord:
        pass

    collab = ConversationCollaborator(_FakeCoord())
    # Patch the refs dir path via the reader closure.
    ...
    provider = ContextProvider(shared_state=None, reference_reader=_reader)
    result = provider.read_reference(stem)
    assert result.strip(), f"read_reference returned empty text for {stem!r}"
    assert "when:" in result.splitlines()[0]


def test_read_reference_rejects_path_traversal(refs_dir):
    ...
    for bad in ("../orchestration", "/etc/passwd", ".hidden"):
        result = provider.read_reference(bad)
        assert "rejected" in result or "not found" in result or "invalid" in result, (
            f"traversal not caught for {bad!r}: got {result!r}"
        )


def test_read_reference_unknown_name_returns_available_list(refs_dir):
    """Unknown names return the list of valid names rather than an error crash."""
    def _reader(name: str) -> str:
        candidate = (refs_dir / name).with_suffix(".md").resolve()
        if candidate.parent != refs_dir.resolve():
            return f"(rejected: {name!r})"
    ...
    provider = ContextProvider(shared_state=None, reference_reader=_reader)
    result = provider.read_reference("nonexistent_reference_xyz")
...
def test_read_reference_not_wired_returns_marker():
    ...
    result = provider.read_reference("anything")
