...
def _isolate(monkeypatch):
    ...
    # Pin the two knobs that would otherwise depend on the host (GPU count and runtime patching), keeping the golden
    # output byte-stable.
...
def test_golden_profile_mode(monkeypatch, tmp_path):
    ...
    # PROFILE is read from benchmark.envs (YAML), not the process env: the loop that copies process env into envs does
    # not include PROFILE.
...
def test_unset_env_dropped_when_not_in_extra_envs(monkeypatch, tmp_path):
    _isolate(monkeypatch)
    # KEEP is only in the YAML base and named in unset_envs with no extra_envs entry, so it is popped and never
    # restored.
    src = _write(tmp_path / "cfg.yaml", {"framework": "sglang", "model": "/m", "envs": {"KEEP": "base"}})
    res = we.materialize_config_with_envs(
        src,
...
def test_an_ordinary_benchmark_is_not_given_the_flydsl_source_dirs(monkeypatch, tmp_path):
    # Widening FlyDSL's JIT cache key recompiles kernels, so a baseline, explore or sweep run that patched nothing
    # must not pay for it.
...
def test_the_run_that_applied_a_flydsl_patch_asks_for_the_source_dirs(monkeypatch, tmp_path):
    ...
    # A host with its own FlyDSL install contributes further roots, so the configured one being named is what matters,
    # not the whole list.
...
def test_an_explicitly_configured_value_is_never_clobbered(monkeypatch, tmp_path):
    # The injection used to be the last writer of all, so it overwrote both the YAML base and an extra_envs value the
    # caller had threaded in on purpose.
