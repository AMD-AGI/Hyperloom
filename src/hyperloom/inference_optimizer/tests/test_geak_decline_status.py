...
def test_absent_reason_is_a_plain_decline_not_an_overlay_verdict() -> None:
    # A missing reason must never be read as "the overlay was the problem": that would send a retryable candidate to a
    # terminal status.
...
def test_neither_status_collides_with_the_dispatch_failure_status() -> None:
    # The dispatch-failure path still writes ``rebench_unavailable``; a decline must never overwrite that live
    # diagnostic.
