...
def _action_family(action: str) -> str:
    ...
    if s == "explore":
        return "explore"
    # The label may carry a tier suffix, so match the base token.
    if s.split(":", 1)[0] == "replay_warm_recipe":
        return "replay_warm_recipe"
    if s == "framework":
        return "framework"
    # Deterministic FP8 tuner KEEPs, bucketed apart from generic ``kernel``.
    if s == "gemm_tuning":
        return "gemm_tuning"
    # Whole-pipeline KERNEL-phase optimizer, bucketed apart from ``kernel``.
    if s == "geak_e2e":
        return "geak"
    return "other"
