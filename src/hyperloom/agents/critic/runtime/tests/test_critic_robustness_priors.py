...
def test_discover_path_via_explicit_dir(tmp_path: Path, monkeypatch) -> None:
...
def test_discover_path_via_session_dir(tmp_path: Path, monkeypatch) -> None:
    sd = tmp_path / "sd"
...
def test_discover_returns_none_when_unset(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("CRITIC_ROBUSTNESS_FINDINGS_DIR", raising=False)
    monkeypatch.delenv("ROBUSTNESS_AGENT_SESSION_DIR", raising=False)
...
def test_prepare_review_injects_robustness_priors(reviewer, tmp_path: Path, monkeypatch):
...
def test_prepare_review_disables_via_env(reviewer, tmp_path: Path, monkeypatch):
    findings_dir = tmp_path / "sd" / "agents" / "robustness" / "findings"
    findings_path = findings_dir / "sess_a.jsonl"
    _seed_findings(
        findings_path,
