...
from hyperloom.agents.critic.runtime.kb_client import HTTPKBClient
...
def test_request_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hyperloom.agents.critic.runtime.kb_client.urllib.request.urlopen",
...
def test_request_recovers_after_one_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("hyperloom.agents.critic.runtime.kb_client.urllib.request.urlopen", flaky)
    assert client.upsert({"topic": "t"}) == {"ok": 1}
    assert calls["n"] == 2


def test_backoff_grows(monkeypatch) -> None:
    monkeypatch.setattr("hyperloom.agents.critic.runtime.kb_client.random.random", lambda: 0.5)
