...
def test_a_connect_landing_on_the_deadline_is_claimed_by_exactly_one_side(monkeypatch):
    """The interleaving a flag check cannot survive, made deterministic.

    ``_connect`` returns just as the caller's join expires. With the check and
    the set on separate threads, the runner reads "not abandoned", returns, and
    the caller then declares abandonment -- leaving the process connected with
    nobody responsible for it. One lock has to decide which side got it.
    """
    import threading
    import types
    ...
    def _init_landing_on_the_deadline(**_kw):
        # Return exactly as the caller's 0.2s join is expiring.
        threading.Event().wait(0.2)
        connected.set()

    fake_ray = types.SimpleNamespace(
        init=_init_landing_on_the_deadline,
    ...
    assert connected.wait(5), "the fake connect never ran"
    for _ in range(100):
        if not timed_out or shutdowns:
            break
        threading.Event().wait(0.05)

    # Exactly one of the two outcomes, never both and never neither: either the
    # caller was handed the session, or the runner gave it back.
    assert timed_out == bool(shutdowns), (
        f"timed_out={timed_out} shutdowns={shutdowns}: a connected process with no owner"
    )
