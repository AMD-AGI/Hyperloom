...
SWSPLAT-33382 / SWSPLAT-33392: the manifest is untrusted at revert time, so
every ``copy2`` source (``backup_path``) must stay under the apply-time backup
tree (the manifest's own directory). A tampered ``backup_path`` pointing at an
arbitrary host file must not be copied onto the restore target, while a
legitimate manifest reverts unchanged.
