...
This is the *sole* op->source resolver, with no static-mapping fallback. Instead
of trusting absolute paths captured once at build time, the finder locates a
kernel's source in the *currently installed* framework tree by its stable
identity:
...
* non-patchable kernels (CK template instantiations / prebuilt assembly) are
  detected from the symbol itself and reported as such.

Triton/TileLang ``.py`` kernels are resolved upstream directly from the trace's
``kernel_file`` (see :func:`_bypass_source_resolver.editable_trace_source`), so
the finder does not need any launcher-path hints.
...
# Symbol markers for kernels we cannot safely edit: CK (Composable Kernel)
# template instantiations have no single editable ``__global__`` source, and
# prebuilt assembly kernels ship as ``.co`` binaries. Detected from the symbol
# alone (no JSON), so the non-patchable gate needs no external metadata.
_CK_SYMBOL_MARKERS = ("ck_tile", "ck::", "ck_tile::")
...
def _non_patchable_kind(device_kernel_name: str) -> str:
    ...
    ``__global__`` source and prebuilt assembly kernels ship as binaries; both
    are detected from the (demangled) symbol so the gate needs no JSON. Returns
    ``"aiter_ck"`` for CK symbols, else ``""``.
    """
    raw = (device_kernel_name or "").strip()
    if not raw:
        return ""
    demangled = _cxxfilt_base(raw) if raw.startswith("_Z") else raw
    hay = (demangled or raw).lower()
    if any(marker in hay for marker in _CK_SYMBOL_MARKERS):
        return "aiter_ck"
    return ""
...
def resolve(
    ...
    *,
    ...
    1. symbol-derived non-patchable gate (CK templates / prebuilt assembly);
    ...
    # Cheap gate first (symbol-derived): CK template instantiations / prebuilt
    # assembly have no editable source, so bail with a clear reason.
    nonp_kind = _non_patchable_kind(device_kernel_name)
    if nonp_kind:
        return finish(
            ResolveResult("", None, "", False, "non_patchable", "symbol", 0.0, reason=nonp_kind)
        )
