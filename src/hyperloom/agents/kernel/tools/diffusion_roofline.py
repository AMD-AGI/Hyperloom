...
def assemble_report(
    ...
    *,
    ...
    Single source of truth for the report *shape* (totals + timeline +
    end-to-end efficiency + per-denoise-step split + optional analytic DiT
    ceiling), fed by ``build_report`` so ``diffusion_roofline.json`` has one
    stable shape.
