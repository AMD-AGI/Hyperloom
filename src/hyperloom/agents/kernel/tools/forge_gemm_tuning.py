...
def _build_cmd(args: dict[str, Any]) -> list[str]:
    ...
    # forge ranks the manifest above shapes_json as a dense-shape source. It
    # degrades safely: forge's ``_safe_is_file`` drops a path that is not there,
    # with a warning. There is deliberately no ``--demand`` port -- forge
    # reconstructs demand from ``--kernel-signature-log``, which Hyperloom
    # always resolves.
