...
# Per-class overrides for constants the config does not expose (diffusers pipeline defaults).
...
def resolve_geometry(model_dir: str | Path) -> DenoiserGeometry | None:
    ...
    if isinstance(ps, list) and ps:
        ...
        patch = int(ps)
    # FLUX / ERNIE / Sana declare patch_size=1 but pack 2x2; force patch=2 for the pixel->token count.
