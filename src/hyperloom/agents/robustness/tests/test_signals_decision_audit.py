...
from hyperloom.agents.robustness.signals.decision_audit import (
    DecisionAuditConfig,
...
def test_empty_patch_kept_silent_on_revert():
    ...
    out = evaluate_decision_audit_signals(_ctx(), SourceData(local_decision_audit=audit))
...
def test_decision_threshold_violated_fires_on_sub_threshold_keep():
    ...
    out = evaluate_decision_audit_signals(
        _ctx(),
        SourceData(local_decision_audit=audit),
    )
        ...
        "oob_attempts": [
            {
                "kernel_id": "gemm_a8w8",
                "backend": "oob_claude",
                "report_text": ("Hoisted loop-invariant. expected speedup ~1.1x. No measurement performed."),
