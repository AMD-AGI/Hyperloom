...
from hyperloom.agents.robustness.signals.external_deps import (
    ExternalDepsConfig,
...
def test_j2_wekafs_latency_medium_at_warn():
    data = SourceData(
        local_external_deps={
            "mounts": [
                {"env_name": "TRACELENS_ROOT", "path": "/x", "ok": True, "error": None, "latency_ms": 7000.0},
            ],
        }
    )
    out = evaluate_external_deps_signals(_ctx(), data)
...
def test_j2_wekafs_latency_high_at_critical():
    data = SourceData(
        local_external_deps={
            "mounts": [
                {"env_name": "TRACELENS_ROOT", "path": "/x", "ok": True, "error": None, "latency_ms": 20000.0},
            ],
        }
    )
    out = evaluate_external_deps_signals(_ctx(), data)
...
def test_j2_silent_when_mount_healthy():
    data = SourceData(
        local_external_deps={
            "mounts": [
                {"env_name": "TRACELENS_ROOT", "path": "/x", "ok": True, "error": None, "latency_ms": 12.0},
            ],
        }
    )
    out = evaluate_external_deps_signals(_ctx(), data)
...
def test_j2_custom_thresholds_apply():
    ...
    data = SourceData(
        local_external_deps={
            "mounts": [
                {"env_name": "TRACELENS_ROOT", "path": "/x", "ok": True, "error": None, "latency_ms": 150.0},
            ],
        }
    )
