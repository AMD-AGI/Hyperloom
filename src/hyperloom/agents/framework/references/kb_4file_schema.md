...
| `with_llm=True` | Lazy-imports `claude_agent_sdk`; raises `RuntimeError` with install hint when SDK is absent. Drives `sdk.query()` synchronously via `asyncio.run`. | `pip install '.[claude]'` + `$OPENAI_BASE_URL` + `$OPENAI_API_KEY` |
