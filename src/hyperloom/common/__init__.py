...
(``io`` / ``env`` / ``jsonio`` / ``timeutil`` / ``gain_math`` /
``paths`` / ``subprocess_bridge`` / ``llm/`` …). Constraint: this package may
only import the stdlib (plus ``httpx`` for the future ``llm`` submodule) and
must never import ``orchestrator`` /
