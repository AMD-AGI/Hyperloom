...
def test_the_shipped_reference_carries_a_bf16_roof_the_chip_can_reach():
    """The profiler halves bf16; gfx950 runs both MFMA paths at one rate.

    Shipping the raw figure would make every bf16 ceiling twice as loose as it
    should be, which reads as the kernel being twice as close to done as it is.
    """
    reference = load_reference(_MI355X)

    assert reference.peak_flops["bf16_mfma"] == pytest.approx(reference.peak_flops["fp16_mfma"])
    # The substitution is on the record, beside the figure it replaced.
