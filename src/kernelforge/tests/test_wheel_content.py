...
def built_wheel(tmp_path_factory):
    ...
    wheels = list(wheel_dir.glob("kernelforge-*.whl"))
    assert wheels, "wheel build succeeded but produced no kernelforge-*.whl"
    return wheels[-1]
...
def test_wheel_contains_package_modules(built_wheel):
    names = zipfile.ZipFile(built_wheel).namelist()
    assert any(n.startswith("kernelforge/") and n.endswith(".py") for n in names)
    assert any(n.startswith("kernelforge/fusion/") and n.endswith(".py") for n in names)


def test_wheel_contains_staged_analysis_profiling_resources(built_wheel):
    names = set(zipfile.ZipFile(built_wheel).namelist())
    root = "kernelforge/data/local_knowledge/common_methodology/profiling"
