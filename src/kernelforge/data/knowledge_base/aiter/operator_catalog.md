...
```python
from aiter.ops.attention import mha_fwd, fmha_v3_fwd
...
### Multi-Latent Attention (MLA, ASM)
```python
...
silu_and_mul(out, input)           # out = SiLU(first_half) * second_half
...
## Causal Convolution
```python
from aiter.ops.causal_conv1d import causal_conv1d_update
...
QuantType.No           # 0 — no quantization
QuantType.per_Tensor   # 1
QuantType.per_Token    # 2
QuantType.per_1x32     # 3 — MXFP4 block scaling
QuantType.per_1x128    # block-scaled
QuantType.per_256x128  # block-scaled
QuantType.per_1024x128 # block-scaled

from aiter.ops.moe_op import ActivationType
ActivationType.Silu       # 0
ActivationType.Gelu       # 1
ActivationType.Gelu_Tanh  # 2
```
