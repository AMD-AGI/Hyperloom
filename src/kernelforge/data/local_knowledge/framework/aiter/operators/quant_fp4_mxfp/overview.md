...
sources:
  ...
  - ROCm/aiter@b467ce342:aiter/ops/triton/quant/quant.py
...
- per 32-element block: `block_amax = max(|x_block|)`; **E8M0 scale** `s = f32_to_mx_e8m0_scale(block_amax /
...
The MFMA wants the E8M0 scales in a **shuffled** layout. aiter's `per_1x32_mx_quant_hip(..., shuffle=True)`
(the fp4-only `per_1x32_f4_quant_hip` is now a thin wrapper) allocates the scale padded to
`((m+255)//256*256, ((n+31)//32+7)//8*8)` and runs `e8m0_shuffle` (which now delegates to
`aiter.ops.shuffle.shuffle_scale`) so Ax/Bx match `v_mfma_scale_*`'s operand map. Wrong shuffle → silent
corruption (numerics.md).
...
| backend | status | card |
|---|---|---|
| aiter | 🟢 sota (HIP + triton MX quant, shuffle) | [aiter.md](aiter.md) |
| triton | 🟢 sota (`dynamic_mxfp4_quant`, dot_scaled prep) | triton |
| hip | 🟢 sota (`dynamic_per_group_scaled_quant_fp4`) | hip |
| ck | 🟢 sota (mxfp4/mxfp6 GEMM + cast) | ck |
...
- aiter MX quant (`per_1x32_f4_quant`, shuffle, e8m0): `ROCm/aiter@b467ce342:aiter/ops/quant.py`.
- Triton `dynamic_mxfp4_quant` (group 32 fixed): `ROCm/aiter@b467ce342:aiter/ops/triton/quant/quant.py`.
