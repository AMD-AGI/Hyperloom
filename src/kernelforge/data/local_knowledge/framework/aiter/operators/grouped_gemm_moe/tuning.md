...
## aiter selection
- The grouped MoE path is **NOT** dispatched through the dense-GEMM DB. It uses its own **`tuned_fmoe.csv`**;
  the runtime lookup key (`_INDEX_COLS` in `fused_moe.py`) is `(gfx, cu_num, token, model_dim, inter_dim,
  expert, topk, act_type, dtype, q_dtype_a, q_dtype_w, q_type, use_g1u1, doweight_stage1)` — a leading `gfx`
  (backfilled from `cu_num` so gfx950/gfx1250 at 256 CU don't collide), **not** the dense 9-tuple.
  `block_m`/`ksplit` are the tuner's chosen config columns (results), not key fields. It is tuned by the MoE
  2-stage tuner `csrc/ck_gemm_moe_2stages_codegen/gemm_moe_tune.py` (`--errRatio` default **0.5**), not the
  GEMM tuners. gfx1250 FlyDSL grouped MoE has a separate `tuned_grouped_fmoe.csv` (`AITER_CONFIG_GROUPED_FMOE`).
  See [fmoe.md](../../skills/optimize/aiter_levers/fmoe.md) and [configs_db.md](../../overall/configs_db.md)
  for the capture/tune/deploy recipe. The token bucket comes from `get_padded_M` (tier logic, `_PADDED_M_TIERS`).
