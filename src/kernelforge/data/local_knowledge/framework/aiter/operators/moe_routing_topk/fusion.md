...
| **shared-expert as synthetic routed experts** | shared MLP injected into the top-k slots so it dispatches with routed experts | `moe_fused_gate(n_share_experts_fusion=…)` reserves gate slots / Wide-EP / shared_expert_fusion | one fused dispatch for shared+routed; no separate Linear+add |
...
[aiter.md](aiter.md) · `backends/aiter/fmoe.md`.

## Sources
- aiter fused gate (`moe_fused_gate` gained an `n_share_experts_fusion` arg), unified `topk_gating`, `biased_grouped_topk`: `ROCm/aiter@b467ce342:aiter/ops/topk.py`, `csrc/kernels/topk_gating_kernels.cu`, `csrc/kernels/moe_fused_gate.cu`.
