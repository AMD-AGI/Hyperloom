...
one kernel (aiter `fused_qk_norm_rope_cache_quant_shuffle`, `rope_cached_*`) removes that. For decode especially,
...
## Sources
- aiter Triton rope (grid, BLOCK_S=32, num_warps=4, BLOCK_D): `aiter/ops/triton/rope/rope.py`.
- in-place / cached rope variants: `aiter/ops/rope.py`.
