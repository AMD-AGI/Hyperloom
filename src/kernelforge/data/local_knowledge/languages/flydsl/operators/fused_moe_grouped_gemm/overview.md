...
| backend | status |
|---|---|
| aiter | 🟢 sota (live `fused_moe`, DB-driven) |
| ck | 🟢 sota (stage-2 `moe_ck2stages_*`, block-scale) |
| hip | 🟢 (stage-1 hand-tuned asm `fmoe_stage1_*`; editable) |
| triton | 🟡 competitive (vLLM/sglang Triton fused-MoE; per-shape JSON tuned) |
| flydsl | 🟢 sota (fp4 a4w4 2-stage + fused act/requant, CDNA4) |
