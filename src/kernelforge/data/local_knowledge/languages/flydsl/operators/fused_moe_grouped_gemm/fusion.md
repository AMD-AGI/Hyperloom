...
## TL;DR
This op is already a mega-fusion (sort → gate+up GEMM → activation → down GEMM → weighted combine); the remaining levers are what else you fold into the two stages: g1u1 gate+up merge, SwiGLU act into stage-1 epilogue, routed-weight multiply, fp8 dequant, shared-expert. The un-fused frontier is combine→down-proj and avoiding the `[M,inter]` HBM round-trip between stages.
