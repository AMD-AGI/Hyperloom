...
## TL;DR
Same bandwidth-bound fusion economics as RMSNorm — fold neighbors to share the single read+write: residual-add+LN in one pass, and LN+fp8/int8 dynamic/smooth quant (aiter's `layernorm2d_fwd_with_*` family) so the downstream GEMM reads ½ the bytes; the residual+norm+quant triple fusion combines both.
