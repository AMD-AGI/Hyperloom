...
## TL;DR
Bandwidth-bound; tuning = reach the `(read x + write y)/BW` floor: single-pass when the row fits an LDS block (all real hidden dims), two-pass blocked only for exotic N; vectorized 128-bit loads + `num_warps` 2–4 (8 spills) + `.cg` cache modifier + fp32 accumulate. Decode is pure latency → the real win is to **fuse it away** (residual-add / quant), not tune it.
