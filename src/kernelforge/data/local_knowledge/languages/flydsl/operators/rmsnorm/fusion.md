...
## TL;DR
RMSNorm is a fusion anchor, not standalone on the serving path — because it's bandwidth-bound, every folded neighbor removes a full HBM round-trip: residual-add+rmsnorm (the dominant `fused_add_rmsnorm`), rmsnorm+fp8/int8 dynamic quant, QK-norm+RoPE+KV-write+quant at attention entry, and all-reduce+rmsnorm under TP.
