...
## TL;DR
Three sharp edges cause real MI300X serving bugs: (1) accumulate `Σx²` in **fp32** (bf16 loses ~3 mantissa bits at N≥4096), (2) **fp32-promote γ** before the multiply (a live vLLM regression class), (3) ε goes **inside the rsqrt on the mean** and reduction order varies across backends — re-gate greedy parity after any swap. The fp8 variant needs the fnuz dialect on gfx942.
