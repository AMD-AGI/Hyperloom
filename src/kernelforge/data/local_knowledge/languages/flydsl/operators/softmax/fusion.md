...
## TL;DR
Softmax is defined by its fusions — standalone it's negligible: online/flash softmax **inside FMHA** (the dominant form — optimize the attention kernel, not softmax), **softmax+topk** for MoE routing (`topk_softmax`), and softmax+sampling after the lm_head.
