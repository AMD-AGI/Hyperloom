...
## TL;DR
Memory-bound two-reduction (max, sum) row op — same bandwidth playbook as the norms plus the **online single-pass** trick to avoid a third pass; round the reduced dim to pow2 for a full wave64 reduce, `num_warps=8` for wide vocab / 2–4 for narrow routing, fp32 exp+accumulate. The real lever is not standalone: fuse into FMHA (attention) or softmax+topk (routing).
