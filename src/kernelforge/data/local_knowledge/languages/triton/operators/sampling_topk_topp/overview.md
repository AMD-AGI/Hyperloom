...
| backend | status | card |
|---|---|---|
| aiter | 🟢 sota (the live AMD path: HIP Dual-Pivot rejection sampler + greedy/Gumbel `module_sample`) | aiter |
| vllm_kernels | 🟢 sota (`forward_hip` → aiter; native sort fallback; Gumbel no-sync `random_sample`) | vllm_kernels |
| triton | 🟡 competitive (MRV2-style fused top-k/Gumbel Triton kernels; sort-free logprobs) | [triton.md](triton.md) |
