...
| backend | status | card |
|---|---|---|
| aiter | 🟢 sota (biased_grouped_topk HIP + DPP, moe_sorting) | aiter |
| hip | 🟢 sota (the actual kernels: `moe_align_block_size_kernels.cu`, `moe_fused_gate.cu`, `topk_softmax_kernels_group.cu`) | hip |
| triton | 🟡 competitive (vLLM/sglang fused-MoE Triton routing; editable fallback) | [triton.md](triton.md) |
| vllm_kernels | 🟢 (registers aiter routing ops + has its own Triton/CUDA `topk_softmax`/`grouped_topk`) | vllm_kernels |
