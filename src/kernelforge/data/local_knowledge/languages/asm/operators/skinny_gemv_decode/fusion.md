...
## Fusion neighbors
- **Bias + activation epilogue** after the split-K reduction → gemm_epilogue_fused/overview.md.
- **Upstream norm+quant**: rmsnorm→fp8 quant feeding the decode GEMM input →
  fused_norm_quant/overview.md.
- **fp8 dequant scale** in epilogue → scaled_quant_gemm/fusion.md.
- **Decode MoE**: tiny per-expert M makes the grouped GEMM behave like many skinny GEMMs →
  grouped_gemm_moe/overview.md.
- Core decomposition technique: splitk_streamk_gemm/fusion.md
