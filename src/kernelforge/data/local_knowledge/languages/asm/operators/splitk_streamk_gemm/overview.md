...
- **Small-M / skinny** (decode-ish, M small, N·K moderate): few output tiles → many idle CUs → split-K /
  stream-K recover utilization. See skinny_gemv_decode/overview.md.
...
- **Large M·N**: plenty of tiles already → split-K/stream-K usually unnecessary or harmful (extra
  reduction cost). Use plain dense GEMM (dense_gemm/overview.md).
...
| triton | 🟢 sota | triton |
| ck | 🟡 competitive | ck |
| hipblaslt | 🟡 competitive | hipblaslt |
| asm | 🟡 competitive | [asm.md](asm.md) |
| hip | 🟡 competitive | hip |
| flydsl | 🟡 competitive | flydsl |

## Fusion neighbors
- The reduction/fix-up epilogue can fold bias/activation; for fp8 it can fold dequant scale →
  scaled_quant_gemm/overview.md,
  gemm_epilogue_fused/overview.md.
