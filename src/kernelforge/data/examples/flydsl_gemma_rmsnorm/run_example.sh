#!/usr/bin/env bash
# Drive this forge-loop task (FlyDSL Gemma RMSNorm) end to end.
#
...
WORKSPACE="${1:-/tmp/forge_loop_flydsl_gemma_rmsnorm_$(date +%s)}"
...
mkdir -p "$WORKSPACE"
cp "$EXAMPLE_DIR/rmsnorm_kernel.py" "$WORKSPACE/"
...
kernelforge forge-loop \
  --kernel "$WORKSPACE/rmsnorm_kernel.py" \
  ...
  --target-functions "gemma_rmsnorm" \
  "${MODEL_ARGS[@]}"

echo "==> Done. Best kernel is checked out in: $WORKSPACE/rmsnorm_kernel.py"
