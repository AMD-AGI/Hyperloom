#!/usr/bin/env bash
# Drive this forge-loop task (Triton dynamic per-tensor FP8 quant) end to end.
#
...
#   * A GPU with torch + triton available, and fp8 (gfx950 for float8_e4m3fn)
...
WORKSPACE="${1:-/tmp/forge_loop_triton_fp8_quant_$(date +%s)}"
...
mkdir -p "$WORKSPACE"
cp "$EXAMPLE_DIR/quant_kernel.py" "$WORKSPACE/"
...
kernelforge forge-loop \
  --kernel "$WORKSPACE/quant_kernel.py" \
  ...
  --target-functions "dynamic_quant_fp8" \
  "${MODEL_ARGS[@]}"

echo "==> Done. Best kernel is checked out in: $WORKSPACE/quant_kernel.py"
