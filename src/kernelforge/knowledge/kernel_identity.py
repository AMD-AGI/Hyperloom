    ...
    def __post_init__(self) -> None:
        for name, value in asdict(self).items():
            if not isinstance(value, str) or not value.strip():
                raise ValueError(
                    f"KernelRecipeIdentity.{name} must be a non-empty string"
                )
        if self.producer not in KERNEL_RECIPE_PRODUCERS:
            supported = ", ".join(sorted(KERNEL_RECIPE_PRODUCERS))
            raise ValueError(
                "KernelRecipeIdentity.producer must be one of: "
                f"{supported}; got {self.producer!r}"
            )
...
def _validate_scheme(value: str) -> str:
    if (
        ...
        or not _SCHEME_RE.fullmatch(value)
...
def _validate_identity_segment(name: str, value: str) -> str:
    if (
        not isinstance(value, str)
        or not _IDENTITY_SEGMENT_RE.fullmatch(value)
...
def kernel_recipe_canonical_id(
    ...
    *,
    ...
    dimensions = [
        _validate_identity_segment(name, identity_values[name])
        for name in KERNEL_CANONICAL_DIMENSIONS
    ]
