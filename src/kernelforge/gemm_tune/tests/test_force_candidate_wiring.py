...
def _prep_and_mock(tmp_path, monkeypatch, splitk):
    # The serve-safe splitK cap trials the real a8w8_blockscale CK kernel when a GPU is present, which JIT-builds
    # aiter modules and turns this pure wiring test into a multi-minute (observed: hanging) build.
...
def _run_no_splitk(tmp_path):
    # Same driver, but no --splitK, so a forced candidate can only come from the new-shape path -- isolating the fix
    # from the split-K force_candidate.
    return ac.run_aiter_dense_tuner(
        tuner_name="a8w8",
        script_key="a8w8_blockscale",
        env_var="AITER_CONFIG_GEMM_A8W8_BLOCKSCALE",
...
def test_all_new_shapes_force_candidate(tmp_path, monkeypatch):
    # aiter reports every shape as NEW (no prior baseline). status=ok with unverified_shapes>0 (bf16-aligned);
    # candidate=True sends configs to E2E.
    _prep_and_mock(tmp_path, monkeypatch, splitk=0)
...
def test_candidate_csv_fallback_forces_candidate(tmp_path, monkeypatch):
    # aiter printed only a "Successfully tuned shapes" summary, so there is no per-shape Pre/Post table and every row
    # recovered from the candidate CSV is tuned_unverified.
    _prep_and_mock(tmp_path, monkeypatch, splitk=0)
...
def test_all_update_shapes_do_not_force_candidate(tmp_path, monkeypatch):
    # A normal comparison with real speedups is promoted through has_improvement, NOT the new-shape force path --
    # guards against over-forcing.
    _prep_and_mock(tmp_path, monkeypatch, splitk=0)
