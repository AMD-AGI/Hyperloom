...
def _resolve_backend(provider: str, model: str, timeout_sec: int):
    """Build the agent backend, following the same ladder forge-loop reads."""
...
def roofline_ceiling_command(
    ...
    algorithm constraints. It is advisory: nothing downstream may gate a KEEP on
    it.
    """
    ...
    destination = Path(output_dir).expanduser() if output_dir.strip() else workspace / WORKSPACE_SUBDIR
    ...
    click.echo(
        f"[ceiling] arch={evidence.hardware.arch} peaks={evidence.hardware.peak_source} "
        f"dispatch_floor={evidence.hardware.dispatch_floor_s * 1e6:.3g}us cases={len(scored_cases)}"
    )

    operator = op_name.strip() or workspace.name
    canonical_id = f"roofline-ceiling:{operator}:{evidence.hardware.arch or 'unknown'}"
    key = cache_key(
    ...
    if cache:
        ...
        if cached is not None:
            ...
            _emit(cached, source="cache", report_path=publish(cached, destination))
            return

    click.echo("[ceiling] running the analyst session...")
    try:
        report = asyncio.run(
            run_ceiling_analysis(
                _resolve_backend(agent_provider, agent_model, agent_timeout_sec),
                canonical_id=canonical_id,
    ...
    except CeilingAnalysisError as exc:
        raise click.ClickException(str(exc)) from exc

    report_path = publish(report, destination)
    if cache:
        store_in_cache(report, key)
    for case in report.cases:
        for issue in case.issues:
            click.echo(f"[ceiling] {case.case_id}: {issue}")
    _emit(report, source="analyst", report_path=report_path)


__all__ = ["RESULT_SENTINEL", "roofline_ceiling_command"]
