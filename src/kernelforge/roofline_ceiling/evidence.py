...
class EvidenceBundle:
    ...
    #: it holds rather than left to assume the one this module used to measure.
...
#: How far the measured fp16:bf16 matrix ratio may sit from 2.0 and still be
#: read as the halving artifact rather than a real difference. Observed at 1.991
#: on rocprofiler-compute 3.4.0; the window is wide enough for run-to-run noise
#: and far too narrow to swallow an architecture that genuinely runs the two
#: paths at different rates, which would show a ratio of 1 or 4.
_BF16_ARTIFACT_RATIO_TOLERANCE = 0.1


def correct_known_tool_artifacts(
    peaks: dict[str, float],
    arch: str,
) -> tuple[dict[str, float], dict[str, str]]:
    """Repair peaks a profiler release is known to misreport.

    Returns ``(peaks, correction_by_path)``. A roof that is wrong low is not a
    conservative error: the ceiling derived from it is too loose, so the kernel
    reads as closer to its limit than it is. Under a campaign that stops on an
    attainment target, that is the difference between finishing and quitting
    with the work half done.

    Only one artifact is corrected, and only where the substitution is decidable
    from figures in hand: rocprofiler-compute reports ``MFMABF16Flops`` at
    exactly half ``MFMAF16Flops`` on architectures whose datasheet lists the two
    MFMA paths at one rate. A ratio near two on such an architecture cannot be a
    property of the chip, so the fp16 measurement -- taken on the same cores, in
    the same run -- stands in for it. Anything outside that window is left
    alone: guessing at a roof is what this module exists to avoid.
    """
    spec = arch_spec(arch)
    if spec is None:
        return peaks, {}
    datasheet_bf16 = float(spec.peak_flops.get("bf16_mfma") or 0.0)
    datasheet_fp16 = float(spec.peak_flops.get("fp16_mfma") or 0.0)
    if datasheet_bf16 <= 0 or datasheet_bf16 != datasheet_fp16:
        return peaks, {}

    measured_bf16 = float(peaks.get("bf16_mfma") or 0.0)
    measured_fp16 = float(peaks.get("fp16_mfma") or 0.0)
    if measured_bf16 <= 0 or measured_fp16 <= 0:
        return peaks, {}
    ratio = measured_fp16 / measured_bf16
    if abs(ratio - 2.0) > _BF16_ARTIFACT_RATIO_TOLERANCE:
        return peaks, {}

    repaired = dict(peaks)
    repaired["bf16_mfma"] = measured_fp16
    note = (
        f"measured bf16_mfma {measured_bf16:.6g} was exactly half measured fp16_mfma "
        f"{measured_fp16:.6g} (ratio {ratio:.3f}), a known rocprofiler-compute artifact; "
        f"{arch} runs both MFMA paths at one rate, so the fp16 measurement was substituted"
    )
    log.warning("ceiling roofs: %s", note)
    return repaired, {"bf16_mfma": note}
    ...
    *,
    ...
    if cached is not None:
        ...
        if roof_provenance.get("measured"):
            peaks, corrections = correct_known_tool_artifacts(peaks, identity.arch)
            if corrections:
                roof_provenance["corrected"] = corrections
                notes.extend(f"corrected {path}: {detail}" for path, detail in corrections.items())
            ...
            stored = store_local(
                DeviceProfile(
                    ...
                    source_by_figure={
                        path: (
                            f"rocprof-compute --roof-only, column {column}"
                            if path not in corrections
                            else corrections[path]
                        )
