...
# rejection.
def _build_framework_fallback_roots() -> dict[str, tuple[str, ...]]:
    """Build framework source root fallbacks from environment and sys.path."""
    workspace = os.environ.get("WORKSPACE_ROOT", "")
    result: dict[str, list[str]] = {"aiter": [], "sglang": [], "vllm": []}

    if workspace:
        result["aiter"].append(workspace + "/aiter")
        result["sglang"].extend([workspace + "/sglang/python", workspace + "/sglang"])
        result["vllm"].append(workspace + "/vllm")

    import sys
    for sp in sys.path:
        if os.path.isdir(sp):
            for pkg in ("aiter", "sglang", "vllm"):
                if os.path.isdir(os.path.join(sp, pkg)) and sp not in result[pkg]:
                    result[pkg].append(sp)

    return {k: tuple(v) for k, v in result.items() if v}


_FRAMEWORK_PKG_FALLBACK_ROOTS: dict[str, tuple[str, ...]] = _build_framework_fallback_roots()
