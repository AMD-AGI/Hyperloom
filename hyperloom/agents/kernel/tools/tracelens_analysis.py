...
from hyperloom.agents.kernel.tools.tracelens_skill_runner import (
...
DEFAULT_TRACELENS_ROOT = os.environ.get("TRACELENS_ROOT", "")
...
def _build_reusable_source_roots() -> tuple[str, ...]:
    """Build source root prefixes dynamically from environment."""
    roots: list[str] = []
    workspace = os.environ.get("WORKSPACE_ROOT", "")
    if workspace:
        for pkg in ("aiter/", "sglang/", "vllm/"):
            roots.append(f"{workspace}/{pkg}")

    import sys
    for sp in sys.path:
        for pkg in ("aiter/", "sglang/", "vllm/"):
            candidate = os.path.join(sp, pkg)
            if os.path.isdir(candidate) and candidate not in roots:
                roots.append(candidate + ("" if candidate.endswith("/") else "/"))

    roots.append("/aiter_meta/csrc/")
    return tuple(roots) if roots else (
        "/aiter_meta/csrc/",
    )
...
def _reusable_source_roots() -> tuple[str, ...]:
    return _build_reusable_source_roots()


_REUSABLE_SOURCE_ROOTS = None  # Lazily resolved via _reusable_source_roots()
...
def _build_known_search_roots() -> tuple[str, ...]:
    """Discover kernel source search roots from environment and sys.path."""
    roots: list[str] = []
    workspace = os.environ.get("WORKSPACE_ROOT", "")
    if workspace:
        for sub in ("aiter", "sglang/sgl-kernel", "sglang/python/sglang", "vllm"):
            candidate = os.path.join(workspace, sub)
...
KNOWN_SEARCH_ROOTS = _build_known_search_roots()
...
_HARNESS_RELATIVE_HINTS: tuple[tuple[tuple[str, ...], tuple[str, ...]], ...] = (
    (
...
def _resolve_harness_hints() -> tuple[tuple[tuple[str, ...], tuple[str, ...]], ...]:
    """Resolve relative harness hints against WORKSPACE_ROOT."""
    workspace = os.environ.get("WORKSPACE_ROOT", "")
    if not workspace:
        return _HARNESS_RELATIVE_HINTS
    return tuple(
        (markers, tuple(os.path.join(workspace, p) for p in paths))
        for markers, paths in _HARNESS_RELATIVE_HINTS
    )


_KNOWN_HARNESS_HINTS = _resolve_harness_hints()
...
_AITER_FALLBACK_REPO = os.environ.get(
    "AITER_REPO_ROOT",
    os.path.join(os.environ.get("WORKSPACE_ROOT", ""), "aiter") if os.environ.get("WORKSPACE_ROOT") else ""
)
...
def _candidate_model_config_paths(model_name: str) -> list[Path]:
    ...
    models_root = Path(os.environ.get("HYPERLOOM_MODELS_ROOT", "/models"))
...
def _default_workspace_path() -> str:
    ...
    return (
        ...
        or os.path.join(os.environ.get("HOME", "/tmp"), "hyperloom")
    )
