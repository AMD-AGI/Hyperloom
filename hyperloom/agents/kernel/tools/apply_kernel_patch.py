...
def _build_target_roots() -> tuple[str, ...]:
    """Build patchable target roots from environment and sys.path."""
    roots: list[str] = []
    ...
    for sp in sys.path:
        for pkg in ("aiter/", "sglang/", "vllm/"):
            ...
            if os.path.isdir(candidate):
                roots.append(candidate if candidate.endswith("/") else candidate + "/")
    return tuple(roots) if roots else ("./",)


KNOWN_TARGET_ROOTS = _build_target_roots()
...
_REBUILD_STRATEGIES: list[tuple[str, str, list[str]]] = [
    ("aiter", "setup.py", ["develop"]),
    ("sglang/sgl-kernel", "-m pip install -e .", []),
    ("sglang", "-m pip install -e python", []),
    ("vllm", "-m pip install -e .", []),
]
...
def _detect_strategy(target_file: Path, *, allow_unknown_target: bool) -> dict[str, Any]:
    ...
    workspace = os.environ.get("WORKSPACE_ROOT", "")
    for pkg_suffix, cmd_args, extra in _REBUILD_STRATEGIES:
        marker = f"/{pkg_suffix}/"
        if marker in lower:
            root = Path(os.path.join(workspace, pkg_suffix)) if workspace else _find_pkg_root(target_file, pkg_suffix)
            parts = cmd_args.split()
            if parts[0] == "-m":
                rebuild_command = [python, *parts]
            else:
                rebuild_command = [python, *parts]
            artifact_roots = [root] if root else []
            break

    if root is None and allow_unknown_target:
...
def _find_pkg_root(target_file: Path, pkg_name: str) -> Path | None:
    """Walk up from target_file to find the package root directory."""
