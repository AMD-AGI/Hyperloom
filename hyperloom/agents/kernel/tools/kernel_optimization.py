...
from hyperloom.agents.kernel.tools._collective_names import kernel_name_implies_multigpu  # noqa: E402
sys.path.pop(0)
...
def build_prompt(
    ...
    *,
    ...
    safety = (
        ...
        f"- Do NOT modify files under {kernel_repo or os.environ.get('KERNEL_REPO_ROOT', '<kernel-repo>')} or any system path.\n"
        ...
        f"  equivalent Triton implementation under `{kernel_repo or os.environ.get('KERNEL_REPO_ROOT', '<kernel-repo>')}/aiter/ops/triton/...`\n"
        ...
        f"  `hipcc -O3 -std=c++17 -DUSE_ROCM -I{kernel_repo or os.environ.get('KERNEL_REPO_ROOT', '<kernel-repo>')}/csrc/include "
        ...
        f"    AITER_INC = '{kernel_repo or os.environ.get('KERNEL_REPO_ROOT', '<kernel-repo>')}/csrc/include'\n"
        "    opt = load(\n"
        "        name='opt_kernel',\n"
...
def _kernel_agent_root() -> Path:
    ...
    """
    return Path(os.environ.get("USER_DATA_PATH", os.path.join(os.environ.get("HOME", "/tmp"), "hyperloom"))) / "kernel-agent"
...
def main() -> int:
    ...
    parser.add_argument(
        "--workspace-path",
        default=os.environ.get("USER_DATA_PATH", os.path.join(os.environ.get("HOME", "/tmp"), "hyperloom")),
        help=(
