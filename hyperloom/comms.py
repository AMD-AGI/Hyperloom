"""File-based communication protocol between orchestrator and agents.
...
class IncrementalResult:
    """A single incremental result from an agent (appended to results.jsonl)."""
...
# ─── Write functions (called by orchestrator or agent) ─────────────────────────
...
# ─── Read functions (called by orchestrator) ───────────────────────────────────
...
def is_agent_alive(session_dir: str, agent_id: str, timeout_s: float = 600.0) -> bool:
    """Check if agent is alive (heartbeat, active process log, or running PID)."""
    ...
    # No heartbeat — check if process.log is actively being written to
...
) -> dict[str, CompletionReport | None]:
    """Wait for multiple agents to complete. Returns their reports."""
    start = time.time()
    results: dict[str, CompletionReport | None] = {}
    ...
    while pending and (time.time() - start) < timeout:
        for aid in list(pending):
            ...
            if report is not None:
                results[aid] = report
                pending.discard(aid)
            elif not is_agent_alive(session_dir, aid):
                results[aid] = None
                pending.discard(aid)
        if pending:
