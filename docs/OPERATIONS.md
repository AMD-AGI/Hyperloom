...
[`reference/environment-variables.md`](reference/environment-variables.md); for
credential setup see [`ENV_AND_AUTH.md`](ENV_AND_AUTH.md); for
recurring symptoms see [`reference/troubleshooting.md`](reference/troubleshooting.md).
...
| Ray worker (GEAK / OOB attempt)    | 8 cores   | 32 GiB    | 1 × MI300X / MI308X / MI325X / MI355X              | ~10 GB per attempt for build artefacts                                                     |
...
## 4. LLM gateway credential checks

Hyperloom no longer runs a local auth-proxy. Claude, Codex, GEAK, and OOB
workers call the configured upstream gateway directly using aliases generated by
preflight and the kernel-agent installer.
...
Clear any stale `GEAK_BASE_URL`, `OOB_BASE_URL`, or `OPENAI_BASE_URL` values that
still point at `127.0.0.1:4002`; explicit endpoint overrides are preserved by
design.
...
### Scenario C: Gateway 401 after stale credentials
...
3. Clear stale `GEAK_BASE_URL`, `OOB_BASE_URL`, or `OPENAI_BASE_URL` overrides
   that still point at the removed local proxy.
