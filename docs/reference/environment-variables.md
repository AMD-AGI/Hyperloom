...
| `HYPERLOOM_`<br>`RUNTIME_DIR`             | No                   | `$USER_DATA_PATH/runtime` (installer)                               | Private writable runtime state. Codex SDK turns create a unique mode-`0700` `CODEX_HOME` here and remove it after the SDK client closes. When unset, Codex uses the first safe declared output root, then a run-local working directory; it never falls back to `/tmp` or a source checkout. When `USER_DATA_PATH` sits on NFS or another network filesystem, set this explicitly to **node-local** fast disk while leaving session outputs on the shared mount — Codex's SQLite thread store requires working file locking. See [Codex turns stall on network storage](troubleshooting.md#codex-turns-stall-or-tracelens-roofline-times-out-on-network-storage). |
