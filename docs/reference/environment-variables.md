...
|------------------------|----------|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `OPENAI_API_KEY`       | Conditional | —    | OpenAI-compatible LLM gateway key. Format `ak-...`. Required for the single-gateway setup; split-gateway deployments can instead provide provider-specific keys. Source for GEAK / Claude / Codex / Critic / Robustness credentials downstream (auto-aliased).                                                        |
...
| `ANTHROPIC_AUTH_TOKEN` | No       | —    | Claude CLI auth token alias; not auto-filled. Set explicitly only for split-gateway deployments (a single gateway auths Claude via `ANTHROPIC_API_KEY` / the generated `~/.claude/config.json`).                                                                        |
| `GEAK_API_KEY`         | No       | Inherits `OPENAI_API_KEY` | Only set explicitly to override the default inheritance.                                                                                                                              |
...
| `ANTHROPIC_API_KEY`    | No       | Inherits `OPENAI_API_KEY` (single-gateway `install.sh` derivation) | Set explicitly for split-gateway; a single gateway derives it from `OPENAI_API_KEY` at install time. Preflight does not cross-fill it.                                                                                                                                |
