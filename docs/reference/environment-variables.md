...
| `OPENAI_API_KEY`       | Conditional | —    | OpenAI-side key, format `ak-...`. Pairs with `OPENAI_BASE_URL`; may omit it only when the Anthropic side has no base URL either, in which case the official OpenAI endpoint is implied. Also the source for the internal GEAK / LLM aliases. Never used for the Anthropic side.                                                        |
