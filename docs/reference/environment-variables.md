...
| `FORGE_PATH`                               | No                   | Unset                                                              | Developer override naming a KernelForge checkout, and the single canonical variable for it. KernelForge ships inside this distribution, so the forge kernel backend (`KERNEL_OPT_BACKEND_ORDER=forge`) works with it unset: the packaged `kernelforge` is imported and the vendor-playbook task bundles resolve from `kernelforge/data/examples/`. When set, `forge_submit.py` prepends it to `sys.path` and prefers its bundles, so a checkout can be iterated on without reinstalling. |
