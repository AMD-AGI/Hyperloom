...
| `ORIGAMI_ROOT`                            | No (used only when Origami is enabled) | `${HYPER`<br>`LOOM_CA`<br>`CHE_DIR:-`<br>`$REPO_ROOT`<br>`/.cache}/ro`<br>`cm-libraries@<sha>/shared/origami` | Existing `rocm-libraries/shared/origami` checkout. When unset and the Origami feature is enabled, the kernel installer creates a pinned sparse checkout. |
| `ORIGAMI_REPO`                            | No | `https://github.com/ROCm/rocm-libraries.git` | Repository used by the managed Origami sparse clone. |
| `ORIGAMI_REF`                             | No | Hyperloom-validated full commit SHA | `rocm-libraries` revision used to build Origami. The installer resolves non-SHA overrides before creating the revision-keyed cache. |
...
| `HYPERLOOM_ORIGAMI_GEMM_FALLBACK` | Unset (off) | Explicit opt-in for the Origami plain A8W8 blockscale pre-tuner. Set to `1` before running the kernel installer. Unset/false performs no Origami installation or runtime work. |
| `HYPERLOOM_ORIGAMI_GEMM_TIMEOUT_SEC` | `3600` | Overall timeout for Origami fallback selection and paired benchmarking. Values below `60` are clamped to `60`. |
| `HYPERLOOM_ORIGAMI_BENCHMARK_WARMUP` | `3` | Warmup launches for both the Origami-selected and default AITER kernels. |
| `HYPERLOOM_ORIGAMI_BENCHMARK_ROUNDS` | `5` | Alternating paired timing rounds; values below `3` are raised to `3`. |
| `HYPERLOOM_ORIGAMI_BENCHMARK_ITERATIONS` | `10` | GPU-event-timed launches per kernel in each round. |
| `HYPERLOOM_ORIGAMI_BENCHMARK_MIN_SPEEDUP` | `1.0` | Minimum measured `default_us / origami_us` required to emit an Origami override row. |
