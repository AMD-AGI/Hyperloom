...
| `OPENAI`<br>`_CUSTOM_HEADERS` | No  | —    | OpenAI-side equivalent, passed to the SDK client as `default_headers`. Read only by the OpenAI side and never derived from `ANTHROPIC_CUSTOM_HEADERS`.                                                                                                             |
