...
| `CLAUDE_CODE`<br>`_USE_BEDROCK` | No | — | Route the Claude runtime through AWS Bedrock, which carries the credential itself. Counts as a configured Anthropic side for backend selection, so a Bedrock box is not read as unconfigured and redirected to Codex. |
| `CLAUDE_CODE`<br>`_USE_VERTEX` | No | — | Route the Claude runtime through Google Vertex AI. Counts as an Anthropic side exactly as `CLAUDE_CODE_USE_BEDROCK` does. |
