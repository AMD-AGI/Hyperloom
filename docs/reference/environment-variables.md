...
| `FORGE_CLAUDE_MODEL`  | No       | Inherits `CLAUDE_MODEL` | Forge Claude backend model id, read by every Forge surface: fusion, rewrite, collective, and the kernel campaign (`forge-loop`) itself. Set when Forge should use a different Claude model than orchestration. `FORGE_AGENT_MODEL` outranks it for a run that names one model regardless of which backend answers. |
| `FORGE_CODEX_MODEL`   | No       | Inherits `CODEX_MODEL`  | Forge Codex backend model id, read by every Forge surface: fusion, rewrite, collective, and the kernel campaign (`forge-loop`) itself. Set when Forge should use a different Codex model than the OpenAI-side default. |
| `HYPERLOOM`<br>`_REASONING`<br>`_EFFORT` | No | Provider default | Reasoning effort for Hyperloom's own LLM calls (`minimal` / `low` / `medium` / `high`), and the effort a Forge campaign runs at unless `FORGE_AGENT_REASONING_EFFORT` names one. Set it once and every component runs at the depth you asked for. |
| `FORGE_AGENT`<br>`_REASONING`<br>`_EFFORT` | No | `high` | Forge-only reasoning effort. Outranks `HYPERLOOM_REASONING_EFFORT`, and outranks whatever a Forge call site would have chosen — every agent session in a campaign runs at this effort. |
