...
| `FORGE_AGENT_MODEL`   | No       | `CLAUDE_MODEL` / `CODEX_MODEL` | One model id for a Forge run regardless of which backend answers it, read by every Forge surface: fusion, rewrite, collective, and the kernel campaign (`forge-loop`) itself. Unset, Forge follows the orchestration-side variable for the backend it is on; Forge has no per-provider variable of its own. |
