...
| `FORGE_CLAUDE_FALLBACK_MODEL` | No | `claude-opus-4-8` | Claude model KernelForge probes/SDK-falls back to after the primary fails. Unset or blank keeps the default. `none` or `off` disables fallback. |
...
| `FORGE_CODEX_FALLBACK_MODEL` | No | `gpt-5.5` | Codex model KernelForge probes after the primary fails. Unset or blank keeps the default. `none` or `off` disables fallback. |
