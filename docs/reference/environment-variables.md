...
| `HYPERLOOM_GEMM_SHAPE_CAPTURE` | `1`                           | Enables automatic runtime GEMM-shape capture for eligible single-node dense vLLM Forge tuning when no explicit shape input is available. Block-FP8 first reuses shapes from a successful Roofline profile with an exactly matching model, precision, TP, concurrency, sequence lengths, server arguments, environment, and backend controls; it launches a dedicated profile only as a fallback. Set to `0` to preserve the no-capture path. |
| `HYPERLOOM_GEMM_SHAPE_CAPTURE_TIMEOUT_SEC` | `1800`          | Timeout in seconds for the additional vLLM shape-capture benchmark. Values below `60` are clamped to `60`. |
| `HYPERLOOM_GEMM_SHAPE_CAPTURE_MAX_ITERATIONS` | `16`          | Maximum vLLM engine iterations captured by the dedicated Block-FP8 fallback. The capture starts at iteration `0`, so this bounded window covers prefill and the early prefill-to-decode transition instead of drifting into a long decode-only trace. |
