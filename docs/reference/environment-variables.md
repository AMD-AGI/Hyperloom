...
| `CLAUDE`<br>`_CONTEXT`<br>`_WINDOW` | No | Unset | Context window to name on a Claude model id, spelled as Anthropic spells it (`1m`), producing `claude-opus-5[1m]`. **Leave it unset unless your gateway publishes windowed ids**: the gateway Hyperloom deploys against answers `400 Invalid model name` to every bracketed form, so a suffix set here fails every session at the startup probe. Codex ids are never suffixed. |
