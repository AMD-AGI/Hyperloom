...
| `HYPERLOOM`<br>`_OPEN_`<br>`SOURCE`<br>`_ROOT`              | No                   | `/opt/hyperloom/`<br>`open-source`<br>`-repos`                      | Pod-local root for auto-cloned open-source dependencies such as Magpie, TraceLens, GEAK, and InferenceX; also used for KernelForge when the forge backend runs `local_setup.sh`. Decoupled from `USER_DATA_PATH` so shared session storage does not collocate concurrent pods' checkouts. |
