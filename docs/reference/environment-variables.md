...
|------------------------|----------|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `OPENAI_BASE_URL`      | Conditional | —    | OpenAI-side endpoint. Required together with `OPENAI_API_KEY` to enable the OpenAI side (Codex / GEAK). Example: `https://<your-gateway-host>/api/v1/llm-proxy/v1`.                                                                                                                  |
| `OPENAI_API_KEY`       | Conditional | —    | OpenAI-side key, format `ak-...`. Required together with `OPENAI_BASE_URL`. Also the source for the internal GEAK / LLM aliases. Never used for the Anthropic side.                                                        |
...
| `ANTHROPIC_API_KEY`    | Conditional | —    | Anthropic-side key. Required together with `ANTHROPIC_BASE_URL`. Never derived from `OPENAI_API_KEY`; setting it without its base URL fails preflight.                                                                                                                                |
| `ANTHROPIC_AUTH_TOKEN` | No       | —    | Claude CLI auth token alias, accepted in place of `ANTHROPIC_API_KEY`. Not auto-filled.                                                                        |
| `GEAK_API_KEY`         | No       | Inherits `OPENAI_API_KEY` | Internal alias; only set explicitly to override.                                                                                                                              |
| `GEAK_BASE_URL`        | No       | Inherits `OPENAI`<br>`_BASE_URL` | Internal alias; only set explicitly to override.                                                                                                                          |
