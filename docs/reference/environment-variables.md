...
| `KB_STORE_URL`                        | Unset                  | Unified KB Service endpoint. Required when `KNOWLEDGE_STORE_MODE=remote`; remote Recipe mode selects the current Recipe View, replays its Config, Patch, and Kernel columns, then writes one final session at CLOSE. The same value also enables the co-hosted PR Monitor at `${KB_STORE_URL}/pr-monitor/v1` (REST/IR-3/Framework/KernelForge) and `${KB_STORE_URL}/pr-monitor/mcp/` (specialist MCP). |
