...
| `HYPERLOOM`<br>`_OPEN_`<br>`SOURCE`<br>`_ROOT`              | No                   | `/opt/hyperloom/`<br>`open-source`<br>`-repos`                      | Pod-local root for auto-cloned open-source dependencies such as Magpie, TraceLens, GEAK, KernelForge, and InferenceX. Decoupled from `USER_DATA_PATH` so shared session storage does not collocate concurrent pods' checkouts. |
