...
| `HYPERLOOM_GEMM_SHAPE_CAPTURE` | `1`                           | Enables automatic runtime GEMM-shape capture for eligible single-node dense vLLM Forge tuning when no explicit shape input is available. Block-FP8 first reuses shapes from a successful Roofline profile with an exactly matching model, precision, TP, concurrency, and sequence-length context; it launches a dedicated profile only as a fallback. Set to `0` to preserve the no-capture path. |
