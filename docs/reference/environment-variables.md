...
| `OPENAI`<br>`_CUSTOM_HEADERS` | No  | —    | OpenAI-side equivalent, passed to the SDK client as `default_headers`. Read only by the OpenAI side and never derived from `ANTHROPIC_CUSTOM_HEADERS`. Unlike its Anthropic counterpart it does not persist: bare-metal setup drops it from `.env` whenever it writes resolved credentials, so export it in the shell or re-add it after setup. |
