...
[Predictor HTTP contract](primatune-predictor.md).
...
| `HYPERLOOM_PREDICTOR_MAX_CHAIN`         | `3`        | Consecutive predictor rounds without a KEEP before the LLM specialists are let back in (`--primatune-max-chain`). A losing streak, not a total: a KEEP clears it, so a chain that keeps landing is never cut off. The count doubles as the attempt number in the idempotency key, which is what lets a sampling predictor take a second look at an unchanged stack depth. Coordinator-owned state, locked in `CORE_STATE_FIELDS`. |
| `HYPERLOOM_PREDICTOR_MAX_VARIANTS`      | `3`        | Variants one answer may contribute to a round. A sampling predictor returns as many distinct proposals as it found — eight samples deduplicated to about six in every batch measured — and each is a benchmark round of roughly seven minutes. Unbounded, one decision point would outspend the whole FRAMEWORK budget. Truncation keeps the head and is deliberately not a ranking: the most valuable flag observed was a minority sample, not the modal one. |
