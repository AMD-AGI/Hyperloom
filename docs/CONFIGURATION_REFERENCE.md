...
| `HYPERLOOM_LANGFUSE_ENABLE` | Enables live Langfuse trace push when `1/true/yes/on` and `LANGFUSE_HOST`, `LANGFUSE_PUBLIC_KEY`, and `LANGFUSE_SECRET_KEY` are set. `assets/install.sh` installs the optional SDK only when needed. Local `reports/trace/*.jsonl` is always written. Each breakdown records a `langfuse` receipt with enabled/disabled status, redacted config, trace/session ids, and sent counts. |
