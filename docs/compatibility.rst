...
.. list-table::
   ...
   * - Atom
     - 7.2.4
     - AMD out-of-tree engine, launched as ``python3 -m atom.entrypoints.openai_server``. Container image only: ``install_baremetal.sh`` verifies ``atom`` but cannot install it, because ``--install-framework`` accepts only ``none``, ``sglang`` and ``vllm``. Run the kernel phase on the KernelForge backend (``KERNEL_OPT_BACKEND_ORDER=forge``); GEAK does not drive kernel rewrites on Atom.
   * - ``custom``
     - Host-defined
...
7.2.3), matching ``vllm/vllm-openai-rocm:v0.28.0``; the SGLang stack
