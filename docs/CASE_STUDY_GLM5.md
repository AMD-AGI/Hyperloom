...
Autonomous optimization of GLM-5-FP8 on AMD MI355X — from **174 tok/s/GPU** to **509 tok/s/GPU** (2.93x), exceeding NVIDIA B200 by 14% on per-GPU throughput. The optimization explored 40+ configurations, produced code patches, and tuned 55 GEMM shapes. The most impactful findings required the agent to move fluidly across repository boundaries — from SGLang's serving layer, into aiter's Composable Kernel library, down to Triton GEMM config files and individual kernel source — building a cross-stack understanding that is time-consuming to develop manually.

**Model**: GLM-5-FP8 (756B, 256-expert MoE, NSA + MLA, 78 layers)
**Hardware**: 4x MI355X (gfx950) | **Framework**: SGLang v0.5.9-dev + aiter

---

## Starting Point

The InferenceX default config ran GLM-5 on 8x MI355X at TP=8 and produced 174 tok/s/GPU at concurrency 64. The B200 reference was 448 tok/s/GPU — a 61% gap. The agent profiled the baseline with TraceLens and found: 44.9% of the decode step was AllReduce communication, 8.2% was the MoE router GEMM, and 49% of GPU time was idle.
...
|---|---|
| `--nsa-decode-backend aiter` — CK attention kernels for NSA decode | +3.1% |
...
<img src="figs/glm5_optimization_breakdown.png" alt="GLM-5 Optimization Breakdown" width="480" />
...
All values in tok/s/GPU. ISL=1024, OSL=1024, FP8 weights.
