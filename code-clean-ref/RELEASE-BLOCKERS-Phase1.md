...
#### P1-41 · External mount timeout 没有真正的 timeout

代码同步调用 `os.stat()`，只在返回后记录耗时；WekaFS/外部挂载真挂死时，
整个 robustness tick 可无限阻塞。

- 锚点：`src/hyperloom/agents/robustness/sources/local_probe.py:2014-2059`
- 建议：放入可取消线程/子进程并实施硬 timeout。
...
`model_identity_candidates()` 对普通 `org/repo` 使用 `Path(raw).name`，候选构造时即丢失 org；
`a/Llama-8B` 与 `b/Llama-8B` 会匹配，陈旧 `model_arch.json` 可被另一模型复用。

- 锚点：`src/hyperloom/common/model_paths.py:42-85`
- 建议：保留规范化的完整 `org/repo`，增加跨 org 负例。

#### P2-02 · `jsonio` fenced JSON 抽取与 tolerant read 的类型契约不闭合

非贪婪 fence regex 无法可靠处理嵌套对象；`read_jsonl()` 标注 list 却可返回任意 default，
`Path.is_file()` 的 OSError 也可越过 tolerant 边界。

- 锚点：`src/hyperloom/common/jsonio.py:14,72-103,141-144,179-200`
- 建议：使用 JSON decoder 扫描；默认值和返回类型显式泛型化。
...
root 为 `/` 时 source containment 也可退化。

- 锚点：`src/hyperloom/common/kernel_source_contract.py:159-198,233-258`
- 建议：对 ID 唯一性、字段类型/值域和 resolved containment 做完整 validation。
...
`_metric_float` 没有 finite 校验；NaN throughput/accuracy 可穿过比较和 gate。

- 锚点：`src/hyperloom/agents/framework/runtime/tools_api.py:187-202,253-275`
- 建议：所有 benchmark metric 统一 finite/non-negative validator。

#### P2-10 · Framework shell template 进行多轮替换，值会被二次解释

前一个变量值中包含 `{later_key}` 时会在后续轮次再次替换，破坏单变量单次引用语义。

- 锚点：`src/hyperloom/agents/framework/shell.py:69-101`
- 建议：使用单遍 regex callback，并分别 quote 原始 value。
