...
_DEFAULT_ORCH_PROMPT = (
    ...
    "                   top_k: 10}}\n"
    "  Wait for `select_kernels_done`. Do NOT re-emit while a response is\n"
    "  pending. Do NOT re-emit if you already have a fresh response in\n"
    "  inbox AND last_kernel_opt is empty — go straight to K2.\n\n"
    "step K2: from the select_kernels response, pick `hot_kernels[i]` with\n"
    "  the smallest `i` such that:\n"
    "    - `source_file` is non-empty (else handler can't dispatch a backend)\n"
    "    - `reusable_native_kernel == true` (stable framework source only)\n"
    "    - `runtime_generated_kernel != true` (skip torch.compile/Inductor cache)\n"
    "    - `recommended_backends` contains 'claude' or 'codex'\n"
    "    - `kernel_id` is NOT equal to last_kernel_opt.kernel_id (don't\n"
    "      retry the same kernel — pick the next hot one instead)\n"
    "  Then emit\n"
