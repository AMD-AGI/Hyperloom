...
def is_complete(dest: Path, repo_id: str, hf_api: HfApi, token: str) -> bool:
    ...
    Args:
        dest (Path): Local destination directory to validate.
        repo_id (str): The HuggingFace repo id.
        hf_api (HfApi): Client used to list the repo's files.
        token (str): HF token for authenticated/gated repos.

    Returns:
        bool: True if every repo file is present locally with non-zero size.
    """
    if not dest.is_dir():
        return False
    try:
...
def _slice_repos(repos: list[str], batch_index: int | None,
    ...
    """Apply optional --batch-index / --batch-size slice.

    Uses the same semantics as generate_hf_matrix.py so a workflow's prewarm +
    optimize stages can target the exact same set of repos.

    Args:
        repos (list[str]): The full list of repo ids.
        batch_index (int | None): 0-based slice index (defaults to 0).
        batch_size (int | None): Slice length; when falsy, no slicing is done.

    Returns:
        list[str]: The selected slice of repos.
    """
    if not batch_size:
        return repos
    bi = batch_index or 0
