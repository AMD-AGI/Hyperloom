...
def render_prompt(
    merged: dict, *, pr_mode: bool = False, git_ref: str | None = None, gh_token: str | None = None
) -> str:
...
def run_model(
    ...
    *,
    ...
    if pr_mode:
        log.info("PR-CI mode: validating commit %s (sandbox will git-clone PR head)", git_ref[:12] if git_ref else "?")
    log.info("═" * 60)

    prompt = render_prompt(merged, pr_mode=pr_mode, git_ref=git_ref, gh_token=gh_token)
    ...
    for attempt in range(1, _MAX_RETRY_ATTEMPTS + 1):
        ...
        def _monitor():
            ...
            status_holder["status"] = claw.monitor_session(session_id, timeout=sandbox_timeout, on_event=_on_event)
        ...
        if status == "running":
            status = "timeout"
            log.warning(
                "Session %s still running after sandbox_timeout (%ds), marking as timeout", session_id, sandbox_timeout
            )

        log.info(
            "Session %s finished with status: %s (elapsed %.0fs, %d tool calls)",
            session_id,
            status,
            elapsed,
...
def main():
    ...
    parser.add_argument(
        "--models",
        default=None,
        help="Comma-separated model subset (matches per-entry `key` field, fallback `inferenceX_key`)",
    )
    ...
    parser.add_argument(
        "--gh-token-env",
        default="HYPERLOOM_PR_CI_GH_TOKEN",
    ...
    with tempfile.TemporaryDirectory() as _tmpdir:
        subprocess.run(
            ["git", "clone", "--depth=1", "--branch=main", ifx_cfg["repo"], _tmpdir],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        if check_script.exists():
            ...
            try:
                result = subprocess.run(
                    cmd, capture_output=True, text=True, timeout=120, env={**os.environ, "PYTHONIOENCODING": "utf-8"}
                )
                if result.stdout:
    ...
    for model_cfg in model_list:
        ...
        merged = merge_model_config(model_cfg, ifx_entry, defaults, harbor_prefix, ifx_benchmarks)
    ...
    if pr_mode:
        ...
        if not gh_token:
            log.error(
                "--pr-mode requires env %s to hold a GitHub token (got empty/unset). "
                "The workflow normally passes ${{ github.token }} here.",
                args.gh_token_env,
            )
            sys.exit(2)
        log.info(
            "PR-CI mode enabled: git_ref=%s, gh_token=*** (from %s)",
            git_ref[:12] if len(git_ref) > 12 else git_ref,
            args.gh_token_env,
        )
    ...
    if args.dry_run:
        for merged in merged_models:
            ...
            print(f"\n{'=' * 60}")
            print(f"Model: {merged['model_hf']} ({merged['precision']}) [pr_mode={pr_mode}]")
            print(f"{'=' * 60}")
            print(prompt)
        sys.exit(0)
    ...
    try:
        ...
        (out_dir / "ci_report_raw.json").write_text(json.dumps(results, indent=2, default=str))
    ...
    try:
        ...
        (out_dir / "ci_summary_raw.json").write_text(json.dumps(results, indent=2, default=str))
    ...
    if json_summary:
        log.info(
            "Summary: %d models, %d completed, %d failed",
        ...
        log.info("Summary (fallback): %d models, %d completed, %d failed", len(results), completed, failed)
...
def _send_webhook(webhook: str, summary: dict):
    ...
    rows = [
        ...
        _row(
            "**Optimization Gain**",
            f"**{'--' if gain is not None and abs(gain) < 0.05 else f'{gain:+.1f}%' if gain is not None else 'N/A'}**",
            gain_color,
        ),
    ]
    ...
    try:
        ...
        if resp.status_code >= 300:
            req.post(
                webhook,
                json={"text": f"{status_emoji} Hyperloom CI [{model}]: {status} | Gain: {_val('gain_pct', '+.1f')}%"},
                timeout=10,
            )
    except Exception as e:
