...
DEFAULT_ARTIFACT_PATTERNS = (
    ...
    "ci_metrics.json",
    # Claw sandbox lifecycle log. For sandbox-start failures this may be the
    # only artifact available, so CI can still synthesize a publishable RCA.
    "claw-",
...
def _infer_incomplete_session_stop_reason(session_dir: str | Path, state: dict) -> tuple[str, str]:
    """Infer a CI-only terminal reason for a matched session missing final artifacts.

    Args:
        session_dir: Source NFS session directory.
        state: Parsed ``state.json``.

    Returns:
        ``(stop_reason, detail)`` suitable for emergency CI artifacts.
    """
    for key in ("stop_reason", "status"):
        value = str(state.get(key) or "").strip()
        if value:
            return value, f"state.{key}={value}"

    name = Path(session_dir).name.lower()
    if "aborted_bf16_misconfig" in name or "bf16_misconfig" in name:
        return (
            "model_config_incompatible",
            "launcher abort marker indicates BF16/model precision misconfiguration",
        )
    if "aborted" in name:
        return "launcher_aborted", "session directory carries an ABORTED marker"

    phase = str(state.get("phase") or "").strip()
    if phase in {"EXPLORE", "SWEEP", "KERNEL", "CLOSE"}:
        return "incomplete_after_progress", f"state reached phase={phase} but final artifacts are missing"
    if phase:
        return "incomplete_session", f"state reached phase={phase} but final artifacts are missing"
    return "incomplete_session", "matched state.json has no stop_reason or phase"


def _write_ci_emergency_artifacts(
    rec: SubmissionRecord,
    source_session_dir: str | Path,
    task_dir: Path,
) -> int:
    """Write local emergency CI artifacts for a state-only/incomplete NFS session.

    This is intentionally CI-local: it does not mutate the source session under
    WekaFS and does not change inference_optimizer's runtime exporter contract.
    """
    source = Path(source_session_dir)
    state = _read_session_state(source)
    if not state:
        return 0

    task_dir.mkdir(parents=True, exist_ok=True)
    stop_reason, detail = _infer_incomplete_session_stop_reason(source, state)
    workload = state.get("workload") if isinstance(state.get("workload"), dict) else {}
    source_display = str(source).replace("\\", "/")
    model_field = str(
        state.get("model")
    ...
    gain = state.get(
        ...
        state.get("cumulative_gain_validated", state.get("cumulative_gain", 0.0)),
    )
    try:
    ...
    payload = {
        "schema_version": "hyperloom.session_breakdown.ci_emergency.v1",
        "exporter_version": "ci-emergency-artifact-1.0.0",
        ...
        "workload": {
            ...
            "framework": state.get("framework") or (rec.detected or {}).get("framework"),
            "gpu_type": state.get("gpu_type") or rec.overrides.get("gpu_type"),
        },
        "baseline": {
            "throughput_tok_s_per_gpu": state.get("baseline_tput", 0.0),
            "total_failures": state.get("baseline_total_failures", 0),
            "failure_streak": state.get("baseline_failure_streak", 0),
        },
        "final": {
            "stop_reason": stop_reason,
            "cumulative_gain_pct_validated": gain_value,
            "best_action": (state.get("current_best") or {}).get("action") if isinstance(state.get("current_best"), dict) else None,
        },
        "warnings": [
            "CI generated this emergency artifact from state.json because the matched session had no publishable session_breakdown.json.",
            detail,
        ],
    }
    ...
    if not breakdown.exists():
        breakdown.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
        _record_artifact_source(
            rec,
            breakdown,
            "nfs_user_session_ci_emergency",
            source_path=str(source / "state.json"),
            session_dir=str(source),
        )
        rec.artifact_files.append(str(breakdown))
    ...
    if not report.exists():
        report.write_text(
            "\n".join(
                [
                    "# Inference Optimizer — CI emergency report",
                    "",
                    "> Auto-generated by CI artifact fallback from `state.json` because final artifacts were missing.",
                    "",
                    f"- model: `{model_field or '-'}`",
                    f"- session: `{source.name}`",
                    f"- phase: `{phase or '-'}`",
                    f"- stop_reason: `{stop_reason}`",
                    f"- reason: {detail}",
                    f"- source_session_dir: `{source_display}`",
                    "",
                ]
            ),
            encoding="utf-8",
        ...
        rec.artifact_files.append(str(report))
        rec.artifact_count += 1
        written += 1

    state_src = source / "state.json"
    state_dst = task_dir / "state.json"
    if state_src.is_file() and not state_dst.exists():
        shutil.copy2(state_src, state_dst)
        _record_artifact_source(
            rec,
            state_dst,
            "nfs_user_session_ci_emergency",
            source_path=str(state_src),
            session_dir=str(source),
        )
        rec.artifact_files.append(str(state_dst))
        rec.artifact_count += 1
        written += 1
...
def _read_claw_jsonl_summary(path: Path) -> dict:
    """Read a local ``claw-*.jsonl`` artifact into a compact lifecycle summary."""
    try:
    ...
    phases = [
        str(ev.get("phase") or ev.get("status") or "")
        for ev in events
        if isinstance(ev, dict) and (ev.get("phase") or ev.get("status"))
    ]
    return {
        "path": path,
        "session_id": header.get("sessionId"),
        "message_id": header.get("messageId") or path.stem,
        "turns": header.get("turns"),
        "event_count": header.get("eventCount"),
        "failure_reason": header.get("failureReason") or last_event.get("reason") or "",
        "error": header.get("error") or last_event.get("message") or "",
...
def _write_claw_emergency_artifacts(rec: SubmissionRecord, task_dir: Path) -> int:
    """Write emergency artifacts from Claw lifecycle JSONL when no session files exist."""
    task_dir.mkdir(parents=True, exist_ok=True)
    if (task_dir / "session_breakdown.json").is_file():
        return 0

    claw_files: list[Path] = []
    for raw in rec.artifact_files:
        p = Path(raw)
        if p.is_file() and p.name.startswith("claw-") and p.suffix == ".jsonl":
            claw_files.append(p)
    claw_files.extend(p for p in task_dir.glob("**/claw-*.jsonl") if p.is_file())
    seen: set[str] = set()
    unique = []
    for p in claw_files:
    ...
    summary = _read_claw_jsonl_summary(unique[0])
    if not summary:
        return 0

    source_claw_artifact = str(unique[0]).replace("\\", "/")
    failure_reason = str(summary.get("failure_reason") or "")
    turns = summary.get("turns")
    if failure_reason == "sandbox_poll_timeout" and turns == 0:
        stop_reason = "sandbox_start_failed"
        detail = str(summary.get("error") or "sandbox poll timed out before the agent started")
    elif rec.final_message == "optimization report not found; skill may have exited early":
        stop_reason = "missing_breakdown_after_claw_activity"
        detail = str(summary.get("error") or rec.final_message or "Claw completed without publishable breakdown")
    else:
        stop_reason = failure_reason or "claw_lifecycle_incomplete"
        detail = str(summary.get("error") or rec.final_message or "Claw lifecycle artifact did not include a publishable breakdown")

    payload = {
        "schema_version": "hyperloom.session_breakdown.ci_emergency.v1",
        "exporter_version": "ci-emergency-claw-artifact-1.0.0",
        "ci_emergency_artifact": True,
        "ci_emergency_reason": detail,
        ...
        "session": {
            ...
            "phase": "SANDBOX_START" if stop_reason == "sandbox_start_failed" else "UNKNOWN",
            "model": rec.model,
            "turns": turns,
        },
        "workload": {
            ...
            "framework": (rec.detected or {}).get("framework"),
            "gpu_type": rec.overrides.get("gpu_type"),
        },
        "baseline": {
        ...
        "warnings": [
            "CI generated this emergency artifact from Claw lifecycle JSONL because no publishable session_breakdown.json was available.",
            detail,
        ],
    }
    ...
    _record_artifact_source(
        rec,
        breakdown,
        "safe_artifact_api_claw_ci_emergency",
        source_path=str(unique[0]),
    )
    rec.artifact_files.append(str(breakdown))
    rec.artifact_count += 1
    written += 1
        ...
        report.write_text(
            "\n".join(
                [
                    "# Inference Optimizer — CI emergency report",
                    "",
                    "> Auto-generated by CI artifact fallback from `claw-*.jsonl` because final artifacts were missing.",
                    "",
                    ...
                    f"- reason: {detail}",
                    f"- source_claw_artifact: `{source_claw_artifact}`",
                    "",
                ]
            ),
            encoding="utf-8",
...
def _nfs_fallback_collect(
    ...
    if not candidates:
        state_only_session = _find_nfs_state_session_dir(rec, current_session_hints)
        if state_only_session:
            n_emergency = _write_ci_emergency_artifacts(rec, state_only_session, task_dir)
            if n_emergency:
                log.warning(
                    "[task %s] NFS fallback synthesized %d emergency artifact(s) from state-only session %s",
                    rec.task_id,
                    n_emergency,
                    state_only_session,
                )
                return copied + n_emergency
        log.info(
    ...
    if not (task_dir / "session_breakdown.json").is_file():
        n_emergency = _write_ci_emergency_artifacts(rec, best_sess, task_dir)
        if n_emergency:
            copied += n_emergency
            log.warning(
                "[task %s] NFS fallback synthesized %d emergency artifact(s) from incomplete session %s",
                rec.task_id,
                n_emergency,
                best_sess,
            )
...
def wait_and_collect_one(
    ...
    has_breakdown = any(p.endswith("session_breakdown.json") for p in rec.artifact_files)
    if not has_breakdown:
        n_added = _write_claw_emergency_artifacts(rec, task_dir)
        if n_added:
            log.info("[task %s] Claw emergency fallback added %d files", rec.task_id, n_added)
