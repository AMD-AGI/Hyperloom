...
`install.sh` is idempotent and one-shot — no flags. It installs:
...
| OOB CLI + claude/codex npm CLIs + `~/.claude/config.json` + `~/.codex/auth.json` | `ensure_oob` (mirrors `${HYPERLOOM_BUNDLE}/OOB` → `${HYPERLOOM_ROOT}/OOB/oob_cli`) |
...
plus auth aliases — source it, don't derive these by hand.
...
| 8  | WARN-only presence check: `node` / `claude` / `codex` CLIs | — |
