...
class _CoordinatorStub:
    ...
    def _framework_discover_repo_urls(self, framework: str) -> list[str]:
        return Coordinator._framework_discover_repo_urls(self, framework)  # type: ignore[arg-type]
...
def test_repo_urls_cover_pr_intel_set_with_frameworkimary():
    ...
    urls = stub._framework_discover_repo_urls("sglang")
...
def test_discover_merges_candidates_across_repos(
    ...
    n_repos = len(stub._framework_discover_repo_urls("sglang"))
    assert len(seen_repo_urls) == n_repos
    batch = stub.shared_state.framework_batches[-1]
