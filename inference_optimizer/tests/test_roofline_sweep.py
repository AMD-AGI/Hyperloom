...
def test_extract_templates_optimized_via_envs_only() -> None:
    state = {"current_best": {"extra_server_args": "", "extra_envs": {"FOO": "1"}}}
...
def test_sweep_one_template_dispatches_per_conc(tmp_path: Path) -> None:
    ...
    with mock.patch.object(rs, "SglangServer", _FakeServer), mock.patch.object(rs, "run_bench", side_effect=fake_bench):
        rows = rs.sweep_one_template(
            tmpl=tmpl,
            model_path="/tmp/nonexistent_model",
            model_meta=meta,
            gpu_type="mi355x",
            tp=1,
            gpu_id=0,
    ...
    assert rows[1]["mbu_pct"] == pytest.approx(4000.0 / rows[1]["ceiling_tps"] * 100.0)
...
def test_sweep_one_template_marks_failed_bench_as_oom(tmp_path: Path) -> None:
    ...
    with mock.patch.object(rs, "SglangServer", _FakeServer), mock.patch.object(rs, "run_bench", return_value={}):
        rows = rs.sweep_one_template(
            tmpl=tmpl,
            model_path="/tmp/nonexistent_model",
            model_meta=meta,
