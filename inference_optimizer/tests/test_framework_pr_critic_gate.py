...
class _CoordinatorStub:
    """Holds just enough state for the gate helper to run (real ``_collect_framework_candidate_priors`` bound)."""
...
def _call_gate(stub: _CoordinatorStub, candidate: dict[str, Any]) -> dict[str, str]:
    return asyncio.run(
        Coordinator._critic_review_framework_candidate(  # type: ignore[arg-type]
            stub,
            candidate,
        ),
    )
...
def test_gate_returns_approve_for_mock_critic(tmp_path: Path) -> None:
    ...
    # Decision cached.
    assert len(stub.shared_state.framework_critic_decisions) == 1
    row = stub.shared_state.framework_critic_decisions[0]
...
def test_gate_abstains_when_no_critic_backend(tmp_path: Path) -> None:
    ...
    assert stub.shared_state.framework_critic_decisions == []
...
def test_gate_uses_cached_decision_on_repeat_call(tmp_path: Path) -> None:
    """Resume path: a repeat call reads ``framework_critic_decisions`` instead of re-invoking the Critic."""
...
def test_prompt_includes_session_local_priors(tmp_path: Path) -> None:
    ...
    backend = _PromptCapturingBackend()
    stub = _CoordinatorStub(tmp_path, backend)
    stub.shared_state.framework_critic_decisions.extend(
        [
            {
    ...
    stub.shared_state.framework_phase_progress.extend(
        [
            {
                "candidate_id": "pr-prev-2",
                "status": "reverted",
...
def test_priors_helper_trims_to_tail_length(tmp_path: Path) -> None:
    ...
    stub = _CoordinatorStub(tmp_path, backend=None)
    for i in range(12):
        stub.shared_state.framework_critic_decisions.append(
            {
                "candidate_id": f"pr-{i}",
                "verdict": "approve",
                "rationale": "",
        ...
        stub.shared_state.framework_phase_progress.append(
            {
                "candidate_id": f"pr-{i}",
                "status": "kept",
                "gain_pct": 1.0,
    ...
    priors = Coordinator._collect_framework_candidate_priors(stub)  # type: ignore[arg-type]
