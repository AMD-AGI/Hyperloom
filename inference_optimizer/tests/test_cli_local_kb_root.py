...
def test_resolve_local_kb_root_falls_back_to_user_data_path(
    ...
    """Tier 3: ``$USER_DATA_PATH/recipe_kb/`` when neither flag nor
    HYPERLOOM_LOCAL_KB_ROOT is set."""
    monkeypatch.setenv("USER_DATA_PATH", str(tmp_path))
    args = _ns()
    assert _resolve_local_kb_root(args) == tmp_path / "recipe_kb"


def test_resolve_local_kb_root_uses_workspace_default(
    env_clean: None,
) -> None:
    """Tier 4: the documented ``/workspace/hyperloom/recipe_kb`` last
    resort when no override is in scope."""
    args = _ns()
    assert _resolve_local_kb_root(args) == Path("/workspace/hyperloom/recipe_kb")
