...
# ===========================================================================
# (formerly test_n9_profile_blocked_from_propose.py)
# ===========================================================================
"""F3-1 N9: direct `profile` propose_action is hard-blocked when the
roofline composite is on.

Pins the F3-1 form of the N9 rule:
``PolicyGate._validate_roofline_composite_supersedes_profile`` — fires
at ``propose_action`` validation time, gated by the SharedState
two-toggle pair (``use_roofline_composite`` + ``deny_direct_profile``),
with rule ``n9_deny_direct_profile_when_composite_on``.
"""


def _policy_with_state(**state_overrides) -> PolicyGate:
    ss = SharedState()
