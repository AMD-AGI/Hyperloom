...
def _snapshot_with_ceiling(
    ...
    **kwargs: Any,
    ...
    snap = _snapshot(snapshot_id=snapshot_id, **kwargs)
    snap["theoretical_peak_tok_per_sec"] = theoretical_peak_tok_per_sec
    snap["achieved_tok_per_sec"] = achieved_tok_per_sec
    snap["within_roofline_pct"] = within_roofline_pct
    snap["gap_to_roofline_pct"] = gap_to_roofline_pct
    return snap


class TestCollectRooflineCeilingPropagation:
    """Ceiling fields stamped by ``record_trace_analyze`` must flow
    through ``collect_roofline`` into baseline/latest sub-dicts, plus
    the ceiling itself gets promoted to the top of the entry for
    consumers that don't want to reach into baseline/latest."""

    def test_ceiling_fields_present_on_baseline_and_latest(self):
        ...
        assert latest["theoretical_peak_tok_per_sec"] == 1000.0

    def test_ceiling_promoted_to_top_level_entry(self):
        """Dashboard consumers don't have to reach into baseline/latest
        for the session-level ceiling — collector promotes it."""
        snap = _snapshot_with_ceiling(
            snapshot_id=1,
            theoretical_peak_tok_per_sec=1000.0,
            achieved_tok_per_sec=527.5,
        ...
        out = collect_roofline({"roofline_snapshots": [snap]}, [])
        entry = out[0]
        assert entry.get("theoretical_peak_tok_per_sec") == 1000.0

    def test_top_level_ceiling_absent_when_snapshots_have_none(self):
        """When ceiling is unknown (legacy / unsupported gpu) the top
        promotion is suppressed so consumers can ``"k" in entry`` check
        to detect availability."""
        snap = _snapshot_with_ceiling(
            snapshot_id=1,
            theoretical_peak_tok_per_sec=None,
            achieved_tok_per_sec=None,
            within_roofline_pct=None,
            gap_to_roofline_pct=None,
        )
        out = collect_roofline({"roofline_snapshots": [snap]}, [])
        entry = out[0]
        assert "theoretical_peak_tok_per_sec" not in entry

    def test_before_after_delta_carries_within_roofline_pct(self):
        ...
        out = collect_roofline(
            {"roofline_snapshots": [snap_base, snap_latest]}, []
        )
        entry = out[0]
        assert entry["mode"] == "before_after"
        delta = entry["delta"]
        assert delta["within_roofline_pct"] == 16.34
