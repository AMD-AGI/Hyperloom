...
def _build_fixture(sd: Path) -> None:
    ...
    _write_json(
        sd / "manifest.json",
        {
            ...
            "gpu_type": "mi300x",
            "tp": 8,
            "workload": {"isl": 1024, "osl": 1024, "max_model_len": 6144, "precision": "fp8", "conc": 64},
    ...
    _write_json(
        sd / "state.json",
        {
            ...
            "baseline_attempts": [
                {
                    "ts": "2026-05-14T07:12:00+00:00",
                    "task_id": "t1",
                    "status": "succeeded",
                    "decision": "promoted",
                    "key_metric": 421.5,
                    "workspace": str(sd / "runs/baseline/t1"),
                },
            ],
            "profile_attempts": [
                {
                    "ts": "2026-05-14T07:14:00+00:00",
                    "task_id": "p1",
                    "status": "succeeded",
                    "decision": "promoted",
                    "key_metric": 421.5,
            ...
            "last_sweep": {
                "grid_size": 9,
                "best_overall": {
                    "variant_name": "variant_05_conc32_isl1024_osl1024",
                    "output_throughput": 812.0,
                    "conc": 32,
                },
                "best_for_each_conc": [
                    {"conc": 32, "tput": 812.0},
            ...
            "kernel_integrate_attempts": {
                "k001|patches/k001/0001.patch|": {
                    ...
                    "target_file": "/path/to/rmsnorm.py",
                    "extra_server_args": "",
                    "attempts": [
                        {
                            "decision": "KEEP",
            ...
            "explore_search": {
                "schema_version": 1,
                "accepted": [
                    {"name": "flag_X", "fingerprint": "g1", "gain_pct": 23.4, "ts": "2026-05-14T07:15:00+00:00"},
                    {
                        "name": "nccl_Y",
                        "fingerprint": "f1",
                        "extra_server_args": "--nccl-Y",
                        "extra_envs": {},
                        "output_throughput": 454.3,
                        "gain_pct": 7.8,
                        "ts": "2026-05-14T07:30:00+00:00",
                    },
                ],
    ...
    _write_json(cw / "emit.json", {"topic": "kernel_opt:k001", "ts": "2026-05-14T08:00:30+00:00"})

    rw = sd / "robustness-workdir/001"
    _write_json(rw / "signal.json", {"signal": "stall", "ts": "2026-05-14T07:55:00+00:00"})
    _write_json(rw / "action.json", {"action": "force_replan", "ts": "2026-05-14T07:55:30+00:00"})
...
def test_baseline_resolves_container_workspace_path(tmp_path: Path) -> None:
    ...
    assert not any("baseline workspace" in w and "does not resolve" in w for w in b["warnings"]), b["warnings"]
...
def test_baseline_resolves_measure_round_benchmark_workspace(tmp_path: Path) -> None:
    ...
    bdir = sd / ("runs/baseline/h1/measure_round/benchmark_sglang_20260605_014141")
    _write_json(
        bdir / "benchmark_report.json",
        {
            "success": True,
    ...
    _write_json(
        sd / "state.json",
        {
            ...
            "last_baseline": {
                "workspace": ("/workspace/runs/baseline/h1/measure_round/benchmark_sglang_20260605_014141"),
            },
        },
    )
...
def test_baseline_unresolvable_workspace_emits_warning(tmp_path: Path) -> None:
    ...
    assert any("baseline workspace" in w and "does not resolve" in w for w in b["warnings"]), b["warnings"]
        ...
        tmp_path,
        {
            ...
            "gain_per_stack_entry": [
                ...
                },
                {
                    ...
                    "stack_len_before": 1,
                    "stack_len_after": 2,
                    "cum_gain_before": 0.53,
                    "cum_gain_after": 22.85,
                    "delta_pct": 22.32,
...
def test_attribution_framework_agent_phase_fallback_when_no_phase_history(
    tmp_path: Path,
) -> None:
...
def test_attribution_phase_breakdown_gemm_tuning_by_tuned_file(
    ...
    sd = _attribution_fixture(
        tmp_path,
        {
            ...
            "optimization_stack": [
                {"action": "gemm_tuning", "variant_name": "a8w8_blockscale_tuned_gemm", "tuned_file": "/tmp/csv_a.csv"},
                {"action": "gemm_tuning", "variant_name": "a8w8_blockscale_tuned_gemm", "tuned_file": "/tmp/csv_b.csv"},
            ],
            "gain_per_stack_entry": [
                {
                    "action": "gemm_tuning",
...
def test_attribution_gemm_tuning_phase_fallback_when_no_phase_history(
    ...
    assert pb.get("kernel_agent", {}).get("total_gain_pct", 0.0) == 0.0
    assert "unattributed" not in pb or pb["unattributed"]["total_gain_pct"] == 0.0
...
def test_attribution_gemm_tuning_falls_back_to_variant_name_then_question_mark(
    ...
    sd = _attribution_fixture(
        tmp_path,
        {
            ...
            "optimization_stack": [
                {"action": "gemm_tuning", "variant_name": "a8w8_blockscale_tuned_gemm"},
                {"action": "gemm_tuning"},
            ],
            "gain_per_stack_entry": [
                {
...
def test_phase_breakdown_explore_by_domain_strips_specialist_prefix(
    ...
    sd = _attribution_fixture(
        tmp_path,
        {
            ...
            "explore_search": {
                "winners_history": [
                    {"fingerprint": "fp_serving", "provenance": "specialist:serving_specialist"},
                    {"fingerprint": "fp_kernel_switch", "provenance": "specialist:kernel_switch_specialist"},
                ],
            },
            "optimization_stack": [
...
def test_kernel_opt_summary_full_payload_passes_through(tmp_path: Path) -> None:
    ...
    payload = {
        ...
        "rejection_breakdown": {
            "revert_decision": 1,
            "max_failures_without_keep": 2,
            "max_partial_attempts_without_keep": 2,
            "other": 0,
        },
        "unattempted_reason_breakdown": {
            "no_source_file": 4,
            "not_reusable_native_kernel": 2,
            "no_recommended_backend": 2,
            "below_priority_cutoff": 1,
            "unknown": 0,
        },
        ...
        "by_kernel": [
            {
                "kernel_id": "k001",
                "kernel_name": "aiter::ck_moe_stage1",
                "kernel_category": "MoE",
                "source_file": "/abs/foo.cu",
                "gpu_pct": 9.2,
                "efficiency_pct": 48.3,
                "bound_type": "memory-bound",
                "arithmetic_intensity": 104.4,
                "category": "ATTEMPTED_REJECTED",
                "attempts_total": 3,
                "rejected_reason": "max_failures_without_keep",
...
def test_kernel_opt_summary_by_kernel_not_a_list_drops_to_empty(tmp_path: Path) -> None:
    ...
    payload = {"schema_version": 1, "totals": {"top_candidates": 1}, "by_kernel": {"k001": {"name": "broken"}}}
    sd = _report_fixture(tmp_path, "reports/kernel_optimization_summary.json", payload)
    bd = build(sd)
    ks = bd["kernel_optimization_summary"]
    assert ks["schema_version"] == 1
...
def test_conc_sweep_summary_full_payload_passes_through(tmp_path: Path) -> None:
    ...
    payload = {
        ...
        "optimized": {
            ...
            "points": [{"arm": "optimized", "conc": 8, "status": "succeeded", "output_throughput": 1313.54}],
        },
        "comparison": [
            {
                "conc": 8,
                "baseline_tput": 1300.34,
                "optimized_tput": 1313.54,
                "speedup": 1.0101,
                "delta_pct": 1.01,
        ...
        "roofline_ceiling": {
            ...
            "rows": [
                {
                    "conc": 8,
                    "t_peak_tok_s": 4032.76,
                    "bound_kind": "memory",
                    "mbu_baseline_pct": 66.17,
                    "mbu_optimized_pct": 66.14,
                }
            ],
        },
    }
...
def test_roofline_progress_no_snapshot_means_no_ceiling(tmp_path: Path) -> None:
    ...
    sd = _roofline_progress_fixture(
        ...
        state={
            ...
            "optimization_stack": [
                {"action": "explore", "tput": 105.0, "variant_name": "v1", "ts": "2026-05-29T11:00:00+00:00"},
            ],
        },
    )
    rl = build(sd)["roofline_progress"]
...
def test_roofline_progress_uses_latest_snapshot_for_ceiling(tmp_path: Path) -> None:
    ...
    sd = _roofline_progress_fixture(
        ...
        state={
            ...
            "optimization_stack": [],
            "roofline_snapshots": [
                {"snapshot_id": 1, "theoretical_peak_tok_per_sec": 1500.0, "ts": "2026-05-29T10:00:00+00:00"},
                {
                    "snapshot_id": 2,
                    "theoretical_peak_tok_per_sec": 1700.0,
                    "ts": "2026-05-29T11:00:00+00:00",
...
def test_roofline_progress_trajectory_diverges_from_current_best_emits_warning(
    ...
    bd = build(sd)
    assert any("roofline.current_best_tput" in w and "current_best.tput" in w for w in bd["warnings"])
...
def test_roofline_list_empty_when_no_snapshots(tmp_path: Path) -> None:
    ...
    sd = _roofline_progress_fixture(
        ...
        state={
            ...
            "cumulative_gain": 1.0,
            "optimization_stack": [
                {"action": "explore", "variant_name": "v1", "tput": 1313.0, "ts": "2026-05-29T11:00:00+00:00"},
            ],
        },
    )
    bd = build(sd)
...
def test_optimization_stack_passes_through_gemm_tuning_evidence(
            ...
            "optimization_stack": [
                {
                    ...
                    "extra_envs": {
                        "AITER_CONFIG_GEMM_A8W8_BLOCKSCALE": "/abs/path/a8w8_blockscale_tuned_gemm.csv",
                    },
                    "tput": 110.0,
    ...
    assert e["extra_envs"]["AITER_CONFIG_GEMM_A8W8_BLOCKSCALE"].endswith("a8w8_blockscale_tuned_gemm.csv")
...
def test_optimization_stack_preserves_promotion_order(
    ...
    sd = _roofline_progress_fixture(
        ...
        state={
            ...
            "optimization_stack": [
                {"action": "params", "variant_name": "p1", "tput": 110.0, "ts": "2026-06-01T10:00:00+00:00"},
                {
                    "action": "gemm_tuning",
                    "variant_name": "a8w8_tuned",
                    "tuned_file": "/abs/csv.csv",
                    "tput": 120.0,
...
def test_final_ttft_reconstructed_from_validate_stack(tmp_path: Path) -> None:
    ...
    assert any("final.ttft_mean_ms reconstructed from validate_stack_disk" in w for w in b["warnings"]), b["warnings"]
...
def test_final_ttft_reconstructed_from_warm_replay_measure_round(
    ...
    bdir = sd / ("runs/replay_warm_recipe/f86b/measure_round/benchmark_sglang_20260604_151328")
    _write_json(
        bdir / "benchmark_report.json",
        {
            "success": True,
...
def test_baseline_attempts_history_reconstructed_from_disk(tmp_path: Path) -> None:
    ...
    assert any("baseline.attempts_history reconstructed from runs/baseline/" in w for w in b["warnings"]), b["warnings"]
...
def test_baseline_attempts_history_state_recorded_takes_precedence(
    ...
    assert not any("baseline.attempts_history reconstructed" in w for w in b["warnings"])
...
def test_baseline_attempts_history_passes_through_error_excerpt(
    ...
    assert history[0]["error_excerpt"] == ("torch.OutOfMemoryError: HIP out of memory")
...
def test_session_image_from_env(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("HYPERLOOM_IMAGE", "registry.example/hyperloom:abc123")
    ...
    if b2["session"]["image"] is None:
        assert any("image: not configured" in w for w in b2["warnings"]), b2["warnings"]
...
def test_baseline_ttft_disk_walk_fallback(tmp_path: Path) -> None:
    ...
    assert any("baseline.ttft_mean_ms reconstructed from runs/baseline/ disk walk" in w for w in b["warnings"]), b[
        "warnings"
    ]
...
def test_framework_args_from_yaml_cmd_fallback(tmp_path: Path) -> None:
    ...
    _make_invocation_fixture(
        ...
        yaml_text=('cmd: "python -m sglang.launch_server --tp 4 --model /weka/m"\nbenchmark:\n  envs:\n    TP: "4"\n'),
    )
    inv = build(sd)["baseline"]["invocation"]
    assert "sglang.launch_server" in inv["framework_args"]
...
def test_framework_args_unknown_with_warning(tmp_path: Path) -> None:
    ...
    assert any("framework_args extraction failed" in w for w in b["warnings"]), b["warnings"]
...
def test_critic_kb_writes_summary_aggregates_by_verdict(tmp_path):
    ...
    for n, verdict in enumerate(("KEEP", "KEEP", "REVERT"), start=1):
        ...
        (iter_dir / "review.json").write_text(
            json.dumps(
                {
                    "verdict": verdict,
                    "summary": f"iter {n}",
        ...
        (iter_dir / "emit.json").write_text(
            json.dumps(
                {
                    "ts": f"2025-01-01T00:0{n}:00Z",
                    "topic": "review",
                }
            )
        )
...
def test_action_timeline_mirrors_phase_timeline(tmp_path):
    ...
    _write_state(
        ...
        _basic_state(
            phase_history=[
                {"to": "EXPLORE", "ts": "2025-01-01T00:00:00Z", "reason": "session_start", "evidence": {}},
            ],
        ),
    )
    b = build(sd)
