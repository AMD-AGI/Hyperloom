...
def test_phase_interleave_env_flag_is_picked_up(monkeypatch):
    """The helpers honour the env flag; WS3 makes interleave ON by default, only an explicit off value disables it."""
    # Unset / empty => ON by default (WS3).
    ...
    # Explicit on values keep interleave on.
    ...
    # Explicit off values disable interleave (opt-out).
