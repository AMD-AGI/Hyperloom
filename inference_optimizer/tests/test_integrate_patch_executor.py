...
# G2 — force RUN_EVAL for framework-authored source patches
def test_framework_run_eval_envs_forces_for_authored():
    assert IntegratePatchExecutor._framework_run_eval_envs({"framework_pr_authoring": True}) == {"RUN_EVAL": "true"}
    assert IntegratePatchExecutor._framework_run_eval_envs({"framework_pr_candidate_id": "c1"}) == {"RUN_EVAL": "true"}


def test_framework_run_eval_envs_none_for_generic_explore():
    # Generic EXPLORE integrate_patch (no framework markers) -> untouched.
    assert IntegratePatchExecutor._framework_run_eval_envs({"specialist_task_id": "s1"}) is None
