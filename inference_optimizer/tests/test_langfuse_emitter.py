...
# ---------------------------------------------------------------------------
# Debug probe (fast link-check)
# ---------------------------------------------------------------------------
def test_probe_sends_generation_and_flushes_immediately(tmp_path, monkeypatch):
    _enable_env(monkeypatch)
    client = _FakeClient()
    _install_fake_sdk(monkeypatch, client)
    sd = tmp_path / "SID"
    ...
    assert em.emit_probe(note="smoke") is True
    # A probe generation went out...
    assert len(client.generations) == 1
    assert client.generations[0].kwargs.get("name") == "probe:session-start"
    assert client.generations[0].kwargs["metadata"]["probe"] is True
    assert client.generations[0].kwargs["metadata"]["note"] == "smoke"
    # ...and was flushed right away (not held in the SDK batch buffer).
    assert client.flushed == 1
    assert em.receipt()["counts"]["generations_sent"] == 1


def test_probe_disabled_returns_false_no_client(tmp_path, monkeypatch):
    monkeypatch.delenv("HYPERLOOM_LANGFUSE_ENABLE", raising=False)
    sd = tmp_path / "SID"
    _write_manifest(sd)
    em = lfe.LangfuseEmitter(sd)
    assert em.enabled is False
    # Disabled probe is a no-op that reports False (caller logs the reason).
    assert em.emit_probe() is False


def test_probe_send_failure_is_swallowed(tmp_path, monkeypatch):
    _enable_env(monkeypatch)
    client = _FakeClient(raise_on_generation=True)
    _install_fake_sdk(monkeypatch, client)
    sd = tmp_path / "SID"
    _write_manifest(sd)
    em = lfe.LangfuseEmitter(sd)
    # A network blow-up never propagates; probe just reports False + counts error.
    assert em.emit_probe() is False
    assert em.receipt()["counts"]["errors"] >= 1


def test_module_level_emit_probe_uses_registry(tmp_path, monkeypatch):
    _enable_env(monkeypatch)
    client = _FakeClient()
    _install_fake_sdk(monkeypatch, client)
    sd = tmp_path / "SID"
    _write_manifest(sd)
    assert lfe.emit_probe(sd, note="hi") is True
    assert client.flushed == 1
    # Same singleton: the probe count is visible on the cached emitter.
    assert lfe.get_emitter(sd).receipt()["counts"]["generations_sent"] == 1
