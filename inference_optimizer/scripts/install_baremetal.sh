...
# Phase 3  dep checkouts   — inference_optimizer/scripts/local_setup.sh
#                            (clone KernelForge/OOB, InferenceX, TraceLens)
# Phase 4  runtime install — inference_optimizer/scripts/install.sh
...
DEFAULT_OPENAI_BASE_URL="https://your-openai-compatible-gateway.example.com/v1"
...
usage() {
  cat <<'EOF'
Usage: inference_optimizer/scripts/install_baremetal.sh [options]
...
write_combined_env() {
  ...
  {
    echo '#!/bin/sh'
    echo '# Generated by inference_optimizer/scripts/install_baremetal.sh'
