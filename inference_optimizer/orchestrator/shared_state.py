...
class SharedState:
    ...
    # Aggregate dynamic_action dispatches by ``dyn_id`` (Coordinator-only, CORE_STATE_FIELDS); value is lifecycle summary dict.
    dynamic_actions: dict[str, dict[str, Any]] = field(default_factory=dict)
    # Per-EXPLORE-round counter of successful dynamic_action dispatches (denials excluded); reset on fresh entry; Coordinator-only.
    ...
    def record_dynamic_action_dispatch(
        ...
        """Register a freshly-dispatched ``dynamic_action`` + bump the per-round counter atomically. Coordinator-only writer (CORE_STATE_FIELDS); idempotent on ``dyn_id``. Returns post-increment counter."""
    ...
    def reset_dynamic_action_round_count(self) -> None:
        """Clear the per-EXPLORE round cap counter; cumulative ``dynamic_actions`` ledger preserved."""
    ...
    def record_dynamic_action_outcome(
        ...
        *,
        ...
        """Transition-validated update of the summary row keyed by ``dyn_id``. Coordinator-only writer (CORE_STATE_FIELDS); illegal transitions logged and skipped."""
        # Local import avoids the cycle with dynamic_action_proposal.
    ...
    def to_dynamic_actions_prompt_section(
        ...
        *,
        ...
        """Compact ``=== <title> ===`` block for prompt injection; most-recent ``max_entries`` summaries (older collapsed to an elision marker), empty when no rows."""
    ...
    def _format_dynamic_action_summary_row(
        ...
        *,
        ...
        """Render one compact summary row (~50 tokens): dyn_id + status/gain header, motivation, artifact."""
