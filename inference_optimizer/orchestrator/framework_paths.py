...
def resolve_atom_arg_utils_path() -> tuple[Path, str]:
    ...
    Returns:
        tuple[Path, str]: ``(path, message)`` where the str is the file
            path on success or a diagnostic message on failure (same
            contract as the sister helpers).
    """
...
def summarise_framework_root_discovery(roots: str) -> str:
    ...
    Args:
        roots (str): Colon-separated roots emitted by
            :func:`probe_framework_source_roots_for_env`.

    Returns:
        str: A space-separated ``<framework>=ok|missing`` summary in
            :data:`_FRAMEWORK_BUCKETS` order.
    """
    parts: list[str] = []
