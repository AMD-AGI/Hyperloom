...
class ProfileExecutor(BaselineExecutor):
    ...
    async def __call__(self, ctx) -> dict[str, Any]:
        ...
        if round_trace_root:
            ...
            # pinned to on first launch. ``_candidate_trace_dirs`` is
            # workspace-local, so it never matches in multi-node —
            # handle it explicitly here.
            #
            # Mtime gate: every profile round writes into the same base
            # dir, so we filter to files created at-or-after this
            # round's ``task_started_unix``. Without this, the executor
            # would always pick up the FIRST round's stale trace.
