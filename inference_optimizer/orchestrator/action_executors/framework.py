...
"""FrameworkExecutor — FRAMEWORK phase per-candidate executor.
...
Inputs (``ctx.task.params``)::
    ...
    batch_id (str, optional) — passed back in the result so the phase
        loop can group ``framework_phase_progress`` entries.
...
class FrameworkExecutor:
    """ActionRunner for the ``framework`` action (FRAMEWORK phase)."""
    ...
    async def __call__(self, ctx) -> dict[str, Any]:
        ...
        output_root = Path(
            ...
            or runs_dir(self.session_dir, "framework", ctx.task.task_id)
        )
