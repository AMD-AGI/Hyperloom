...
def _magpie_probe_env(base: dict[str, str] | None = None) -> dict[str, str]:
    """Env for ``import Magpie`` probes without resolving MAGPIE_PYTHON."""
    env = dict(base or os.environ)
    magpie_dir = os.environ.get("MAGPIE_DIR", "").strip()
    if magpie_dir:
        existing = env.get("PYTHONPATH", "")
        parts = [p for p in existing.split(":") if p]
        if magpie_dir not in parts:
            env["PYTHONPATH"] = (
                f"{magpie_dir}:{existing}" if existing else magpie_dir
            )
    return env


_MAGPIE_PYTHON_CACHE: str | None = None


def _resolve_magpie_python() -> str:
    """Resolve the Python interpreter for Magpie subprocesses.

    Order: $MAGPIE_PYTHON env (must import Magpie) > first `python3` on PATH
    that can ``import Magpie`` > /opt/venv/bin/python (if it exists).
    """
    global _MAGPIE_PYTHON_CACHE
    if _MAGPIE_PYTHON_CACHE is not None:
        return _MAGPIE_PYTHON_CACHE
    ...
    if env_val:
        if _can_import_magpie(env_val):
            _MAGPIE_PYTHON_CACHE = env_val
            return _MAGPIE_PYTHON_CACHE
        log.warning(
            "MAGPIE_PYTHON=%r cannot import Magpie; falling back to PATH probe",
            env_val,
        )

    candidate = shutil.which("python3")
    if candidate and _can_import_magpie(candidate):
        _MAGPIE_PYTHON_CACHE = candidate
        return _MAGPIE_PYTHON_CACHE

    fallback = Path("/opt/venv/bin/python")
    if fallback.exists() and _can_import_magpie(str(fallback)):
        _MAGPIE_PYTHON_CACHE = str(fallback)
        return _MAGPIE_PYTHON_CACHE

    _MAGPIE_PYTHON_CACHE = (
        candidate or str(fallback) if fallback.exists() else "/opt/venv/bin/python"
    )
    return _MAGPIE_PYTHON_CACHE


def magpie_subprocess_env(base: dict[str, str] | None = None) -> dict[str, str]:
    """Build subprocess env with Magpie PYTHONPATH + MAGPIE_PYTHON wired."""
    env = _magpie_probe_env(base)
    env["MAGPIE_PYTHON"] = _resolve_magpie_python()
    return env
