...
class Coordinator:
    ...
    def __init__(
        ...
        *,
        ...
        self._maybe_clear_stale_kernel_opt_prune()
        ...
        self._auto_integrate_in_flight = False
        self._auto_trace_analyze_in_flight = False
        self._auto_kernel_opt_in_flight = False
        self._auto_baseline_in_flight = False
    ...
    def _maybe_clear_stale_kernel_opt_prune(self) -> None:
        """Auto-unprune kernel_opt when Ray + runtime env look healthy."""
        families = list(self.shared_state.pruned_families or [])
        env_path = os.environ.get(
            "KERNEL_AGENT_ENV",
            os.path.join(
                os.environ.get("HYPERLOOM_RUNTIME_DIR", ""),
                "kernel-agent.env.sh",
            ),
        )
        if not env_path or not Path(env_path).is_file():
            return
        ray_ok = self._ray_status_ok()
        if not ray_ok:
            return
        if "kernel_opt" in families:
            self.shared_state.pruned_families = [
                f for f in families if f != "kernel_opt"
            ]
            log.info(
                "resume preflight: cleared stale pruned_families kernel_opt "
                "(ray healthy, env present)",
            )
        cleared = self.shared_state.clear_ray_transient_kernel_rejections()
        if cleared:
            log.info(
                "resume preflight: cleared ray-transient rejected_kernel_ids=%s",
                cleared,
            )
        if "kernel_opt" in families or cleared:
            try:
                self.shared_state.save(self.session_dir)
            except Exception:  # noqa: BLE001
                log.exception("failed to persist kernel_opt ray recovery state")

    @staticmethod
    def _ray_status_ok() -> bool:
        try:
            return subprocess.run(
                ["ray", "status"], capture_output=True, timeout=15,
            ).returncode == 0
        except Exception:
            return False
    ...
    def _ray_gpu_count() -> int:
        try:
            ...
            for line in (proc.stdout or "").splitlines():
                if "GPU" in line and "0.0/" in line:
                    # e.g. " 0.0/8.0 GPU"
                    try:
    ...
    def _bootstrap_ray_for_kernel_opt(self, *, force_restart: bool = False) -> bool:
        """Ray hard-bootstrap before GEAK/kernel_opt dispatch."""
        root = os.environ.get("HYPERLOOM_KERNEL_AGENT_ROOT", "")
        if not root:
            return self._ray_status_ok()
        tools_dir = Path(root) / "tools" / "backends"
        log_path = self.session_dir / "ray_bootstrap.log"
        expected_gpus = self._ray_gpu_count() or 8
        script = (
            "import os, sys\n"
            f"sys.path.insert(0, {str(tools_dir)!r})\n"
            "from ray_runtime import bootstrap_ray_cluster\n"
            "try:\n"
            "    import torch\n"
            "    num_gpus = torch.cuda.device_count() or 0\n"
            "except Exception:\n"
            "    num_gpus = 0\n"
            f"bootstrap_ray_cluster(num_gpus=num_gpus, log_path={str(log_path)!r}, "
            f"force_restart={bool(force_restart)})\n"
            "print('ray_bootstrap_ok', num_gpus)\n"
        )
        try:
            proc = subprocess.run(
                ["python3", "-c", script],
            ...
            if proc.returncode != 0:
                log.warning(
                    "kernel_opt ray bootstrap failed rc=%s stderr=%r",
                    proc.returncode,
                    (proc.stderr or "")[-500:],
                )
        except Exception:  # noqa: BLE001
            log.exception("kernel_opt ray bootstrap raised")
        healthy = self._ray_status_ok()
        gpu_count = self._ray_gpu_count()
        if healthy and gpu_count > 0:
            log.info(
                "kernel_opt ray bootstrap ok: ray status reports %s GPU(s)",
                gpu_count,
            )
            reset = self.shared_state.reset_ray_transient_kernel_counters()
            if reset:
                log.info(
                    "kernel_opt ray bootstrap: reset ray_transient counters=%s",
                    reset,
                )
            self._maybe_clear_stale_kernel_opt_prune()
        elif healthy and gpu_count == 0:
            log.warning(
                "kernel_opt ray bootstrap: ray status ok but 0 GPUs visible "
                "(expected ~%s)",
                expected_gpus,
            )
        return healthy and gpu_count > 0

    def _preserve_validated_state_on_signal(self) -> None:
        """Do not regress validated E2E gain when stopped by SIGTERM/SIGINT."""
        disk_path = self.session_dir / "state.json"
        ...
        if not isinstance(on_disk, dict):
            return
        disk_validated = float(on_disk.get("cumulative_gain_validated") or 0.0)
        mem_validated = float(self.shared_state.cumulative_gain_validated or 0.0)
        if disk_validated > mem_validated:
            self.shared_state.cumulative_gain_validated = disk_validated
            self.shared_state.cumulative_gain_validated_ts = str(
                on_disk.get("cumulative_gain_validated_ts") or ""
            )
            self.shared_state.cumulative_gain_validated_stack_len = int(
                on_disk.get("cumulative_gain_validated_stack_len") or 0
            )
        disk_best = on_disk.get("current_best")
        mem_best = self.shared_state.current_best or {}
        disk_tput = float((disk_best or {}).get("tput") or 0.0)
        mem_tput = float((mem_best or {}).get("tput") or 0.0)
        if disk_tput > mem_tput and isinstance(disk_best, dict):
            self.shared_state.current_best = dict(disk_best)

    async def _auto_dispatch_pending_integrate(self) -> None:
        """Zero-touch: drain KEEP→integrate without waiting for LLM tick."""
        flag = os.environ.get("HYPERLOOM_AUTO_INTEGRATE", "1").strip().lower()
        if flag in ("0", "false", "no", "off"):
            return
        if self._auto_integrate_in_flight:
            return
        kid = self.shared_state.next_pending_keep_kernel_id()
        if not kid:
            return
        from .kernel_request_handlers import integrate_handler
        self._auto_integrate_in_flight = True
        try:
            log.info(
                "auto-integrate: dispatching integrate for kernel_id=%s", kid,
            )
            ...
            log.exception("auto-integrate failed for kernel_id=%s", kid)
        finally:
            self._auto_integrate_in_flight = False
        ...
        * ``max_ticks`` reached (test guard) → ``"max_ticks"``
        ...
        try:
            while not stop_reason:
                ...
                try:
                    ...
                    if not self._stop.is_set():
                        await self._pump_dispatcher_once()
                    if not in_closing:
                        await self._auto_dispatch_baseline_if_needed()
                        await self._auto_dispatch_pending_integrate()
                        await self._auto_dispatch_pending_kernel_work()
                        await self._maybe_zero_touch_fast_path_to_kernel()
                ...
                if objective.reached(self.shared_state):
                    if self._kernel_required_before_target_stop():
                        await self._defer_target_reached_for_kernel_work()
                    else:
            ...
            if stop_reason == "signal":
                self._preserve_validated_state_on_signal()
            self._finalize_journal_on_stop(stop_reason)
    ...
    def _kernel_required_before_target_stop(self) -> bool:
        """Return True when ``target_reached`` must wait for kernel work.

        Zero-touch GEAK path: explore-only stacks cannot satisfy
        ``--target-gain`` until at least one ``kernel_opt`` attempt
        has run and a KEEP integrate landed (or is still pending).
        """
        if not self._kernel_enabled():
            return False
        if not str(self.shared_state.last_profile_trace or "").strip():
            return False
        if self.shared_state.kernel_integrate_keep_satisfied():
            return False
        if self.shared_state.kernel_opt_attempts_count < 1:
            return True
        if self.shared_state.has_keep_pending_integrate:
            return True
        if not self.shared_state.kernel_opt_has_keep_decision():
            return True
        if self.shared_state.untried_hot_reusable_kernels():
            return True
        return True

    def _finalize_journal_on_stop(self, stop_reason: str) -> None:
        """Best-effort journal finalize on any terminal stop path."""
        if not stop_reason:
            return
        if getattr(self.shared_state, "close_sequence_done", False):
            return
        try:
            ...
            log.exception(
                "Coordinator.run: journal finalize failed on stop_reason=%s",
                stop_reason,
            )

    @staticmethod
    def _zero_touch_minimal_explore_enabled() -> bool:
        return os.environ.get(
            "HYPERLOOM_ZERO_TOUCH_MINIMAL_EXPLORE", "1",
        ).strip().lower() not in {"0", "false", "no", "off"}

    async def _maybe_zero_touch_fast_path_to_kernel(self) -> None:
        """Skip heavy EXPLORE in zero-touch mode once roofline is ready."""
        if not self._zero_touch_auto_kernel_enabled():
            return
        if not self._zero_touch_minimal_explore_enabled():
            return
        ...
        if not str(self.shared_state.last_profile_trace or "").strip():
            return
        explore_search = self.shared_state.explore_search or {}
        winners = explore_search.get("winners_history") if isinstance(
            explore_search, dict,
        ) else []
        if winners:
            return
        ...
        hint = str(self.shared_state.pending_escalate_hint or "").strip()
        if hint == ESCALATE_HINT_SKIP_TO_KERNEL:
            await self._advance_phase_if_needed()
            return
        log.info(
            "zero-touch minimal-explore: roofline ready with no explore "
            "winners; forcing skip_to_kernel",
        )
    ...
    async def _auto_dispatch_baseline_if_needed(self) -> None:
        """Zero-touch: enqueue baseline without waiting for LLM tick."""
        if not self._zero_touch_auto_kernel_enabled():
            return
        if self._auto_baseline_in_flight:
            return
        if float(self.shared_state.baseline_tput or 0.0) > 0:
            return
        phase = (self.shared_state.phase or "").strip().upper()
        if phase != _phase_state.PHASE_PRELUDE:
            return
        if self.shared_state.closing_phase or self.shared_state.stop_reason:
            return
        queued = await self.tasks.queued()
        for task in queued:
        ...
        for task in running:
            if task.kind == "baseline":
                return
        self._auto_baseline_in_flight = True
        try:
            await self._enqueue_internal_baseline_task(reason="zero_touch_prelude")
        except Exception:  # noqa: BLE001
            log.exception("auto-baseline enqueue failed")
        finally:
            self._auto_baseline_in_flight = False

    async def _enqueue_internal_baseline_task(self, *, reason: str) -> Task:
        """Build + enqueue a Coordinator-internal ``baseline`` task."""
        params: dict[str, Any] = {
            "source": "coordinator_internal",
            "reason": str(reason),
        }
        lanes, ttl = self._registry_lanes_ttl("baseline")
        task, was_existing = await self.tasks.create_or_return_existing(
            kind="baseline",
            params=params,
            idempotency_key=f"internal-baseline-{reason}",
            requires_lanes=lanes,
            lease_ttl_sec=ttl,
        )
        if was_existing:
            log.info(
                "internal-baseline task already exists (idempotent: "
                "task_id=%s, state=%s)",
                task.task_id, task.state,
            )
        else:
            log.info(
                "internal-baseline task enqueued (task_id=%s reason=%s)",
                task.task_id, reason,
            )
        return task

    @staticmethod
    def _zero_touch_auto_kernel_enabled() -> bool:
        """Bundle zero-touch kernel automation with auto-integrate."""
        return os.environ.get(
            "HYPERLOOM_AUTO_INTEGRATE", "1",
        ).strip().lower() not in {"0", "false", "no", "off"}

    async def _defer_target_reached_for_kernel_work(self) -> None:
        """Hold ``target_reached`` and route into KERNEL instead of stopping."""
        from .phase_state import ESCALATE_HINT_SKIP_TO_KERNEL

        phase = (self.shared_state.phase or "").strip().upper()
        log.info(
            "Deferring target_reached: kernel integrate KEEP not yet "
            "satisfied (phase=%s kernel_opt_attempts=%d "
            "integrate_keep=%s pending_integrate=%s)",
            phase or "<unset>",
            self.shared_state.kernel_opt_attempts_count,
            self.shared_state.kernel_integrate_keep_satisfied(),
            self.shared_state.has_keep_pending_integrate,
        )
        if phase == _phase_state.PHASE_EXPLORE:
            self.shared_state.set_pending_escalate_hint(
                ESCALATE_HINT_SKIP_TO_KERNEL,
    ...
    async def _auto_dispatch_pending_kernel_work(self) -> None:
        """Zero-touch: trace_analyze then run_optimization without LLM ticks."""
        if not self._zero_touch_auto_kernel_enabled():
            return
        if not self._kernel_enabled():
            return
        if self.shared_state.closing_phase or self.shared_state.stop_reason:
            return
        await self._auto_dispatch_trace_analyze_if_needed()
        await self._auto_dispatch_run_optimization_if_needed()

    async def _auto_dispatch_trace_analyze_if_needed(self) -> None:
        """Re-run trace_analyze when profile exists but cache is stale/empty."""
        if self._auto_trace_analyze_in_flight:
            return
        ...
        cache_ok = (
            isinstance(cached, dict)
            and cached.get("trace_input") == trace
            and bool(cached.get("candidates_path"))
        )
        if cache_ok:
            return
        from .kernel_request_handlers import trace_analyze_handler

        self._auto_trace_analyze_in_flight = True
        try:
            log.info(
                "auto-trace_analyze: dispatching for trace_input=%r", trace,
            )
            payload = {"trace_input": trace, "top_k": 10}
            result = await trace_analyze_handler(
                payload,
                session_dir=self.session_dir,
            )
            if isinstance(result, dict) and result.get("status") == "ok":
                self.shared_state.record_trace_analyze(payload, result)
                self.shared_state.save(self.session_dir)
            else:
                log.warning(
                    "auto-trace_analyze: failed status=%r error=%r",
                    result.get("status") if isinstance(result, dict) else None,
                    result.get("error") if isinstance(result, dict) else result,
                )
        except Exception:  # noqa: BLE001
            log.exception("auto-trace_analyze failed")
        finally:
            self._auto_trace_analyze_in_flight = False

    async def _auto_dispatch_run_optimization_if_needed(self) -> None:
        """Zero-touch: fan out GEAK on untried hot kernels once unlocked."""
        if self._auto_kernel_opt_in_flight:
            return
        if not self._kernel_opt_unlocked():
            return
        if self._gemm_tuning_required_before_kernel_opt():
            return
        if self.shared_state.next_pending_keep_kernel_id():
            return
        owes_ray_retry = self.shared_state.kernel_phase_blocked_on_ray_unresolved(
            ray_healthy=False,
        )
        if (
            not self.shared_state.untried_hot_reusable_kernels()
            and not owes_ray_retry
        ):
            return
        cached = self.shared_state.last_trace_analyze or {}
        candidates_path = str(cached.get("candidates_path") or "")
        if not candidates_path:
            return
        owes_ray = self.shared_state.kernel_opt_owes_ray_recovery()
        if not self._bootstrap_ray_for_kernel_opt(
            force_restart=True,
        ):
            log.warning(
                "auto-run_optimization: Ray unhealthy after bootstrap; deferring "
                "(owes_ray_recovery=%s)",
                owes_ray,
            )
            return
        ...
        self._auto_kernel_opt_in_flight = True
        try:
            log.info(
                "auto-run_optimization: dispatching batch on %s",
                candidates_path,
            )
            result = await run_optimization_handler(
                {
            ...
            log.exception("auto-run_optimization failed")
        finally:
            self._auto_kernel_opt_in_flight = False
