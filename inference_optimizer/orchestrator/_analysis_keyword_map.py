...
Lets PolicyGate detect when the LLM under-includes analysis-implied variants
and surface a non-blocking advisory. Advisory-not-blocking (keeps LLM agency);
case-insensitive substring match; intersected with available per-framework
variants.
"""
...
# Curated map: analysis.md substring (case-insensitive) -> required variant
# names. Intentionally inclusive on the variant side (a keyword maps to the
# whole family); executor grid de-dup skips variants already tested.
...
def extract_required_variants_from_analysis(
    ...
    1. A sorted list of analysis-implied variant names, narrowed to
       ``available_variants`` (per-framework).
...
def format_missing_variants_advice(
    ...
    *,
    ...
    if not missing:
        return None
    # Per-keyword breakdown so the LLM sees why each missing variant is implied.
    trigger_lines = []
