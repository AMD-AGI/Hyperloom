...
def _section_decision_framework(*, kernel_enabled: bool) -> list[str]:
    ...
    lines.extend([
        ...
        "        params={benchmark_script: '<this_run_framework>_<gpu>.sh',",
        "                result_dir: '<session_dir>/runs/baseline/<task>/leak'},",
        "        predicted_gain_pct: 0,",
        "        notes: 'recover from no_report streak — pin the framework-matching",
        "                script that honors $RESULT_DIR; default model script",
        "                hardcodes /workspace/ (never cross frameworks)'}",
        "",
