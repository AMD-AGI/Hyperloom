...
_ALLOWED_SHAPE_PROVENANCE = frozenset({"torch_trace", "tuning_csv", "workload_config"})
...
def _enrich_candidates_artifact(
    ...
    *,
    ...
    model = model_name or str(
        (metadata.get("runtime_args") or {}).get("model") or ""
    )
    ...
    try:
        ...
        if spec is not None and spec.loader is not None:
            ...
            enrich_candidates_with_workload_shapes = getattr(
                mod, "enrich_candidates_with_workload_shapes", None,
            )
    except Exception as exc:  # noqa: BLE001
        log.warning("workload shape enrichment unavailable: %s", exc)
    for key in ("hot_kernels", "hot_kernels_top15"):
        kernels = data.get(key)
        if isinstance(kernels, list) and enrich_candidates_with_workload_shapes:
...
async def _run_subprocess(cmd: list[str], *, timeout_sec: int) -> tuple[int, str, str]:
    ...
    def _run() -> subprocess.CompletedProcess[str]:
        ...
        tools_dir = str(_kernel_agent_tool_path("tracelens_analysis.py").parent)
...
# ---------------------------------------------------------------------------
_STEADY_STATE_MODE_LADDER = ("mixed", "prefilldecode", "decode_only")


def _trace_analyze_retryable(result: dict[str, Any]) -> bool:
    """True when a different steady-state mode may recover trace analysis."""
    if result.get("status") in ("ok", "succeeded"):
        return False
    err = str(result.get("error") or "")
    if "trace_split_no_steady_state" in err:
        return True
    for warning in result.get("trace_health_warnings") or []:
...
async def trace_analyze_handler(
    ...
    # T1: auto-retry steady-state mode ladder on split/chunk failures.
    requested_mode = str(
    ...
    modes_to_try = [requested_mode] + [
        m for m in _STEADY_STATE_MODE_LADDER if m != requested_mode
    ]
    result: dict[str, Any] = {"status": "failed", "error": "trace_analyze not run"}
    for mode in modes_to_try:
        run_cmd = list(cmd)
        if "--steady-state-mode" in run_cmd:
            idx = run_cmd.index("--steady-state-mode")
            run_cmd[idx + 1] = mode
        else:
            run_cmd += ["--steady-state-mode", mode]
        rc, stdout, stderr = await _run_subprocess(run_cmd, timeout_sec=timeout_sec)
        result = _shape_tool_result(rc, stdout, stderr)
        result["steady_state_mode_used"] = mode
        if not _trace_analyze_retryable(result):
            break
        log.info(
            "trace_analyze: steady_state_mode=%s failed (%s); retrying ladder",
            mode,
    ...
    if isinstance(result, dict):
        ...
        if isinstance(result.get("hot_kernels"), list):
            try:
                import importlib.util
                import sys
                tl_path = _kernel_agent_tool_path("tracelens_analysis.py")
                tools_dir = str(tl_path.parent)
                ...
                if spec is not None and spec.loader is not None:
                    mod = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(mod)
                    enrich = getattr(mod, "enrich_candidates_with_workload_shapes", None)
                    if enrich:
                        enrich(
                            result["hot_kernels"],
                            workload=workload if isinstance(workload, dict) else {},
                ...
                log.warning("hot_kernels workload shape enrichment failed: %s", exc)
    return result
...
def _geak_budget_minutes(payload: dict) -> float:
    ...
    name = str(payload.get("name") or payload.get("kernel_name") or "").lower()
    src = str(payload.get("source_file") or "").lower()
    base = Path(src).name.lower() if src else ""
    is_fused_moe = (
        "fused_moe" in name
        or "fused_moe" in src
        or base in ("fused_moe_triton_kernels.py", "fused_moe_triton_config.py")
    )
    if is_fused_moe:
        try:
            floor = float(os.environ.get("HYPERLOOM_FUSED_MOE_GEAK_BUDGET_MIN", "180"))
        except (TypeError, ValueError):
            floor = 180.0
...
async def _run_optimization_single(
    ...
    if isinstance(result, dict):
        ...
        from .shared_state import is_ray_transient_kernel_opt_failure
        if is_ray_transient_kernel_opt_failure(result):
            result["error_class"] = "ray_transient"
    return result
...
async def integrate_handler(
    ...
    # Cache-invalidation rebuild env (#485 + #459): when apply_kernel_patch
    # moved a toolchain's compiled/runtime cache aside (aiter cpp_itfs, Triton,
    # torch inductor, ...), the re-baseline server must (re)compile the patched
    ...
    #
    # chaojhou review (point e) CONCURRENCY CAVEAT: this mutates the
    # process-global ``os.environ``. integrate_handler runs serially per
    # session (one re-baseline server at a time), so there is no in-process
    # race today, and the override is ALWAYS restored (``_restore_rebuild_env``
    # in the ``except``/``finally`` below) so it never leaks past this call. If
    # integrate is ever made concurrent within a single process, this must
    # become a lock-guarded section or a per-subprocess env hand-off instead of
    # mutating os.environ.
    ...
    if is_multi_node():
        ...
        try:
            ...
            # This path returns before the bench try/finally below, so restore
            # the rebuild env override here explicitly.
    ...
    # Don't score a stale binary (#485, generalizing #459 point 2): for any
    # toolchain whose cache the apply moved aside (cpp_itfs runtime lib.so,
    # Triton .hsaco, inductor artifact), a re-baseline that reused the stale
    # binary would silently measure the PRE-patch kernel. Before trusting
    # gain_pct, assert a real rebuild landed: verify_rebuilt_for_apply_result()
    # runs the detected toolchain's fresh-build check (a freshly-compiled
    # artifact newer than the invalidation) and is a strict no-op
    # (verified=True) when no gating cache was invalidated -- so this gate is
    # byte-for-byte off the cache-invalidation paths.
    #
    # chaojhou review (point d): this is an INDEPENDENT correctness/served-
    # binary gate -- it does NOT infer correctness from throughput gain_pct.
    # It only refuses to trust a measurement taken against a binary we cannot
    # prove was freshly rebuilt from the patched source.
    #
