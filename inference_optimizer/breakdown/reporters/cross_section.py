...
def _gain_attribution_lines(
    ...
    Returns:
        tuple[list[str], str]: A list of human-readable attribution lines and
            a label describing the attribution method used (e.g. ``"validated"``,
            ``"single-source ..."``, ``"best-effort reconstructed"`` or
            ``"missing"``).
    """
...
def _data_quality_flags(
    ...
    Args:
        breakdown (dict[str, Any]): The full ``session_breakdown.json`` dict.
        rendered (list[RenderedSection]): The already-rendered sections whose
            per-section warnings should be folded in (skipped sections are
            ignored).

    Returns:
        list[str]: De-duplicated, section-tagged data-quality flag strings.
    """
    flags: list[str] = []
    seen: set[str] = set()
